...
namespace Tuhu.Service.Activity.DataAccess
{
    public class DalFlashSale
    {
        ...
        /// <returns></returns>
        public static async Task<FlashSaleModel> GetFlashSaleFromDBActivityPage(Guid activityID, List<string> pids, bool isreadonly = true)
        {
            Logger.Info($"GetFlashSaleFromDBActivityPage查DB=>参数activityID=》{activityID}");
            string sql = @" SELECT FS.ActivityID ,
                                   FS.ActivityName ,
                                   FS.StartDateTime ,
                                   FS.EndDateTime ,
                                   FS.CreateDateTime ,
            ...
            if (pids?.Count > 0)
            {
                sql += " JOIN Activity..SplitString(@pids, ',', 1) AS B ON FSP.PID = B.Item";
            }

            sql += @"   WHERE  FS.ActivityID = @ActivityID
                         ORDER BY ISNULL(FSP.Position, 1000),FS.StartDateTime,FSP.PKID;";
        ...
        /// <param name="isreadonly"></param>
        /// <returns></returns>
        public static async Task<Tuple<List<FlashSaleModel>, List<FlashSaleProductModel>>> SelectFlashSaleListFromDBAsync(List<Guid> activityIDs, bool isreadonly = true)
        {

            Logger.Warn($"SelectFlashSaleListFromDBAsync,查询DB");
            ...
            string sql = @" SELECT FS.ActivityID ,
                           ...
                           FROM    Activity..tbl_FlashSale AS FS WITH ( NOLOCK )
                                   JOIN Activity..tbl_FlashSaleProducts AS FSP WITH ( NOLOCK ) ON FS.ActivityID = FSP.ActivityID
                                   JOIN Tuhu_productcatalog..vw_Products AS VP ON FSP.PID = VP.PID
                                   JOIN Activity..SplitString(@activityIDs, ',', 1) AS B ON FS.ActivityID = B.Item
                         ORDER BY ISNULL(FSP.Position, 1000),FS.StartDateTime,FSP.PKID;";
            var flashList = new List<FlashSaleModel>();//活动信息
            var flashProductList = new List<FlashSaleProductModel>();//活动商品信息
            using (var cmd = new SqlCommand(sql))
            {
                ...
                var FlashSale = await DbHelper.ExecuteQueryAsync(isreadonly, cmd, dt =>
                {
                    if (dt == null || dt.Rows.Count == 0)
                        return false;
                    flashList = dt.ConvertTo<FlashSaleModel>()?.ToList();
                    flashProductList = dt.ConvertTo<FlashSaleProductModel>()?.ToList();
                    return true;
                });
            }
            return Tuple.Create(flashList, flashProductList);
        }
        ...
        /// <param name="isreadonly"></param>
        /// <returns></returns>
        public static async Task<Tuple<List<FlashSaleActivityPageModel>, List<FlashSaleActivityPageProductModel>>> GetFlashSaleListActivitPageAsync(List<Guid> activityIDs, bool isreadonly = true)
        {
            string sql = @" SELECT FS.ActivityID ,
                                   FS.ActivityName ,
                                   FS.StartDateTime ,
                           ...
                           FROM    Activity..tbl_FlashSale AS FS WITH ( NOLOCK )
                                   ...
                                   JOIN Activity..SplitString(@activityIDs, ',', 1) AS B ON FS.ActivityID = B.Item
                         ORDER BY ISNULL(FSP.Position, 1000),FS.StartDateTime,FSP.PKID;";
            var flashList = new List<FlashSaleActivityPageModel>();//活动信息
            var flashProductList = new List<FlashSaleActivityPageProductModel>();//活动商品信息
            using (var cmd = new SqlCommand(sql))
            {
                ...
                var FlashSale = await DbHelper.ExecuteQueryAsync(isreadonly, cmd, dt =>
                {
                    if (dt == null || dt.Rows.Count == 0)
                        return false;
                    flashList = dt.ConvertTo<FlashSaleActivityPageModel>()?.ToList();
                    flashProductList = dt.ConvertTo<FlashSaleActivityPageProductModel>()?.ToList();
                    return true;
                });
            }
            return Tuple.Create(flashList, flashProductList);
        ...
        /// <param name="isreadonly"></param>
        /// <returns></returns>
        public static async Task<FlashSaleModel> SelectSecKillFlashSaleFromDBAsync(Guid activityID, bool isreadonly = true)
        {
            using (var cmd = new SqlCommand(@"SELECT  FS.ActivityID ,
                                                      FS.ActivityName ,
                                                      FS.StartDateTime ,
                                                      ...
                                                      VP.CP_Brand
                                              FROM    Activity..tbl_FlashSale AS FS WITH ( NOLOCK )
                                                      JOIN Activity..tbl_FlashSaleProducts AS FSP WITH ( NOLOCK ) ON FSP.IsShow = 1 AND FS.ActivityID = FSP.ActivityID
                                                                                                            AND FS.ActivityID = @ActivityID
                                                      JOIN Tuhu_productcatalog..vw_Products AS VP ON FSP.PID = VP.PID
                                              ORDER BY FS.StartDateTime,ISNULL(FSP.Position, 1000),FSP.PKID;")
            )
            {
                cmd.CommandType = System.Data.CommandType.Text;
                cmd.Parameters.AddWithValue("@ActivityID", activityID);
        ...
        /// <returns></returns>
        public static async Task<List<FlashSaleProductModel>> SelectFlashSaleProductPagerAsync(Guid activityId, bool isreadonly = true, int pageNumber = 0, int pageSize = 4)
        {
            using (var cmd = new SqlCommand(@"SELECT  FS.ActivityID ,
                                                      FSP.PKID ,
                                                      FSP.PID ,
                                                      ...
                                                      FSP.InstallService,
                                                      VP.CP_Brand,
                                                      ISNULL(VP.cy_marketing_price,0) AS FalseOriginalPrice
                                              FROM    Activity..tbl_FlashSale AS FS WITH ( NOLOCK )
                                                      JOIN Activity..tbl_FlashSaleProducts AS FSP WITH ( NOLOCK ) ON FSP.IsShow = 1 AND FS.ActivityID = FSP.ActivityID AND FS.ActivityID = @ActivityID
                                                      JOIN Tuhu_productcatalog..vw_Products AS VP ON FSP.PID = VP.PID
                                              ORDER BY FS.StartDateTime,ISNULL(FSP.Position, 1000),FSP.PKID
				                              ...
				                              FETCH NEXT @PageSize ROW ONLY;"))
            {
                ...
                var productList = await DbHelper.ExecuteQueryAsync(isreadonly, cmd, dt =>
                {
                    if (dt == null || dt.Rows.Count == 0)
                        return null;
                    return dt.ConvertTo<FlashSaleProductModel>();
                });
                return productList?.ToList() ?? new List<FlashSaleProductModel>();
            }
        }

        public static async Task<IEnumerable<FlashSaleModel>> SelectFlashSaleFromDbByActivityIdsAsync(Guid[] activityIds,
            bool isReadOnly = true)
        {
            using (var cmd = new SqlCommand(@"SELECT  FS.ActivityID ,
                                                      ...
                                                      VP.CP_Brand
                                              FROM    Activity..tbl_FlashSale AS FS WITH ( NOLOCK )
                                                      JOIN Activity..tbl_FlashSaleProducts AS FSP WITH ( NOLOCK ) ON FS.ActivityID = FSP.ActivityID AND FS.ActivityID IN ( SELECT * FROM Gungnir.dbo.Split(@ActivityIDs, ',') )
                                                      JOIN Tuhu_productcatalog..vw_Products AS VP ON FSP.PID = VP.PID;")
            )
            {
                cmd.CommandType = CommandType.Text;
                ...
                var flashSale = await DbHelper.ExecuteQueryAsync(isReadOnly, cmd, dt =>
                {
                    ...
                    list = dt.ConvertTo<FlashSaleModel>()?.ToList();
                    ...
                    {
                        item.Products = dt.Select($"ActivityID = '{item.ActivityID}'").ConvertTo<FlashSaleProductModel>();
                    }

                    return list.GroupBy(a => a.ActivityID).Select(it => it.First()).ToList();
        ...
        /// <param name="activityId"></param>
        /// <returns></returns>
        public static async Task<IEnumerable<FlashSaleProductModel>> GetFlashSaleSaleOutQuantityActivityPage(
            Guid activityId, List<string> pids)
        {

            return await DbHelper.ExecuteSelectAsync<FlashSaleProductModel>(true, @"SELECT  FSP.PID ,
                                        FSP.SaleOutQuantity
                                FROM    Activity.dbo.tbl_FlashSaleProducts AS FSP WITH ( NOLOCK )
                                        JOIN Activity..SplitString(@Pids, ',', 1) AS B ON FSP.PID = B.Item
                                WHERE   FSP.ActivityID = @ActivityID;",
                CommandType.Text, new SqlParameter("@ActivityID", activityId),
        ...
        {
            string sql = @"SELECT   FSP.ActivityID ,
                                    FSP.PID ,
                                    FSP.Quantity
            ...
            {
                sql += " OR FSP.DeviceID = @DeviceID";
            }
            sql += ");";

            return await DbHelper.ExecuteSelectAsync<FlashSaleRecordsModel>(true, sql, CommandType.Text,
                  new SqlParameter[]
                  {
            ...
            string deviceId, Guid ActivityID)
        {
            ...
            {
                return await dbhelper.ExecuteSelectAsync<FlashSaleRecordsModel>(sql, CommandType.Text,
...
Where FS.ActivityId=@ActivityID"))
    ...
    string deviceId, Guid activityId, string userTel, List<string> pids)
        ...
        /// <returns></returns>
        public static async Task<List<FlashSaleModel>> SelectSecondKillTodayData(int activityType, DateTime dt)
        {
            var model = await DbHelper.ExecuteSelectAsync<FlashSaleModel>(true, @"SELECT  FS.ActivityID ,
                                                                                          FS.ActivityName ,
                                                                                          FS.StartDateTime ,
                                                                                          FS.EndDateTime ,
                                                                                          FS.CreateDateTime ,
                                                                            ...
                                                                            WHERE   FS.ActiveType = @ActiveType
                                                                                    AND FS.StartDateTime >= CONVERT(VARCHAR(100), CONVERT(DATE, @Dt), 23)
                                                                                    AND FS.StartDateTime <= CONVERT(VARCHAR(100), DATEADD(DAY, 1, CONVERT(DATE, @Dt)), 23)
                                                                                    AND FS.EndDateTime >= CONVERT(VARCHAR(100), CONVERT(DATE, @Dt), 23)
                                                                                    AND FS.EndDateTime <= CONVERT(VARCHAR(100), DATEADD(DAY, 1, CONVERT(DATE, @Dt)), 23);",
                CommandType.Text, new SqlParameter("@ActiveType", activityType), new SqlParameter("@Dt", dt));
            return model?.ToList() ?? new List<FlashSaleModel>();
        }
        ...
        /// <returns></returns>
        public static async Task<IEnumerable<Guid>> GetSecondsBoysDataSqlAsync(int activityType, DateTime? startDate, DateTime? endDate)
        {
            var model = await DbHelper.ExecuteSelectAsync<FlashSaleModel>(true,
                @"SELECT  FS.ActivityID
                FROM    Activity..tbl_FlashSale AS FS WITH ( NOLOCK )
                WHERE   FS.ActiveType = @ActiveType
                        AND FS.StartDateTime >= CONVERT(VARCHAR(100), CONVERT(DATE, @StartDate), 20)
                        AND FS.EndDateTime <= CONVERT(VARCHAR(100), DATEADD(DAY, 1, CONVERT(DATE, @EndDate)), 20);",
                CommandType.Text,
                new SqlParameter[]
                {
                    new SqlParameter("@ActiveType", activityType),
...
WITH    pids
    ...
    WHERE   ( tfs.StartDateTime >= @sSchedule
              ...
              OR tfs.StartDateTime < @sSchedule
              AND tfs.EndDateTime > @sSchedule
              OR tfs.StartDateTime < @eSchedule
              AND tfs.EndDateTime > @eSchedule
            )
            AND tfs.ActiveType = 1
            AND IsDefault = 0;"))
            {
                cmd.CommandType = CommandType.Text;
        ...
        /// 活动页秒杀场次查询
        /// </summary>
        /// <param name="topNumber">场次top数量</param>
        /// <returns></returns>
        public static async Task<List<FlashSaleModel>> GetActivePageSecondKillSession(int topNumber)
        {
            string sqlGetActivePageSecondKillSession = @"SELECT TOP " + topNumber + @" FS.ActivityID ,
                                FS.ActivityName ,
                                FS.StartDateTime ,
                                FS.EndDateTime ,
                                FS.CreateDateTime ,
                                ...
                                FS.IsNewUserFirstOrder
                        FROM    Activity..tbl_FlashSale AS FS WITH ( NOLOCK )
                        WHERE   FS.ActiveType = 3
        AND ( ( FS.StartDateTime <= CONVERT(VARCHAR(16), DATEADD(DAY, 1,
                                                              CONVERT(DATETIME, GETDATE())), 20)
                AND FS.StartDateTime >= CONVERT(VARCHAR(16), CONVERT(DATETIME, GETDATE()), 20)
              )
              OR ( FS.EndDateTime >= CONVERT(VARCHAR(16), CONVERT(DATETIME, GETDATE()), 20))
            ) ORDER BY FS.StartDateTime";

            var model = await DbHelper.ExecuteSelectAsync<FlashSaleModel>(true, sqlGetActivePageSecondKillSession,
                CommandType.Text);
            return model?.ToList() ?? new List<FlashSaleModel>();
        }
        ...
        /// <param name="activityId"></param>
        /// <returns></returns>
        public static async Task<IEnumerable<FlashSaleActivityPageProductModel>> GetFlashSaleSaleQuantityActivityPage(Guid activityId)
            =>
                await DbHelper.ExecuteSelectAsync<FlashSaleActivityPageProductModel>(true, @"SELECT   FSP.PID ,
                                                                                      FSP.SaleOutQuantity
                                                                             FROM     Activity.dbo.tbl_FlashSaleProducts AS FSP WITH ( NOLOCK )
                                                                             WHERE    FSP.ActivityID = @ActivityID;",
                    CommandType.Text, new SqlParameter("@ActivityID", activityId));
