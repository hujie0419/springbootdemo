...
namespace Tuhu.C.Job.ThirdPart.Manager.GFCard
{
    public class GFSmsTaskExecutor
    {
        ...
        /// <returns></returns>
        public async Task<IEnumerable<GFBankSmsTask>> ExecuteTasks(DateTime sendTime)
        {
            ...
            if (sendSmsResult)
            {
                await _gfDal.BatchUpdateGFBankSmsTasksStatus(_task, nameof(GFTaskStatus.Complated));
                return new List<GFBankSmsTask>();
            }
            else
            {
                await _gfDal.BatchUpdateGFBankSmsTasksStatus(_task, nameof(GFTaskStatus.SmsFailed));
                return _task;
            }
        }
        /// <summary>
        ...
        /// <param name="sendTime"></param>
        /// <returns></returns>
        public async Task<IEnumerable<GFBankSmsTask>> ExecuteSmsFailedTasks(DateTime sendTime)
        {
            if (_task == null || !_task.Any())
                return new List<GFBankSmsTask>();
            var mobiles = _task.Select(s => s.Mobile).Distinct();
            ...
            {
                await _gfDal.BatchUpdateGFBankSmsTasksStatus(_task, nameof(GFTaskStatus.RetrySmsFailed));
                return _task;
            }
        }
    }
