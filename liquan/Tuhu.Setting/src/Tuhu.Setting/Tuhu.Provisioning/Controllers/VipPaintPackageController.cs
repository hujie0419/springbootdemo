...
namespace Tuhu.Provisioning.Controllers
{
    public class VipPaintPackageController : Controller
    {
        ...
        public ActionResult ExportExcelTemplate()
        {
            ...
            row.CreateCell(cellNum++).SetCellValue("券有效期开始时间");
            row.CreateCell(cellNum++).SetCellValue("券有效期结束时间");
            cellNum = 0;
        ...
        private static Tuple<List<VipPaintPromotionTemplateModel>, string> ConvertExcelToList(ISheet sheet)
        {
            ...
            if (titleRow.GetCell(index++)?.StringCellValue == "手机号" &&
                ...
                titleRow.GetCell(index++)?.StringCellValue == "塞券数量" &&
                 titleRow.GetCell(index++)?.StringCellValue == "券有效期开始时间" &&
                  titleRow.GetCell(index++)?.StringCellValue == "券有效期结束时间"
                )
            {
                ...
                for (var rowIndex = sheet.FirstRowNum + 1; rowIndex <= sheet.LastRowNum; rowIndex++)
                {
                    ...
                    string startDateTime = getStringValue(row.GetCell(cellIndex++));
                    DateTime? startDate = null;
                    if (DateTime.TryParse(startDateTime, out DateTime date))
                    {
                        startDate = date.Date;
                    }
                    var endDateTime = getStringValue(row.GetCell(cellIndex++)); 
                    ...
                    {
                        message = $"第{rowIndex + 1}行格式不正确, 时间必须全部填写或者全部不写";
                        break;
                    }
                    if ((startDate != null && endDate != null && (startDate.Value < SqlDateTime.MinValue.Value || endDate.Value < SqlDateTime.MinValue.Value)))
                    {
