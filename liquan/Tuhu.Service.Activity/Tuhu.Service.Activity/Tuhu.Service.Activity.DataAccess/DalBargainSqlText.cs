...
FROM    Activity..BargainOwnerAction AS BO WITH ( NOLOCK )
        LEFT JOIN Configuration..BargainProduct AS BP WITH ( NOLOCK ) ON BO.ActivityProductId = BP.PKID AND BP.IsDelete=0
...
select BU.UserId,
       ...
       CONVERT(DECIMAL(18, 2),  IIF(BO.Average > 0, BU.Reduce /BO.Average, IIF(BU.Reduce>0,BU.Reduce /BU.Reduce,0))) as Rate
from Activity..BargainOwnerAction as BO with (nolock)
...
where BO.OwnerId = @userId
      ...
      and BU.PKID is not null;";
       ...
       BP.HelpCutPriceTimes,
       BP.CutPricePersonLimit,
       BP.BeginDateTime,
       BP.BigCutBeforeCount,
       BP.BigCutPriceRate
from Activity..BargainOwnerAction as BO with (nolock)
    join Configuration..BargainProduct as BP with (nolock)
        on BO.ActivityProductId = BP.PKID AND BP.IsDelete=0
...
where BS.UserId = @userId
      and BO.idKey = @idkey;";
        #endregion
        #region CheckBargainAllShare
        internal const string sql4checkBargainAllShare = @"
select BO.idKey,
	   BS.UserId,
	   BO.OwnerId,
       BS.Reduce
from Activity..BargainShareAction as BS with (nolock)
    left join Activity..BargainOwnerAction as BO with (nolock)
        on BO.PKID = BS.ParentId
where BS.UserId = @userId 
      and BO.pid= @pid and (BO.CreateDateTime>=@beginDateTime and BO.CreateDateTime<=@endDateTime)";
        #endregion
...
from Activity..BargainOwnerAction as BO with (nolock)
    left join Configuration..BargainProduct as BP with (nolock)
        on BP.IsDelete=0 AND BO.ActivityProductId = BP.PKID
where BP.PKID = @apId
...
from Activity..BargainOwnerAction as T
    left join Configuration..BargainProduct as BP with (nolock)
        on T.ActivityProductId = BP.PKID AND BP.IsDelete=0 
...
order by LastUpdateDateTime desc;";
        ...
        BO.FinalPrice
FROM    Configuration..BargainProduct AS BP WITH ( NOLOCK )
        JOIN Activity..BargainOwnerAction AS BO WITH ( NOLOCK ) ON BP.PKID = BO.ActivityProductId AND BP.IsDelete=0
...
FROM	Configuration..BargainProduct AS BP WITH ( NOLOCK )
		LEFT JOIN Activity..BargainOwnerAction AS BOA WITH ( NOLOCK ) ON BOA.ActivityProductId = BP.PKID AND BP.IsDelete=0 
      ...
      and T.CreateDateTime >= @todayDate;";
        ...
        internal static string sql4SelectBargainProductInfo = @"
                    SELECT TOP 1
                                ...
                                ProductType,
                                ShowBeginTime
                            FROM Configuration..BargainProduct
                            WHERE IsDelete = 0
                                  AND PKID = @PKID; ";
        ...
        internal static string sql4SelectUserBargainCountAtTimeRange = @"
                                    SELECT COUNT(1)
                                    FROM Activity..BargainShareAction s WITH (NOLOCK)
                                        LEFT JOIN [Activity].[dbo].[BargainOwnerAction] o WITH (NOLOCK)
                                            ON s.ParentId = o.PKID
                                    WHERE s.UserId = @UserId
                                          AND o.OwnerId <> @UserId
                                          AND s.CreateDateTime >= @beginTime
                                          AND s.CreateDateTime <= @endTime;";
        #endregion
    }
}
