...
namespace Tuhu.Provisioning.Controllers
{
    public class ThirdShopController : Controller
    {
        ...
        #endregion
        #region 最低限价
        public JsonResult SetLowestLimitPrice(string PID, decimal? OldPrice, decimal LowestLimitPrice)
        {
            ...
            var result = ThirdShopManager.SetLowestLimitPrice(PID, OldPrice, LowestLimitPrice, ThreadIdentity.Operator.Name);
            return Json(new
            {
                Code = result,
        ...
        public JsonResult GetLowestLimitLog(string PID)
        {
            if (string.IsNullOrWhiteSpace(PID))
            {
                return Json(new { Code = 0, Info = "参数错误" });
            }
            var result = ThirdShopManager.GetLowestLimitLog(PID);
            return Json(new
            {
                Code = 1,
                Data = result
        ...
        public FileResult ExportExcel(ProductListRequest request)
        {
            ...
            row.CreateCell(4).SetCellValue("最低限价");
            row.CreateCell(5).SetCellValue("最小毛利");
            ...
            row.CreateCell(20).SetCellValue("京东自营");
            row.CreateCell(21).SetCellValue("京东自营最低价");
            ...
            foreach (var item in result)
            {
                ...
                rowtemp.CreateCell(4).SetCellValue(item.LowestLimit == null ? "" : item.LowestLimit.Value.ToString("0.00"));
                rowtemp.CreateCell(5).SetCellValue(item.Min_maoliValue == null ? "" : item.Min_maoliValue.Value.ToString("0.00"));
