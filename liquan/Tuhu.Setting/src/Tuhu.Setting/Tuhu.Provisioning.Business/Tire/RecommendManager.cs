...
using Tuhu.Service.Product.ModelDefinition.Request;
using Tuhu.Service.TireProductCache;
...
namespace Tuhu.Provisioning.Business.Tire
{
    public class RecommendManager
    {
        ...
        public static IEnumerable<VehicleTireModel> SelectListNew(string Departments, string VehicleIDS, string PriceRanges,
            string VehicleBodyTypes, string Specifications, string Brands, int IsShow, int IsRof, string PID, string Province, string City,
            decimal? StartPrice, decimal? EndPrice)
        {
            var source = DALRecommendTire.SelectListNew(Departments, VehicleIDS, PriceRanges, VehicleBodyTypes,
                Specifications, Brands, IsRof, PID, Province, City, StartPrice, EndPrice);
            if (!source.Any())
            ...
            var list = source.GroupBy(V => V.VehicleId + V.TireSize).Select(C => new VehicleTireModel()
            {
                ...
                RecommendTires = C.Select(D => new VehicleTireRecommend()
                {
                    ...
                    Postion = D.Postion,
                    Reason = D.Reason,
                    RecommendedPicture = D.RecommendedPicture,
                    StartTimeStr = D.StartTime.HasValue ? D.StartTime.Value.ToString("yyyy-MM-dd HH:mm:ss") : "",
                    EndTimeStr = D.EndTime.HasValue ? D.EndTime.Value.ToString("yyyy-MM-dd HH:mm:ss") : ""
                }),
        ...
        /// <param name="list"></param>
        /// <returns></returns>
        public static async Task<Boolean> RefreshRecommendTire(List<VehicleTireRecommend> list)
        {
            if (list!=null && list.Any())
            {
                list = list.GroupBy(t => t.Vehicleid + t.TireSize).Select(C => new VehicleTireRecommend()
                {
                    TireSize = C.FirstOrDefault()?.TireSize,
                    Vehicleid = C.FirstOrDefault()?.Vehicleid,
                }).ToList() ;
            ...
            try
            {
                var resultPid = false;
                //开始刷新人工推荐缓存
                using (var client = new Tuhu.Service.SearchProduct.CacheClient())
                {
                    var requests = list.ToList().Select(t => new SearchProductRequest()
                    {
                        VehicleId = t.Vehicleid,
                        TireSizeModel = Service.Product.ModelDefinition.Models.TireSizeModel.Parse(t.TireSize),
                    }).ToList();
                    //查询参数
                    var paras = requests.Select(t=> new  SearchProductRequest()
                    {
                        VehicleId = t.VehicleId,
                        Parameters = new Dictionary<string, IEnumerable<string>>(){

                            {"CP_Tire_Width",new List<string>(){ t.TireSizeModel?.Width } },
                            {"CP_Tire_AspectRatio",new List<string>(){ t.TireSizeModel?.AspectRatio} },
                            {"CP_Tire_Rim",new List<string>(){ t.TireSizeModel?.Rim } },
                        }
                    }).ToList();
                    var result = await client.RefreshManualTireSortAsync(paras);
                    resultPid = result.Success;
                }
                var resultPic = true;
                //更新推荐图片缓存
                using (var client = new TireProductCacheClient())
                {
                    foreach (var item in list)
                    {
                       var resultPicture= await client.RefreshRecommendByVehicleAsync(item.Vehicleid, item.TireSize);
        ...
        public static int SaveTireEnforceRecommend(List<QZTJModel> list)
        {
            ...
            foreach (var recommend in list)
            {
                ...
                recommend.Products?.ToList().ForEach(item => changeList.Add(new TireEnforceRecommend
                {
                    PId = item.PID,
                    RecommendPID = item.RecommendPID,
                    Reason = item.Reason,
