...
{
    /// <summary>618福利保养半价券 huhengxing 2016-6-13 已过期</summary>
    [DisallowConcurrentExecution]
    public class BaoYangHalfQuanJob : IJob
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(BaoYangHalfQuanJob));
        ...
        public void Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                {
                    Logger.Info("2016-6-14开始发放618福利日保养半价券");
                    return;
                }
                if (DateTime.Now > new DateTime(2016, 8, 3))
                {
                    Logger.Info("2016-8-3后结束发放618福利日保养半价券");
                    return;
                }
                Logger.Info("启动任务");
                using (var cmd = new SqlCommand(@"[Gungnir].[dbo].[ProMotionCode_BaoYangHalfQuan]"))
                {
                    ...
                    foreach (var i in dic)
                    {
                        if (!string.IsNullOrWhiteSpace(i.SumMoney))
                        {
                            ...
                            if (result > 0)
                            {
                                BLL.Business.SendMarketingSms(i.UserTel, "途虎618福利日保养半价券", m, "到店保养券");
                            }
                        }
                    }
                }
        ...
        public static int CreateOrYzPromotion(string userId, int money, string orderID)
        {
            try
            {
                ...
                promotionCodeModel.Description = "618保养半价返券";
                ...
                promotionCodeModel.CodeChannel = "618福利日保养半价券活动";
