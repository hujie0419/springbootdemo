...
WHERE	IsDelete = 0
		...
		AND ISNULL(@shopname, ShopName) = ShopName;";
            using (var cmd = new SqlCommand(SqlStr))
            {
                ...
                return DbHelper.ExecuteDataTable(cmd).ConvertTo<TireCouponModel>();
            }
        }
...
WHERE	T.IsShow = 1
        ...
        public static int SetLowestLimitPrice(string PID, decimal LowestLimitPrice)
        {
...
ON T.PID = S.PID
WHEN MATCHED THEN
	UPDATE SET	T.LowestPrice = @lowestprice
WHEN NOT MATCHED THEN
	INSERT	( PID, LowestPrice )
...
WHERE	PID = @pid;";
        ...
        public static int AddLowestLimitPriceLog(string PID, decimal? oldPrice, decimal newPrice, string Operator)
        {
