...
namespace Tuhu.Provisioning.DataAccess.DAO.Repository
{
    ...
    {
        private SettingDbContext dbcontext = null;

        public RepositoryBase(bool isReadonly)
        {
            dbcontext = new SettingDbContext(isReadonly);
        }

        public RepositoryBase(string configConnectionName)
        {
            dbcontext = new SettingDbContext("name=" + configConnectionName);
        }
        ...
        public DataTable Query(string sql)
        {
            ...
            conn.ConnectionString = this.dbcontext.Database.Connection.ConnectionString;
        ...
        public int Insert<TEntity>(TEntity entity) where TEntity : class
        {
            ...
            if (props != null && props.Any())
            {
                foreach (var prop in props)
                {
                    ...
                    if (value != null && String.Equals("&nbsp;", value?.ToString()))
                    {
        ...
        public List<TEntity> FindList<TEntity>(Pagination pagination) where TEntity : class, new()
        {
            ...
            string[] _order = pagination?.sidx?.TrimEnd(',')?.Split(',');
