   ...
   public class DalBigBrand
    {
        ...
        {
            string sql = @"SELECT * FROM Configuration.dbo.BigBrandRewardList WITH (NOLOCK) WHERE HashKeyValue=@HashKeyValue AND Is_Deleted=0";
        ...
        {
            string sql = @"SELECT * FROM Configuration.dbo.BigBrandRewardPool WITH (NOLOCK) WHERE FKPKID=@FKPKID  AND Status=1";
            using (var db = DbHelper.CreateDbHelper(true))
            {
        ...
        {
            string sql = @"SELECT * FROM Configuration.dbo.BigBrandPageStyle WITH (NOLOCK) WHERE FKPKID=@FKPKID AND Is_Deleted=0";
            using (var db = DbHelper.CreateDbHelper(true))
            {
        ...
        {
            string sql = @"SELECT * FROM Configuration.dbo.BigBrandWheel WITH (NOLOCK) WHERE FKBigBrand=@FKBigBrand AND Is_Deleted=0";
            using (var db = DbHelper.CreateDbHelper(true))
            {
...
VALUES  ( GETDATE() , -- CreateDateTime - datetime
        ...
        public  static bool IsExistShareLog(Guid userId, string hashKey,string channel)
        {
            string sql = @"SELECT COUNT(*) FROM SystemLog.dbo.BigBrandRewardLog WITH (NOLOCK) WHERE ChanceType=2 AND UserId=@UserId  
                AND Channel=@Channel AND FKBigBrandPkid IN (SELECT  TOP 1 PKID  FROM Configuration.dbo.BigBrandRewardList WITH (NOLOCK) WHERE HashKeyValue=@HashKeyValue  AND Is_Deleted=0)";
            using (var db = DbHelper.CreateDbHelper(true))
            {
                SqlCommand cmd = new SqlCommand(sql);
 ...
 ON BBRL.FKPKID =BBRP.PKID WHERE BBRP.Status=1";
            using (var db = DbHelper.CreateDbHelper(true))
            {
                SqlCommand cmd = new SqlCommand(sql);
                cmd.Parameters.AddWithValue("@FKBigBrandPkid", fkBigBrandPkid);
        ...
        FROM    Configuration.dbo.BigBrandRewardPool RP WITH ( NOLOCK )
                INNER JOIN ( SELECT TOP 1
                           ...
                           ) M ON RP.PKID = M.FKPKID WHERE RP.Status=1; ";
            using (var db = DbHelper.CreateDbHelper(true))
            {
                SqlCommand cmd = new SqlCommand(sql);
                cmd.Parameters.AddWithValue("@UserId", userId);
...
VALUES  ( @CreateDateTime ,
        ...
        {
            string sql = @"SELECT * FROM Configuration.dbo.BigBrandAnsQues WITH (NOLOCK) WHERE BigBrandPKID=@BigBrandPKID AND Is_Deleted=0";
            using (var db = DbHelper.CreateDbHelper(true))
            {
        ...
        public static async Task<BigBrandPageConfigModel> GetBigBrandPageConfigModel(int FKPKID, int ActivityType)
        {
            string sql = @"SELECT [PKID]
                      ...
                      WHERE FKPKID=@FKPKID AND ActivityType=@ActivityType AND Is_Deleted=0";
            using (var db = DbHelper.CreateDbHelper(true))
            {
                SqlCommand cmd = new SqlCommand(sql);
                cmd.Parameters.AddWithValue("FKPKID", FKPKID);
        ...
        /// <param name="fkPKID"></param>
        /// <returns></returns>
        public static async Task<BigBrandPageConfigBrand> GetBigBrandPageConfigBrand(int fkPKID)
        {
            string sql = @"
            SELECT BigBrandBackImgUrl,
                   BigBrandCenterBtnImgUrl,
                   BroadCastOutImgUrl,
                   BroadCastInnerImgUrl
            FROM [Configuration].[dbo].[BigBrandPageConfigBrand] WITH (NOLOCK)
            WHERE FPKID = @FKPKID AND IsDelete = 0;";
            using (var db = DbHelper.CreateDbHelper(true))
            {
                using (SqlCommand cmd = new SqlCommand(sql))
                {
                    cmd.Parameters.AddWithValue("FKPKID", fkPKID);
                    return await db.ExecuteFetchAsync<BigBrandPageConfigBrand>(cmd);
                }
            }
        }
    }
