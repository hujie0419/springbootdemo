...
VALUES  ( @FKHomePage , -- FKHomePage - int
         ...
         @TagContent , -- TagContent - nvarchar(500)
          ...
          @Margin,@UriCount,@IsNewUser
            ...
            int n = transaction == null ? connection.Execute(sqlString, model) : connection.Execute(sqlString, model, transaction);
            if (n > 0)
            {
                return true;
            }
 ...
 VALUES  ( @FKHomePage , -- FKHomePage - int
        ...
        public bool UpdateHomePageModule(SqlConnection connection, SE_HomePageModuleConfig model)
        {
            ...
            sqlString.Append(" Margin=@Margin, Pattern=@Pattern, ModuleName =@ModuleName, IsEnabled=@IsEnabled,PriorityLevel=@PriorityLevel, UpdateDateTime=@UpdateDateTime, ");
        ...
        public bool UpdateHomePageModulePriorityLevel(SqlConnection connection, IEnumerable<SE_HomePageModuleConfig> modules)
        {
            ...
            try
            {
                foreach (var item in modules)
                    connection.Execute("UPDATE Configuration.dbo.SE_HomePageModuleConfig SET PriorityLevel=@PriorityLevel  WHERE ID=@ID ", item, trans);
                trans.Commit();
                return true;
            }
            catch (Exception em)
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public bool AddHomePageModuleHelper(SqlConnection connection, SE_HomePageModuleHelperConfig model, IEnumerable<SE_ModuleHelperCityConfig> citys)
        {
...
VALUES  ( @FKHomePageModuleHelperID , -- FKHomePageModuleHelperID - int
          ...
          GETDATE()  -- UpdateDateTime - datetime
                ...
                if (model.ModuleType != 30)//如果是拼图模块就不默认增加父模块内容了，单独的替换逻辑处理
                {
                    ...
                    if (parentContent != null)
                    {
                        foreach (var content in parentContent)
                        {
                            ...
                            AddHomePageContent(connection, content);
                        }
                    }
                }
...
VALUES  ( @FKHomePageModuleHelperID , -- FKHomePageModuleHelperID - int
        ...
        /// <param name="citys"></param>
        /// <returns></returns>
        public bool UpdateHomePageModuleHelper(SqlConnection connection, SE_HomePageModuleHelperConfig model, IEnumerable<SE_ModuleHelperCityConfig> citys)
        {
            SqlTransaction trans = connection.BeginTransaction();
            try
            {
                ...
                sqlString.Append(" where ID=@ID ");
                int i = connection.Execute(sqlString.ToString(), model, trans);
                if (i <= 0)
                {
...
VALUES  ( @FKHomePageModuleHelperID , -- FKHomePageModuleHelperID - int
        ...
        {
            return connection.Execute("UPDATE Configuration.dbo.SE_HomePageModuleHelperConfig SET BgImageUrl=@BgImageUrl  where ID=@ID ", model) > 0;
        }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public bool DeleteHomePageModule(SqlConnection connection, int id)
        {
...
VALUES  ( @Name , -- Name - nvarchar(50)
        ...
        /// <param name="connection"></param>
        /// <param name="model"></param>
        public int AddHomePageContent(SqlConnection connection, SE_HomePageModuleContentConfig model)
        {
            string sql = @"
                     DECLARE @InseredTable TABLE (
                    ID INT 
            ...
            int n = connection.Execute(sql, dp);
            if (n > 0)
        ...
        public int AddHomePageContent(SqlConnection connection, SqlTransaction trans, SE_HomePageModuleContentConfig model)
        {
            string sql = @"
                     DECLARE @InseredTable TABLE (
                    ID INT 
            ...
            int n = connection.Execute(sql, dp, trans);
            if (n > 0)
                return dp.Get<int>("@PKID");
            else
                return -1;
        ...
        /// <param name="moduleHelper"></param>
        /// <returns></returns>
        public bool DeleteHomePageContent(SqlConnection connection, int id)
        {
