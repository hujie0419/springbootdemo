...
namespace Tuhu.Provisioning.Controllers
{
    public class WechatHomeController : Controller
    {
        ...
        public ActionResult GetWechatHomeListFilter(int homepageconfigId,int sourceid, int ispintuan)
        {
            WechatHomeManager manager = new WechatHomeManager();
            var rows = manager.GetWechatHomeList(homepageconfigId)?.ToList();
            if (ispintuan>0)
            {
                rows = rows?.Where(p => string.Equals(p.TypeName, "拼团模块", StringComparison.OrdinalIgnoreCase)).ToList();
            }
            else
            {
                rows = rows?.Where(p => !string.Equals(p.TypeName, "拼团模块", StringComparison.OrdinalIgnoreCase)).ToList();
            }
            rows?.RemoveAll(p => p.ID == sourceid);
            return Content(JsonConvert.SerializeObject(rows?.OrderBy(_ => _.OrderBy)));
        }
            ...
            using (var client = new Tuhu.Service.PinTuan.PinTuanClient())
            {
                ...
                jr.Data = new
                {
                    ...
                    data = result
                        ...
                        //.Where(p => p.IsStockout == false)
                        // 正在上架的
                        .Where(p => p.StartTime <= datetime && p.EndTime >= datetime)
                        // 排除团长免单的
                        .Where(p=>p.GroupType != 3)
                        .ToList()
                };
            }
            return jr;
        ...
        public ActionResult ContentCopy(int id)
        {
            ViewBag.id = id;
            WechatHomeManager manager = new WechatHomeManager();
            var entity=manager.GetWechatHomeEntity(id);
            ViewBag.isPintuan = string.Equals(entity?.TypeName, "拼团模块", StringComparison.OrdinalIgnoreCase) ? 1:0;
            return View();
        }

        public ActionResult SaveWechatHomeContentCopy(int sourceid,int destid,int ispintuan)
        {
            try
            {
                WechatHomeManager manager = new WechatHomeManager();
                ...
                if (ispintuan>0)
                {
                    manager.DeleteProductContentByFKID(destid);
                    var contetnlist = manager.GetProductContentList(sourceid)?.ToList(); ;
                    for (int i = 0; i < contetnlist.Count; i++)
                    {
                        var item = contetnlist[i];
                        item.ID = 0;
                        item.FKID = destid;                        
                        manager.AddProductContent(item);
                    }
                }
                else
                {
                    manager.DeleteContentByFKID(destid);
                    var contetnlist = manager.GetContentList(sourceid)?.ToList(); ;
                    for (int i = 0; i < contetnlist.Count; i++)
                    {
                        var item = contetnlist[i];
                        item.ID = 0;
        ...
        public ActionResult GetKeywordReplyEntity(int pkid)
        {
            ...
            foreach (var item in menuMaterialMappingList)
            {
                ...
                else//其他素材
                {
                    ...
                    materialModel.MediaID = item.MediaID;
                    materialModel.MediaType = item.MediaType;
                    materialModel.URL = item.ImageUrl;
                    materialModel.AppID = item.AppID;
        ...
        public ActionResult GetWechatSubscribeGrid(string originalId)
        {
            ...
            return Content(JsonConvert.SerializeObject(list?.OrderBy(_ => _.OrderBy)?.ThenBy(p=>p.PKID)));
        }
        ...
        public ActionResult SaveWechatSubsc(WXEventMsgConfigEntity entity)
        {
            ...
            entity.PicUrl = entity.PicUrl ?? "";
            entity.Urls = entity.Urls ?? "";
            entity.MediaId = entity.MediaId ?? "";
        ...
        public ActionResult GetWeChatOfficialAccounts(int type = 0)
        {
            using (var client = new WeiXinPushClient())
            {
                ...
                return Content(JsonConvert.SerializeObject(accounts.Where(t => (type == 0 ? t.Type == "WX_Open" : t.Type == "WX_APP")  && !string.IsNullOrWhiteSpace(t.channel)).Select(t => new { t.OriginalId, t.name ,t.channel,t.appId})));
            }
        }
        ...
        public ActionResult WechatSocialCardConfigList(int pageIndex, int pageSize)
        {
            ...
            {
                item.CreateMidName = CreateMidList.Where(p => p.OriginalId == item.CreateMid).FirstOrDefault()?.DisplayName;
                item.UseMidList = item.UseMids.Split(',').ToList();
                item.UseMidNameList = new List<string>();
                for (int i = 0; i < item.UseMidList.Count; i++)
                {
                    item.UseMidNameList.Add(CreateMidList.Where(p => p.OriginalId == item.UseMidList[i]).FirstOrDefault()?.DisplayName);
                }
            }
        ...
        public JsonResult SaveWechatSocialCardConfig(WechatSocialCardConfigModel model)
        {
            ...
            model.UseMidList = model.UseMidList.Distinct().ToList();
            ...
            #region 根据model生成微信请求参数 => 调用微信接口创建微信社交立减金代金券
            try
            {
                        ...
                        cash = new Cash
                        {
                            base_info = new Base_Info
                            {
                                ...
                                pay_info = new Pay_Info
                                {
                                    ...
                                    {
                                        create_mid = model.CreateMid,
                                        use_mid_list = model.UseMidList.ToArray(),//new[] { MerchantNo },
                                        is_swipe_card = true
                                    }
                                },
            ...
            #region 保存配置到数据库并记录日志
            model.MerchantNo = MerchantNo;
            model.UseMids = string.Join(",", model.UseMidList);
        ...
        public ActionResult WechatSocialActivityConfigList(int pageIndex, int pageSize)
        {
            ...
            foreach (var item in list)
            {
                item.MerchantNoName = CreateMidList.Where(p => p.OriginalId == item.MerchantNo).FirstOrDefault()?.DisplayName;
            }
            return Content(JsonConvert.SerializeObject(list, new JsonSerializerSettings
            {
                DateFormatString = "yyyy-MM-dd HH:mm:ss"
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult AddCreateMidList() => Json(CreateMidList, JsonRequestBehavior.AllowGet);
        ...
        /// 创建代金券的商户号
        /// </summary>
        private static readonly IReadOnlyCollection<WxAppEntity> CreateMidList =
            new[]
            {
                new WxAppEntity("1426920202 主小程序、洗车小程序、拼团小程序", "1426920202",""),
                new WxAppEntity("1269954001 APP、门店扫码", "1269954001",""),
                new WxAppEntity("1228441902 H5、PC","1228441902","")
            };
        ...
        private static string GetSocialWechatAccessToken()
        {
            using (var client = new WeiXinServiceClient())
            {
                var response = client.GetAccessTokenByPlatform(0);
        ...
        public ActionResult SubmitWX_QRCodeConfig(WX_QRCodeConfigModel model)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    //删除菜单和素材关联数据
                    mappingManager.DeleteWXMenuMaterialMapping(entity.PKID, objectType);
                    //删除文本素材数据
                    materialTextManager.DeleteWXMaterialText(entity.PKID, objectType);
                }
                ...
                if (model.MaterialList != null)
                {
                    foreach (var item in model.MaterialList)
                    {
                        ...
                        mappingModel.ObjectID = entity.PKID;
                        mappingModel.ObjectType = objectType;
                        mappingModel.OriginalID = entity.OriginalID;
                        mappingModel.MediaType = item.MediaType;
                        mappingModel.MediaID = item.MediaID;
                        mappingModel.AppID = item.AppID;
                ...
                if (model.MaterialTextList != null)
                {
                    foreach (var item in model.MaterialTextList)
                    {
                        textModel = new WXMaterialTextModel();
                        textModel.ObjectID = entity.PKID;
                        textModel.ObjectType = objectType;
        ...
        public ActionResult WX_MaterialList()
        {
            return View();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult RedEnvelopeActivity()
        {
            WechatHomeManager manager = new WechatHomeManager();
            var OARedEnvelopeSettingModel = manager.GetOARedEnvelopeSettingModel();
            var OARedEnvelopeActivityTime = manager.GetOARedEnvelopeActivityTime();

            var OARedEnvelopeModel = new WXOARedEnvelopeSettingModel();
            if (OARedEnvelopeSettingModel != null) OARedEnvelopeModel = OARedEnvelopeSettingModel;
            if (OARedEnvelopeActivityTime != null)
            {
                OARedEnvelopeModel.StartTime = OARedEnvelopeActivityTime.StartTime;
                OARedEnvelopeModel.EndTime = OARedEnvelopeActivityTime.EndTime;
            }
            return View(OARedEnvelopeModel);
        }

        /// <summary>
        /// 修改领红包活动配置
        /// </summary>
        /// <param name="redEnvelopeSettingmodel"></param>
        /// <returns></returns>
        [HttpPost]
        public ActionResult SaveOARedEnvelopeSetting(WXOARedEnvelopeSettingModel redEnvelopeSettingmodel)
        {
            using (var client=new Tuhu.Service.Activity.OARedEnvelopeClient())
            {
                if (redEnvelopeSettingmodel.AvgMoney < 1)
                {
                    return Content(JsonConvert.SerializeObject(new
                    {
                        Code = 1,
                        Msg = "每人平均领取红包金额必须大于等于1"
                    }));
                }
                //var redEnvelopeSettingmodel = JsonConvert.DeserializeObject<WXOARedEnvelopeSettingModel>(fdata);
                var redEnveloperequest = ObjectMapper.ConvertTo<WXOARedEnvelopeSettingModel, OARedEnvelopeSettingUpdateRequest>(redEnvelopeSettingmodel);
                WechatHomeManager manager = new WechatHomeManager();
                var OARedEnvelopeSettingModel = manager.GetOARedEnvelopeSettingModel();
                if (OARedEnvelopeSettingModel == null)
                {
                    redEnveloperequest.CreateBy = User.Identity.Name;
                }
                redEnveloperequest.LastUpdateBy = User.Identity.Name;
                var result= client.OARedEnvelopeSettingUpdate(redEnveloperequest);

                if (result.Result)
                {
                    client.OARedEnvelopeRefreshCache();
                    return Content(JsonConvert.SerializeObject(new
                    {
                        Code =1,
                        Msg = "操作成功"
        ...
        public ActionResult RedEnvelopeStatistic()
        {
            return View();
        }
        ...
        /// <returns></returns>
        public ActionResult GetOARedEnvelopeStatisticsList(Pagination pagination, string startTime = "", string endTime = "")
        {
            WechatHomeManager manager = new WechatHomeManager();
            string dtStartTime;
            string dtEndTime;
            ...
            if (!string.IsNullOrEmpty(startTime))
                dtStartTime = startTime+ " 00:00";
            else
                dtStartTime = DateTime.Now.AddDays(-6).ToString().Split(' ')[0]+" 00:00";
            if (!string.IsNullOrEmpty(endTime))
                dtEndTime = endTime+ " 23:59";
            else
                dtEndTime = DateTime.Now.ToString().Split(' ')[0]+" 23:59";
            var OARedEnvelopeStatisticsList = manager.GetOARedEnvelopeStatisticsList(pagination, dtStartTime, dtEndTime);
            var newOARedEnvelopeStatisticsList = (from RedEnvelopeStatistic in OARedEnvelopeStatisticsList
                                                      select new
                                                      {
                                                          StatisticsDateString = RedEnvelopeStatistic.StatisticsDate.ToString("yyyy-MM-dd"),
                                                          DayMaxMoney = RedEnvelopeStatistic.DayMaxMoney,//每日红包上限
                                                          RedEnvelopeCount = RedEnvelopeStatistic.RedEnvelopeCount,//已发放红包数
                                                          RedEnvelopeSumMoney = RedEnvelopeStatistic.RedEnvelopeSumMoney,//已发放红包金额
                                                          RedEnvelopeAvg = RedEnvelopeStatistic.RedEnvelopeAvg//红包平均金额
                                                      }).ToList();
            var allRedEnvelopeStatisticsList=manager.GetOARedEnvelopeStatisticsListByTime(dtStartTime, dtEndTime);
            if (allRedEnvelopeStatisticsList.Count != 0&&pagination.page==1)
            {
                newOARedEnvelopeStatisticsList.Insert(0, new
                {
                    StatisticsDateString = "总计",
                    DayMaxMoney = allRedEnvelopeStatisticsList.Sum(x => x.DayMaxMoney),
                    RedEnvelopeCount = allRedEnvelopeStatisticsList.Sum(x => x.RedEnvelopeCount),
                    RedEnvelopeSumMoney = allRedEnvelopeStatisticsList.Sum(x => x.RedEnvelopeSumMoney),
                    RedEnvelopeAvg = Math.Round(allRedEnvelopeStatisticsList.Sum(x => x.RedEnvelopeAvg) / allRedEnvelopeStatisticsList.Count,2),
                });
            }
               
            return Content(JsonConvert.SerializeObject(new
            {
                rows = newOARedEnvelopeStatisticsList,
        ...
        public ActionResult ExportExcel(string startTime, string endTime)
        {
            ...
            var num = 0;
            row.CreateCell(num++).SetCellValue("日期");
            row.CreateCell(num++).SetCellValue("每日红包上限（元）");
            row.CreateCell(num++).SetCellValue("已发放红包数");
            row.CreateCell(num++).SetCellValue("已发放红包金额（元）");
            row.CreateCell(num++).SetCellValue("红包平均金额");
            ...
            sheet.SetColumnWidth(num++, 16 * 256);
            sheet.SetColumnWidth(num++, 16 * 256);
            var exm = new WechatHomeManager();
            var list = exm.GetOARedEnvelopeStatisticsListByTime(dtStartTime, dtEndTime);
            if (list.Count != 0)
            {
                row = sheet.CreateRow(index++);
                num = 0;
                row.CreateCell(num++).SetCellValue("总计");
                row.CreateCell(num++).SetCellValue((double)list.Sum(x => x.DayMaxMoney));
                row.CreateCell(num++).SetCellValue(list.Sum(x => x.RedEnvelopeCount));
                row.CreateCell(num++).SetCellValue((double)list.Sum(x => x.RedEnvelopeSumMoney));
                row.CreateCell(num++).SetCellValue((double)Math.Round((list.Sum(x => x.RedEnvelopeAvg) / list.Count),2));
            }
            ...
            foreach (var item in list)
            {
                row = sheet.CreateRow(index++);
                num = 0;
                row.CreateCell(num++).SetCellValue(item.CreateDatetime.ToString("yyyy-MM-dd"));
                row.CreateCell(num++).SetCellValue((double)item.DayMaxMoney);
                row.CreateCell(num++).SetCellValue(item.RedEnvelopeCount);
                row.CreateCell(num++).SetCellValue((double)item.RedEnvelopeSumMoney);
                row.CreateCell(num++).SetCellValue((double)item.RedEnvelopeAvg);
            }
            var ms = new MemoryStream();
            workbook.Write(ms);
            return File(ms.ToArray(), "application/x-xls", $"每日发放红包概况-{DateTime.Now.ToString("yyyy年MM月dd日HH时mm分ss秒")}.xlsx");
        }
        #endregion
