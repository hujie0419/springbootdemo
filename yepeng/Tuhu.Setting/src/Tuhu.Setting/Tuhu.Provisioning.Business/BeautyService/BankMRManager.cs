...
namespace Tuhu.Provisioning.Business.BeautyService
{
    public class BankMRManager
    {
        ...
        public bool UpdateBankMRActivityRoundLimitCount(Guid activityId, int limitCount)
        {
            var result = false;

            try
            {
                TuhuGrouponDbScopeManager.Execute(conn => result = BankMRActivityDal.UpdateBankMRActivityRoundLimitCount(conn, activityId, limitCount));
            }
            catch (Exception ex)
            {
                Logger.Error(ex.Message, ex);
        ...
        /// <param name="config"></param>
        /// <returns></returns>
        public bool InsertMrCooperateUserConfig(MrCooperateUserConfig config)
        {
            bool result = false;
        ...
        /// <param name="config"></param>
        /// <returns></returns>
        public bool UpdateMrCooperateUserConfig(MrCooperateUserConfig config)
        {
            bool result = false;
        ...
        public Tuple<bool, string> DeleteImportBankMRActivityByBatchCode(string batchCode,string operateUser)
        {
            try
            {
                var result = TuhuGroupon_GungnirManagerReadOnly.Execute(conn => BankMRActivityDal.DeleteImportBankMRActivityByBatchCode(conn, batchCode));
                if (result)
                {
                    var log = new BeautyOprLog
                    {
