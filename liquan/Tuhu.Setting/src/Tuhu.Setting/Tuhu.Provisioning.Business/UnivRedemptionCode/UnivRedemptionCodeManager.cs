...
namespace Tuhu.Provisioning.Business.UnivRedemptionCode
{
   ...
   new ConnectionManager(ConfigurationManager.ConnectionStrings["Gungnir"].ConnectionString);
            ...
            try
            {
                ...
                else if (!string.IsNullOrWhiteSpace(mobile))
                {
                    ...
                    if (user != null && user.UserId != Guid.Empty)
                    {
                        ...
                        redemptionCodes.Item1?.ToList().ForEach(i =>
                        {
        ...
        /// <param name="pageSize"></param>
        /// <returns></returns>
        public static async Task<List<GeneralBeautyServerCodes>> GetRedeemMrServerCodes(string mobile, string serviceCode, int pageIndex, int pageSize)
        {
            var userId = Guid.Empty;
            if (!string.IsNullOrEmpty(mobile))
            {
                var users = UserAccountService.GetUserByMobile(mobile);
                userId = users?.UserId ?? Guid.Empty;
            }
            var result = await DALUnivRedemptionCode.GetRedeemMrServerCodes(mobile, serviceCode,userId, pageIndex, pageSize);//兑换码
            ...
            try
            {
                ...
                if (list != null && list.Any())
                {
                    list.ForEach(x =>
                    {
                        if (x.CompanyId > 0)
                        {
                            var companyInfo = UserAccountService.SelectCompanyInfoById(x.CompanyId);
                            x.CompanyName = companyInfo?.FirstOrDefault()?.Name;
                        }
                        if (x.CompanyUserId != Guid.Empty)
                        {
                            var userInfo = UserAccountService.SelectCompanyUserInfo(x.CompanyUserId);
                            x.CompanyUserName = userInfo?.UserName;
                            x.Mobile = userInfo?.UserMobile;
                        }
                    });
                }
            }
        ...
        public Tuple<bool,string> SaveThirdAppSetting(OpenAppModel model, string userName)
        {
            bool result = false;
            OpenAppModel oldModel = null;
            var logType = "";
            var msg = string.Empty;
            try
            {
                DbTuhuGunginrScopeManager.CreateTransaction(conn =>
                {
                    var openAppModels = DALUnivRedemptionCode.GetAllBigCustomerOpenAppIds(conn, "大客户");
                    if (model.Id > 0)
                    {
                        if (openAppModels?.Where(_ => _.Id != model.Id && _.CompanyUserId == model.CompanyUserId)?.Count() > 0)
                        {
                            msg = "同一个公司用户ID只能关联到一个APPID";
                        }
                        else
                        {
                            logType = "UpdateThirdAppSetting";
                            oldModel = openAppModels?.Where(_ => _.Id == model.Id)?.FirstOrDefault();
                            result = DALUnivRedemptionCode.UpdateThirdAppSetting(conn, model);
                        }
                    }
                    else
                    {
                        if (openAppModels?.Where(_ => _.CompanyUserId == model.CompanyUserId)?.Count() > 0)
                        {
                            msg = "同一个公司用户ID只能关联到一个APPID";
                        }
                        else
                        {
                            ...
                            result = DALUnivRedemptionCode.InsertThirdAppSetting(conn, model);
                        }
                    }
                });
            }
