...
namespace Tuhu.Provisioning.DataAccess.DAO
{
    public class DALWechatHome
    {
        ...
        public bool AddProductContent(SqlConnection conn, WechatHomeProductContent model)
        {
            ...
            if (model.IsTimeSet > 0)
            {
                sql = @"INSERT INTO Configuration.dbo.WechatHomeProductContent
                            ( FKID,
                              PID ,
                              GroupId,
                              ProductName,
        ...
        public bool UpdateProductContent(SqlConnection conn, WechatHomeProductContent model)
        {
            ...
            if (model.IsTimeSet > 0)
            {
                sql = @"    UPDATE  Configuration.dbo.WechatHomeProductContent
                                SET     PID=@PID,
                                        ImageUrl = @ImageUrl ,
                                        GroupId=@GroupId,
                                        ProductName=@ProductName,
                                        IsEnabled = @IsEnabled ,                                  
                                        UDateTime = GETDATE(),OrderBy=@OrderBy,BuriedPointParam=@BuriedPointParam,IsTimeSet=@IsTimeSet,StartTime=@StartTime,EndTime=@EndTime
                                WHERE   ID = @ID;";
            }
            else
            {
                sql = @"    UPDATE  Configuration.dbo.WechatHomeProductContent
                                SET     PID=@PID,
                                        ...
                                        ProductName=@ProductName,
                                        IsEnabled = @IsEnabled ,                                  
                                        UDateTime = GETDATE(),OrderBy=@OrderBy,BuriedPointParam=@BuriedPointParam,IsTimeSet=@IsTimeSet
                                WHERE   ID = @ID;";
            }
        ...
        public bool DeleteContentByFKID(SqlConnection conn, int fkid)
        {
            string sql = "DELETE FROM Configuration.dbo.WechatHomeContent WHERE FKID=@fkid ";
            DynamicParameters parameter = new DynamicParameters();
            parameter.Add("@fkid", fkid);
            return conn.Execute(sql, parameter) > 0;
        }

        public bool DeleteProductContentByFKID(SqlConnection conn, int fkid)
        {
            string sql = "DELETE FROM Configuration.dbo.WechatHomeProductContent WHERE FKID=@fkid ";
            DynamicParameters parameter = new DynamicParameters();
            parameter.Add("@fkid", fkid);
            return conn.Execute(sql, parameter) > 0;
        }
...
ORDER BY [PKID] DESC OFFSET @offset ROWS FETCH NEXT @size ROWS ONLY;",
        ...
        /// </summary>
        /// <returns></returns>
        public WXOARedEnvelopeSettingModel GetOARedEnvelopeSettingModel()
        {
            const string sql = @"
                                SELECT TOP 1
                                        [PKID] ,
            ...
            {
                return conn.Query<WXOARedEnvelopeSettingModel>(sql).ToList().FirstOrDefault();
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public WXOARedEnvelopeSettingModel GetOARedEnvelopeActivityTime()
        {
            const string sql = @"
                                SELECT TOP 1
                                        StartTime ,
                                        EndTime
                                FROM   Activity..tbl_Activity WITH ( NOLOCK )
        ...
        /// <returns></returns>
        public List<WXOARedEnvelopeStatisticsModel> GetOARedEnvelopeStatisticsList(Pagination pagination,  string startTime, string endTime)
        {
            #region SQL
            const string sql = @"
                                SELECT  [PKID] ,
            ...
            using (var conn = new SqlConnection(ConnectionHelper.GetDecryptConn("Gungnir_ReadOnly")))
            {
                ...
                if (pagination.records > 0)
                    return conn.Query<WXOARedEnvelopeStatisticsModel>(sql, dp).ToList();
                return new List<WXOARedEnvelopeStatisticsModel>();
            }
        }
        ...
        /// <param name="endTime"></param>
        /// <returns></returns>
        public List<WXOARedEnvelopeStatisticsModel> GetOARedEnvelopeStatisticsListByTime(string startTime, string endTime)
        {
            #region SQL
            const string sql = @"
                                SELECT  [PKID] ,
