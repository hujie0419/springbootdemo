...
namespace Tuhu.Service.Activity.DataAccess
{
    public class DalSalePromotionActivity
    {
     ...
     Activity.dbo.SalePromotionActivity(ActivityId,Name,Description,Banner,
        ...
        /// <summary>
        /// 修改活动的审核、下架状态字段 - 审核状态(0初始，1待审核，2已审核，3已拒绝)
        /// </summary>
        public static async Task<int> UpdateActivityAuditAndUnShelveStatusAsync(BaseDbHelper dbHelper, string activityId, int newAuditStatus, string operatorUser)
        {
            var result = 0;
            ...
            using (var cmd = new SqlCommand(sql))
            {
                cmd.Parameters.Add(new SqlParameter("@activityId", activityId));
                cmd.Parameters.Add(new SqlParameter("@newAuditStatus", newAuditStatus));
                cmd.Parameters.Add(new SqlParameter("@userName", operatorUser));
        ...
        /// <param name="isUpdateAll">true:设置活动下所有商品,false:设置指定的pids</param>
        /// <returns></returns>
        public static async Task<int> BatchUpdateProductDetailImageAsync(BaseDbHelper dbHelper,
            SetDiscountProductDetailImgRequest request, bool isUpdateAll = false)
        {
            int result = 0;
            ...
            {
                SqlProduct = @"UPDATE [Activity].[dbo].[SalePromotionActivityProduct] WITH (ROWLOCK)
                                SET   DetailImageUrl = @imgUrl,
                                      LastUpdateDateTime = GETDATE()
                               WHERE  ActivityId = @activityId
                                      AND Is_Deleted = 0;";
            }
            else if (request.Pid?.Count > 0)
            {
                SqlProduct = @" UPDATE p
                                  SET  p.DetailImageUrl = @imgUrl,
                                       p.LastUpdateDateTime = GETDATE()
                                 FROM  [Activity].[dbo].[SalePromotionActivityProduct] p WITH (ROWLOCK)
            ...
            using (var cmd = new SqlCommand(SqlProduct))
            {
                ...
                cmd.Parameters.Add(new SqlParameter("@imgUrl", request.DetailImgUrl));
                cmd.Parameters.Add(new SqlParameter("@activityId", request.ActivityId));

                if (!isUpdateAll && request.Pid?.Count > 0)
                {
                    cmd.Parameters.Add(new SqlParameter("@pidList", string.Join(",", request.Pid)));
                }
            ...
            using (var dbHelper = DbHelper.CreateDbHelper())
            {
                ...
                try
                {
                    ...
                    if (transResult)
                    {
                        ...
                        //读写库标识
                        await SetDBFlagCache(activityId);
                        DBReadOnlyFlagHelper.SetDBReadOnlyFlagCache(SalePromotionActivityConst.SalePromotionActivityAndProduct,
                            new List<string>() { pid });
                    }
                    else
                    {
                    ...
                    using (var cmd = new SqlCommand("", conn, tran))
                    {
                        try
                        {
                            ...
                            if (tansResult)
                            {
                                ...
                                DBReadOnlyFlagHelper.SetDBReadOnlyFlagCache(SalePromotionActivityConst.SalePromotionActivityAndProduct, delList);
                            }
                            else
                            {
                                tran.Rollback();
        ...
        public static async Task<IEnumerable<SalePromotionActivityProduct>> GetProductInfoListAsync(string activityId, List<string> pidList)
        {
            ...
            using (var cmd = new SqlCommand())
            {
                if (pidList?.Count > 0)
                {
                    sql = @" select p.Pid,p.ProductName,p.LimitQuantity,p.TotalStock,p.SoldQuantity
                ...
                {
                    sql = @"select Pid,ProductName,LimitQuantity,TotalStock,SoldQuantity
                             from  [Activity].[dbo].[SalePromotionActivityProduct] with(nolock)
                             where ActivityId=@activityId and Is_Deleted=0";
                }
                cmd.CommandText = sql;
        ...
        public static async Task<PagedModel<SalePromotionActivityProduct>> SelectProductListAsync(SelectActivityProduct condition, int pageIndex = 1, int pageSize = 20)
        {
            ...
            string sqlSelect = @"select distinct p.Pid,p.TotalStock,p.ProductName,p.CostPrice,p.ImageUrl,p.DetailImageUrl,
        ...
        public static async Task<List<SalePromotionActivityProductDB>> GetProductListByActivityIdAsync(string activityId)
        {
            ...
            string sql = @" select ActivityId,Pid,LimitQuantity,SalePrice,SoldQuantity
                             from  [Activity].[dbo].[SalePromotionActivityProduct] with(nolock)
                            where  Is_Deleted=0 and ActivityId=@activityId ;";
            using (var cmd = new SqlCommand(sql))
            {
        ...
        /// <param name="activityId"></param>
        /// <returns></returns>
        public static async Task<int> GetActivityAuditStatus(string activityId)
        {
            int auditStatus = 0;
            string sql = @"select AuditStatus
                            from  [Activity].[dbo].[SalePromotionActivity] with(nolock)
        ...
        /// <param name="activityId"></param>
        /// <returns></returns>
        public static async Task<bool> SetDBFlagCache(string activityId)
        {
