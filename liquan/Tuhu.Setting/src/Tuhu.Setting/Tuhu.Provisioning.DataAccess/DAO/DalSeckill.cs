...
namespace Tuhu.Provisioning.DataAccess.DAO
{
    public class DalSeckill
    {
        ...
        public static ActivityApprovalStatus SelectActivityStatusModel(Guid activityId)
        {
            using (var dbHelper = new SqlDbHelper(ConnectionHelper.GetDecryptConn("Gungnir_AlwaysOnRead")))
            {
                var sql = @"SELECT Pkid,
                            ...
                            CreateDateTime,
                            LastUpdateDateTime
                        FROM Configuration..ActivityApprovalStatus WITH (NOLOCK)
                ...
                ).ConvertTo<ActivityApprovalStatus>()?.FirstOrDefault();
            }
        }
        ...
        {
            string sql = @"	 INSERT INTO Activity..ActivityApprovalStatusLog
                              ( FlashPKID,
                                ActivityId,
                                Status,
                                CreateDateTime,
            ...
            using (var dbHelper = new SqlDbHelper(ConnectionHelper.GetDecryptConn("Gungnir")))
            {
                using (var cmd = new SqlCommand(sql))
                {
                    ...
                    cmd.Parameters.AddWithValue("@LastUpdateDateTime", model.LastUpdateDateTime);
...
WHERE	FS.ActivityID = @ActivityID ORDER BY FSP.PKID;", CommandType.Text, new SqlParameter("@ActivityID", aid));
	...
	when NOT MATCHED THEN 
        ...
        public static List<string> SelectActivityProductPids(Guid aid)
        {
            ...
            {
                using (var cmd = new SqlCommand(
                  @"SELECT Pid FROM activity..tbl_FlashSaleProducts AS tfsp WITH ( NOLOCK) WHERE tfsp.ActivityID=@ActivityId"))
                {
                    cmd.Parameters.AddWithValue("@ActivityId", aid);
                    cmd.CommandType = CommandType.Text;
                    var dt = dbHelper.ExecuteDataTable(cmd);
                    return dt.Rows.Cast<DataRow>().Select(row => row["Pid"].ToString()).ToList();
                }
            }
        }
