...
namespace Tuhu.Provisioning.Business.BaoYang
{
    public class BaoYangManager
    {
        ...
        /// <param name="partName"></param>
        /// <returns></returns>
        public List<ProductPriorityRegular> GetProductPriorityRegularByPartName(IEnumerable<string> partNames)
        {
            List<ProductPriorityRegular> result = null;
            try
            {
                result = GRAlwaysOnReadDbScopeManager.Execute(conn => DALBaoyang.GetProductPriorityRegularByPartName(conn, partNames)).ToList();
            }
            catch (Exception ex)
            {
                logger.Error(ex.Message, ex);
            }
            return result ?? new List<ProductPriorityRegular>();
        }
