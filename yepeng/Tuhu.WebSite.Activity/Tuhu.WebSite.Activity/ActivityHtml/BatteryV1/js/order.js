...
function CreateOrderInfo(serialNumbers, payAmount, subject, body, casgiersData, orderNumber) {
    ...
    if (userAgentInfo.indexOf("Android") >= 0) {
        ...
        if (isIosVersion) {
            iosend("customSegue#THOrderCashiersVC", '{"casgiersData":' + JSON.stringify(casgiersData) + ',"orderID":"' + orderNumber + '"}');
        }
        else {
            iosend("customSegue#TNOrderDetailVC", '{ "orderID":"' + orderNumber + '"}');
        }
    }
};
    ...
    $.ajax({
        ...
        success: function (str) {
            ...
            $(addressModelList).each(function (i) {
                AddressID = addressModelList[i].AddressID;
