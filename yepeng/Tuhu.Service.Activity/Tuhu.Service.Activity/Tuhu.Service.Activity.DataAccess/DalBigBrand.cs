...
VALUES  ( GETDATE() , -- CreateDateTime - datetime
        ...
        {
            string sql = @"SELECT BBRP.Name, SUBSTRING(BBRL.Phone,0,4)+'****'+SUBSTRING(BBRL.Phone,7,4) AS  Phone FROM 
 (SELECT TOP 100  FKPKID, Phone FROM SystemLog.dbo.BigBrandRewardLog WITH (NOLOCK) WHERE   FKBigBrandPkid=@FKBigBrandPkid AND ChanceType=1 
  ORDER BY CreateDateTime DESC) AS BBRL  LEFT JOIN Configuration.dbo.BigBrandRewardPool  BBRP WITH (NOLOCK) 
