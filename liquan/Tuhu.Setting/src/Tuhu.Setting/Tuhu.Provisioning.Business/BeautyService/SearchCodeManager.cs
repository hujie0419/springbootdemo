...
namespace Tuhu.Provisioning.Business.BeautyService
{
    public class SearchCodeManager
    {
        ...
        /// <returns></returns>
        public static async Task<Tuple<List<ServiceCodeDetail>, int>> GetBeautyServicePackageDetailCodes(string mobile, string code, int pageIndex, int pageSize)
        {
            var result = new List<ServiceCodeDetail>();
            var totalCount = 0;
            if (!string.IsNullOrEmpty(mobile))
            {
                var data = await GetServiceCodeDetailByMobile(mobile, pageIndex, pageSize);
                result = data.Item1;
                totalCount = data.Item2;
            }
            else
            {
                var data = await GetServiceCodeDetailByCode(code);
                result = data.Item1;
                totalCount = data.Item2;
            }
                                ...
                                if (packageDetail != null && products.Any())
                                {
                                    ...
                                    if (product != null)
                                    {
                                        return new ServiceCodeDetail()
                                        {
                                            ...
                                            ServiceCode = t.Code,
                                            PackageCode = packageDetailCode.PackageCode,
