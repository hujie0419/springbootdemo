...
namespace Tuhu.Service.Activity.Server.Manager
{
    public class FlashSaleCreateOrderManager
    {
        ...
        public static async Task<CreateOrderResult> FlashSaleCreateOrder(Order.Request.CreateOrderRequest request)
        {
            ...
            var normalProducts = new List<Order.Request.OrderItem>();//普通商品列表
            ...
            try
            {
                if (request.Items != null && request.Items.Any())
                {
                    foreach (var item in request.Items)
                    {
                        if (item.ActivityId.HasValue)
                        {
                            ...
                            else if ((await DalSalePromotion.CheckActivityIsEffictive(item.ActivityId.Value.ToString())))///打折活动
                            {
                                activityTypeModel =
                                new ActivityTypeModel { ActivityId = item.ActivityId.Value, Type = 11 };
                            }
                            else
                            {
                                // 不是拼团/打折 ActivityId
                            ...
                            else if (item.Price > 0 && !(bool)item.Pid?.StartsWith("FU-")) //非赠品、非服务
                            {
                                normalProducts.Add(item);
                            }
                        }
                        else if (item.Price > 0 && !(bool)item.Pid?.StartsWith("FU-"))
                        {
                            normalProducts.Add(item);
                        }
                    }
                ...
                if (flashrequest.Products != null && flashrequest.Products.Any())
                {
                    ...
                    if (orderItemses.Any())
                    {
                        ...
                        //限时抢购验证通过后 普通商品验证
                        var flashSaleNormalCheck = await CheckNormalProductAsync(flashSaleResult, fsRequest, normalProducts);
                        if (!flashSaleNormalCheck.Item1)
                        {
                            return new CreateOrderResult
                            {
                                ErrorCode = (int)Model.CheckFlashSaleStatus.NormalProductLimit,
                                ErrorMessage = request.OrderChannel == "1网站" ? Model.CheckFlashSaleStatus.NormalProductLimit.GetRemark() : flashSaleNormalCheck.Item2
                            };
                        }
                ...
                try
                {
                    using (var client = new CreateOrderClient())
                    {
                        ...
                        if (result.Success)
                        {
                            ...
                            if (discountProducts != null && discountProducts.Any())//如果是打折活动
                            {
                                foreach (var item in discountProducts)
                                {
                                    if (item.ActivityId != null && item.ActivityId != Guid.Empty)
        ...
        /// <returns></returns>
        public static async Task<bool> CheckIsPassKumhoTireFlashSale(string activityId, Guid userid)
        {
            await Task.Yield();

            var activVerifyCode = await ActivityManager.ActiveOrderVerify(new Models.Requests.Activity.ActivityOrderVerifyRequests()
            {
                ActivityId = activityId,
                UserId = userid
            });
            if (activVerifyCode == 2) //是锦湖活动并且用户未绑定  不能下单
            {
                return false;
            }
            return true;
        ...
        /// 验证抢购活动商品是否通过普通商品验证
        /// </summary>
        /// <param name="flashSaleResult"></param>
        /// <param name="fsRequest"></param>
        /// <param name="normalProducts">需要验证普通商品限购的</param>
        /// <returns></returns>
        public static async Task<Tuple<bool, string>> CheckNormalProductAsync(List<CheckFlashSaleResponseModel> flashSaleResult,
            FlashSaleOrderRequest fsRequest, List<Order.Request.OrderItem> normalProducts)
        {
            ...
            {
                //抢购活动商品中 未配置限购的商品
                var noLimits = flashSaleResult?.Where(x => !x.HasPlaceLimit && !x.HasTotalQuantityLimit && !x.HasQuantityLimit)?.ToList();
                if (noLimits?.Count > 0)
                {
                    var normalCheckRequest = noLimits.Select(x => new Order.Request.OrderItem()
                    {
                        Pid = x.PID,
                        Num = x.Num,
                        Name = fsRequest.Products.FirstOrDefault(p => p.PID.Equals(x.PID, StringComparison.CurrentCultureIgnoreCase)).ProductName
                    });
                    normalProducts.AddRange(normalCheckRequest);
                }

                //验证普通商品每单限购
                if (normalProducts.Count > 0)
                {
                    var normalCheckResult = await CheckNormalProductOrderLimit(normalProducts);
                    if (!normalCheckResult.Item1)
                    {
                        //没通过验证 限时抢购所有商品减掉计数器 (限时抢购验证中 加了限购计数器)
                        await FlashSaleCounter.DecrementAllFlashCount(fsRequest, flashSaleResult);
                        returnResult = false;
                        returnMessage = normalCheckResult.Item2;
                    }
                }
            }
            catch (Exception ex)
            {
                Logger.Error($"CheckFlashSaleNormalProductAsync异常,flashSaleResult:{JsonConvert.SerializeObject(flashSaleResult)}," +
                    $"fsRequest:{JsonConvert.SerializeObject(fsRequest)}", ex);
            }

            return new Tuple<bool, string>(returnResult, returnMessage);
        }
        ...
        /// <returns></returns>
        public static async Task<Tuple<bool, string>> CheckNormalProductOrderLimit(IEnumerable<Order.Request.OrderItem> products)
        {
            if (!(products?.Count() > 0))
            {
            ...
            bool result = true;
            var returnMessage = Model.CheckFlashSaleStatus.NormalProductLimit.GetRemark();

            try
            {
                var pids = products.Select(x => x.Pid)?.ToList();
                var productNames = new List<string>();

                //获取商品限购信息
                var pidLimitDicResult = await ProductConfigServiceProxy.GetProductLimitCountWithPidListAsync(pids);
                var pidLimitDic = pidLimitDicResult.Result;
                if (!(pidLimitDicResult.Success && pidLimitDic?.Count > 0))
                {
                    Logger.Warn($"CheckNormalProductOrderLimit =>GetProductLimitCountWithPidList失败," +
                ...
                else
                {
                    ...
                    {
                        var dicGetResult = pidLimitDic.TryGetValue(item.Pid, out int pidLimitNum);
                        if (!dicGetResult)
                        {
                            Logger.Warn($"CheckNormalProductOrderLimit=>获取限购字典数据失败,Pid:{item.Pid}");
                        }
                        else
                        {
                            if (pidLimitNum > 0 && item.Num > pidLimitNum)//pidLimitNum > 0 
                            {
                                result = false;
                    ...
                    if (!result)
                    {
                        if (productNames.Count > 0)
                        {
                            ...
                            returnMessage = "亲，以下商品超过限购数量，请修改后重新提交\r\n" + string.Join("\r\n", productNames);
                        }

                        Logger.Warn($"CheckNormalProductOrderLimit=>未通过验证,products:{JsonConvert.SerializeObject(products)}，" +
                            $"pidLimitDic:{JsonConvert.SerializeObject(pidLimitDic)}");

                        return new Tuple<bool, string>(false, returnMessage);
                    }
                }
            }
            catch (Exception ex)
            {
                Logger.Error($"CheckNormalProductOrderLimit异常,products:{JsonConvert.SerializeObject(products)}", ex);
            }

            return new Tuple<bool, string>(true, returnMessage);
        }
