...
namespace Tuhu.Service.Activity.Server.Manager
{
    public static class ActivityManager
    {
                     ...
                     new
                     {
                         ...
                         r.BackgroundColor
                            ...
                            Select(a => new ActivePageHomeModel
                            {
                                ...
                                IsHome = a.Key.IsHome,
                                BackgroundColor = a.Key.BackgroundColor,
        ...
        private static async Task<BigBrandRewardListModel> GetBigBrandAsync(string hashKey)
        {
            try
            {
                //var result = await TuhuMemoryCache.Instance.GetOrSetAsync(hashKey + "BigBrand",() => BigBrandManager.GetBigBrand(hashKey), TimeSpan.FromSeconds(15));

                var result = await TuhuMemoryCacheNoJson.Instance.GetOrSetAsync(hashKey + "BigBrand",
        ...
        private static async Task<FlashSaleModel> GetFlashSaleModelByFloor(FlashSaleModel productPool, string rim, Guid activityId, int cityId)
        {
            try
            {
                ...
                swf.Start();
                var result = await FetchRegionTiresActivityWithMemory(new FlashSaleTiresActivityRequest()
                {
        ...
        private static async Task<IEnumerable<SimpleProductModel>> SelectSimpleProductModels(string hashKey, List<string> pids)
        {
            try
            {
                var result = await TuhuMemoryCacheNoJson.Instance.GetOrSetAsync($"LocalSimpleProducts{hashKey}", async () =>
                {
                    var sku = await SelectSkuProductDetailModels(pids);
                    return sku.Select(r => new SimpleProductModel()
                    {
                        Pid = r.Pid,
                        Onsale = r.Onsale,
                        Brand = r.Brand,
                        ...
                        RootCategoryName = r.RootCategoryName,
                        Price = r.Price,
                        MarketingPrice = r.MarketingPrice
                    });
            ...
            {
                Logger.Error($"活动页从本地内存缓存中读取产品信息失败Exception{ex.Message}{ex.InnerException}");
                return new List<SimpleProductModel>();
            }
        }
        ...
        internal static async Task<string> ActivityTestMethodAsync(int testType)
        {
            await Task.Yield();
            TuhuNotification.SendNotification("test", "");
            using (var zklock = new ZooKeeperLock("test"))
            {
                if (await zklock.WaitAsync(100))
                {
                    Logger.Info($"ZooKeeperLock-->");
                }
            }
            using (var client = CacheHelper.CreateCacheClient("test"))
            {
                var time = DateTime.Now.Millisecond;
                var cachePrefix = "ActivityTestMethodAsync/" + time;
                var empetyCache = client.Get<string>(cachePrefix);
                Logger.Info($"empetyCache-->{cachePrefix}-->{JsonConvert.SerializeObject(empetyCache)}");

                var setCache = client.Set<string>(cachePrefix, "setSuccess", TimeSpan.FromMinutes(1));
                Logger.Info($"setCache-->{cachePrefix}-->{JsonConvert.SerializeObject(setCache)}");

                var getGetCache = client.GetOrSet<string>(cachePrefix, () => "setSuccess", TimeSpan.FromMinutes(1));
                Logger.Info($"getGetCache-->{cachePrefix}-->{JsonConvert.SerializeObject(getGetCache)}");
            }
            return "success";
        }
        ...
        private static async Task<bool> CheckActivityTime(ActivePageListModel activePageList, ActivtyPageRequest request)
        {
            ...
            {
                var allWihiteUsers = await GetActivityAllPageWhiteListByUserIdWithMemoryAsync();
                var isExist = allWihiteUsers.Contains(request.UserId.ToString(), StringComparer.OrdinalIgnoreCase);
                if (isExist)
                {
        ...
        private static async Task<ActivePageListModel> ActivePageListModelAssembly(ActivePageListModel activePageList, ActivtyPageRequest request)
        {
            ...
            if (allPids.Any())
            {
                var activityGuid = pageContents
                    .Where(o => o.ActivityId.HasValue && (o.Type != (int)ActivityPageContentType.ProductPool && o.Type != (int)ActivityPageContentType.NewProductPool))
                    .Select(r => r.ActivityId.Value)
                    .Distinct().ToArray();

                var productAsync = SelectSimpleProductModels(request.HashKey, allPids);
                var flashSaleDatasAsync = FlashSaleManager.GetFlashSaleDataAndQuantityByIdsAsync(activityGuid);
                await Task.WhenAll(productAsync, flashSaleDatasAsync);

                var products = (productAsync?.Result.ToList() ?? new List<SimpleProductModel>()).ToDictionary(p => p.Pid ?? "", p => p);
                var flashSaleDatas = (flashSaleDatasAsync.Result?.ToList() ?? new List<FlashSaleModel>()).ToDictionary(p => p.ActivityID, p => p);
                flashSaleDatas.Values.ForEach(p =>
                    p.ProductsDic = (p.Products ?? new List<FlashSaleProductModel>()).ToDictionary(q => q.PID, q => q));
                foreach (var pageContent in pageContents)
                {
                    SimpleProductModel item = null;
                    if (products.ContainsKey(pageContent.Pid ?? ""))
                        item = products[pageContent.Pid ?? ""];

                    if (!string.IsNullOrWhiteSpace(pageContent.Pid))
                        pageContent.ActivityPrice = item?.Price ?? 0;
                    ...
                    if (item != null)
                    {
                        ...
                        pageContent.Category = item.RootCategoryName;
                        pageContent.MarketingPrice = item.MarketingPrice;
                    }

                    FlashSaleModel sale = null;
                    if (pageContent.ActivityId.HasValue &&
                        flashSaleDatas != null &&
                        flashSaleDatas.Count > 0 &&
                        flashSaleDatas.ContainsKey(pageContent.ActivityId.Value))
                        sale = flashSaleDatas[pageContent.ActivityId.Value];
            ...
            if (model != null)
            {
                ...
                foreach (var groupContent in groupContents)
                {
                    ...
                    if (groupContent.OrigionType == (int)ActivityPageContentType.ProductPool)
                    {
                        ...
                        if (activityId != null)
                        {
                            ...
                            if (productPool.Products != null)
                            {
                                ...
                                foreach (var item in productPool.Products ?? new List<FlashSaleProductModel>().Where(r => r.IsShow == true))
                                {
                    ...
                    if (groupContent.OrigionType == (int)ActivityPageContentType.VehicleBanner)
                    {
                        ...
                        {
                            Logger.Error($"获取分车型头图失败->request?.HashKey:{request?.HashKey}->first.Group:{groupContent?.Contents?.FirstOrDefault()?.Group}", e);
                        }
                    }
                    if (groupContent.OrigionType == (int)ActivityPageContentType.NewProductPool && request.Channel == "wap")
                    {
                        var first = groupContent.Contents.FirstOrDefault();
                        var adaptAllPidList = await GetAdaptAllPidList(first, request, first.ProductType);
                        ...
                        {
                            Logger.Info($"新商品池获取排序好的商品列表为空活动{request.HashKey};组号{first.Group}sortedList:{JsonConvert.SerializeObject(sortedList)}sortedPidsRequest:{JsonConvert.SerializeObject(sortedPidsRequest)}");
                        }
                        else
                        {
                            ...
                            var query = (from product in products
                                         join actvity in flashSale.Products ?? new List<FlashSaleProductModel>() on product.Pid equals actvity.PID into temp
                            ...
                            foreach (var item in query)
                            {
                                innerContents.Add(new ActivePageContentModel
                                {
                                    ...
                                    Price = item.product?.MarketingPrice ?? item.product.Price,
                                    ...
                                    Category = item.product.RootCategoryName,
                                    TemplateType = first.TemplateType,
                                    IsProgressBar = first.IsProgressBar,
                                    IsBrandName = first.IsBrandName
                                });
                            }
        ...
        private static async Task<List<string>> GetAdaptAllPidList(ActivePageContentModel first, ActivtyPageRequest request, int productType)
        {
            ...
            switch (productType)
            {
                case (int)ProductType.AutoProduct:
                    searchRequest.Parameters["Category"] = new[] { nameof(ProductType.AutoProduct) };
                    return await SearchProductByPidsAsync(searchRequest);
                case (int)ProductType.Tires:
                    searchRequest.Parameters["Category"] = new[] { nameof(ProductType.Tires) };
                    searchRequest.Parameters["CP_Tire_Rim"] = new[] { request.Rim };
                    searchRequest.Parameters["CP_Tire_Width"] = new[] { request.Width };
                    searchRequest.Parameters["CP_Tire_AspectRatio"] = new[] { request.AspectRatio };
                    return await SearchProductByPidsAsync(searchRequest);
                case (int)ProductType.hub:
                    searchRequest.Parameters["Category"] = new[] { nameof(ProductType.hub) };
                    searchRequest.Parameters["CP_Tire_Rim"] = new[] { request.Rim };
                    return await SearchProductByPidsAsync(searchRequest);
                case (int)ProductType.Default:
                    var list = new List<string>();
                    list.AddRange(await GetAdaptAllPidList(first, request, (int)ProductType.AutoProduct));
                    list.AddRange(await GetAdaptAllPidList(first, request, (int)ProductType.Tires));
                    list.AddRange(await GetAdaptAllPidList(first, request, (int)ProductType.hub));
                    return list;
                default:
        ...
        private static void FullEntity(ActivePageContentModel detail, FlashSaleModel saleModel)
        {
            ...
            //detail.Image =  detail.Image.GetImageUrl();
            if (productTypelist.Contains(detail.Type) && !string.IsNullOrEmpty(detail.Pid))
            {
                ...
                if (detail.IsUploading == 0)
                {
                    ...
                    {
                        FlashSaleProductModel pIdSaleModel = null;
                        saleModel?.ProductsDic.TryGetValue(detail.Pid, out pIdSaleModel);

                        if (pIdSaleModel != null)
                        {
                            detail.Price = (detail.MarketingPrice == 0 || detail.MarketingPrice == null) ? pIdSaleModel.FalseOriginalPrice : detail.MarketingPrice;
        ...
        private static async Task<ActivePageListModel> ActivePageListModelAsCacheWithRedisWithMemory(int id, string channel, string hashKey)
        {
            var swAll = Stopwatch.StartNew();
            var sw = Stopwatch.StartNew();

            var cacheResult = await TuhuMemoryCacheNoJson.Instance.GetOrSetAsync(
                "ActivePageListModelAsCacheWithRedisWithMemory2" + id + channel + hashKey,
                () => ActivePageListModelAsCacheWithRedis(id, channel, hashKey),
                TimeSpan.FromMinutes(1.2));

            var result = ModelConvertExtensions.ModelConvert<ActivePageListModel, ActivePageListModel>(cacheResult);

            return result;
        }

        private static async Task<ActivePageListModel> ActivePageListModelAsCacheWithRedis(int id, string channel, string hashKey)
        {
            var swAll = Stopwatch.StartNew();
            var sw = Stopwatch.StartNew();
            using (var client = CacheHelper.CreateCacheClient(GlobalConstant.ActivityPageClientName))
            {
                ...
                var cacheResult = await client.GetOrSetAsync(GlobalConstant.ActivityPagePrefix + id + channel + hashKey,
                    () => ActivePageListModelAsCache(id, channel, hashKey), GlobalConstant.ActivityPageCache);

                if (!cacheResult.Success)
                {
                    Logger.Warn($"查询活动页接口redis缓存失败GetActivePageListModel:{GlobalConstant.ActivityPagePrefix + id + channel};Error:{cacheResult.Message}", cacheResult.Exception);
                }

                if (cacheResult.Value == null)
                {
                    Logger.Error($"查询活动页接口redis缓存失败-->缓存为null 或者无活动-->{GlobalConstant.ActivityPagePrefix + id + channel};Error:{cacheResult.Message}", cacheResult.Exception);
                }
                return cacheResult.Value;
            }
        }

        public static async Task<ActivePageListModel> GetActivePageListModelByMemory(int id, ActivtyPageRequest request, string hashKey)
        {
            ActivePageListModel result = await ActivePageListModelAsCacheWithRedisWithMemory(id, request.Channel, hashKey);

            if (result != null)
            {
                result = await ActivePageListModelAssembly(result, request);
            }
            return result;
        }
        ...
        public static async Task<ActivePageListModel> GetActivePageListModel(int id, ActivtyPageRequest request, string hashKey, bool isAssembly = true)
        {
            using (var client = CacheHelper.CreateCacheClient(GlobalConstant.ActivityPageClientName))
            {
                ...
                else
                {
                    ...
                    var dbResult = new ActivePageListModel();// await ActivePageListModelAsCache(id, channel, activityId);
                    if (retryCache)
                    {
                        var retryReslut =
                        ...
                        if (retryReslut.Success)
                            dbResult = retryReslut.Value;
                    }
                    else
                    {
                        //dbResult = await TuhuMemoryCache.Instance.GetOrSetAsync("LocalCacheForactivityError", () => ActivePageListModelAsCache(id, request.Channel, hashKey), TimeSpan.FromSeconds(30));
                        dbResult = await TuhuMemoryCacheNoJson.Instance.GetOrSetAsync("LocalCacheForactivityError", () => ActivePageListModelAsCache(id, request.Channel, hashKey), TimeSpan.FromSeconds(30));
                    }
                    if (!isAssembly)
                        result = dbResult;
                    else
                    {
                        result = await ActivePageListModelAssembly(dbResult, request);
                    }
                    return result;
                }
            }
        ...
        /// <param name="channel"></param>
        /// <returns></returns>
        public static async Task<CouponActivityConfigNewModel> SelectCouponActivityConfig
            (CouponActivityConfigRequest request)
        {
            var result = null as CouponActivityConfigNewModel;
            var config = await GetCouponActivityConfig(request.ActivityNum, request.Type);//获取活动配置
            var platforms = config?.Channel?.Split(',')?.ToList() ?? new List<string>();//活动适用平台
            var platform = ChannelConvert(request.Channel);//渠道转换为平台
            if (config != null && config.Id > 0 && !string.IsNullOrEmpty(platform) && platforms.Contains(platform))
            {
                var details = await GetCouponActivityChannelConfig(config.Id);//获取配置详情
                var jumpMethod = details?.Select(s => s.Type)?.Distinct()?.FirstOrDefault() ?? string.Empty;//跳转优惠券或者链接
                var ruleGuidList = null as List<Guid>;
                var url = string.Empty;
                switch (jumpMethod.ToUpper())
                {
                    case "COUPON":
                        details = details?.Where(s => s.GetRuleGUID != Guid.Empty &&
                        string.Equals(s.Channel, platform, StringComparison.OrdinalIgnoreCase))?.ToList();//对应渠道的优惠券的配置
                        ruleGuidList = details?.Select(s => s.GetRuleGUID)?.Distinct()?.ToList();
                        break;
                    case "URL":
                        details = details?.Where(s => !string.IsNullOrEmpty(s.Url) &&
                            string.Equals(s.Channel, platform, StringComparison.OrdinalIgnoreCase))?.ToList();//对应渠道的跳转Url配置
                        url = details?.FirstOrDefault()?.Url ?? string.Empty;
                        break;
                    default:
                        break;
                }
                result = new CouponActivityConfigNewModel()
                {
                    ActivityNum = config.ActivityNum,
                    ActivityName = config.ActivityName,
                    CheckStatus = config.CheckStatus,
                    LayerImage = config.LayerImage,
                    ActivityImage = config.ActivityImage,
                    Type = config.Type,
                    GetRuleGUIDList = ruleGuidList ?? new List<Guid>(),
                    Url = url
                };
            }
            return result;
        ...
        /// <summary>
        /// 根据活动Guid获取蓄电池/加油卡活动配置
        /// </summary>
        /// <param name="activityNum"></param>
        /// <returns></returns>
        private static async Task<SE_CouponActivityConfigModel> GetCouponActivityConfig(string activityNum, int type)
        {
            using (var client = CacheHelper.CreateCacheClient(GlobalConstant.CouponActivityConfigName))
            {
                var key = string.Format(GlobalConstant.CouponActivityConfigKey, activityNum, type);
                var result = await client.GetOrSetAsync(key,
                     () => DalActivity.GetCouponActivityConfig(activityNum, type), GlobalConstant.LazyExpireTime);
                return result?.Value;
            }
        }
        ...
        /// <returns></returns>
        private static async Task<List<SE_CouponActivityChannelConfigModel>> GetCouponActivityChannelConfig(int configId)
        {
            using (var client = CacheHelper.CreateCacheClient(GlobalConstant.CouponActivityConfigName))
            {
                var key = string.Format(GlobalConstant.CouponActivityChannelConfigKey, configId);
                var result = await client.GetOrSetAsync(key,
                     () => DalActivity.GetCouponActivityChannelConfig(configId), GlobalConstant.LazyExpireTime);
                return result?.Value;
            }
        }
        ...
        /// <param name="channel"></param>
        /// <returns></returns>
        private static string ChannelConvert(string channel)
        {
            var result = string.Empty;
            switch (channel.ToUpper())//避免大小写问题
            {
                case "JIOS":
                case "8手机":
                    result = "APP";
        ...
        public static async Task<TiresActivityResponse> FetchRegionTiresActivityWithMemory(FlashSaleTiresActivityRequest request)
        {
            var memoryResult = await TuhuMemoryCacheNoJson.Instance.GetOrSetAsync($"FetchRegionTiresActivityWithMemory{request.ActivityId}{request.RegionId}", () => FetchRegionTiresActivity(request), TimeSpan.FromSeconds(34));
            var result = ModelConvertExtensions.ModelConvert<TiresActivityResponse, TiresActivityResponse>(memoryResult);
            result.TiresFloorActivity?.ForEach(
                p => p?.OtherImgAndProductMap?.ForEach(
                    pp => pp.IsAdaptaionTireSize = !string.IsNullOrEmpty(request.TireSize) ? pp.ImgType == request.TireSize : false));
            return result;
        }
        ...
        public static async Task<TiresActivityResponse> ConvertTiresActivityModel(TiresActivityConfig data, FlashSaleTiresActivityRequest request)
        {
            ...
            if (data.TiresFloorActivity != null && data.TiresFloorActivity.Any())
            {
                ...
                foreach (var item in data.TiresFloorActivity)
                {
                    ...
                    {
                        // 查询轮胎赠品牛皮癣
                        var pids = item.TireProductFiltrationInfo.Select(p => p.PID).ToList();
                        var productsInfo = await ProductConfigServiceProxy
                            .SelectProductInfoByTagRequestAsync(new ProductInfoByTagRequest
                            {
                                Tags = new List<ProductCommonTag>() { ProductCommonTag.GiftProduct },
                                Pids = pids
                            });

                        var productsTagDetail = new List<ProductCommonTagDetail>();
                        if (productsInfo != null && productsInfo
                            .ContainsKey(nameof(ProductCommonTag.GiftProduct)))
                        {
                            productsTagDetail = productsInfo[nameof(ProductCommonTag.GiftProduct)];
                        }
                        ...
                        item.TireProductFiltrationInfo.ForEach(x =>
                        {
                            ...
                            x.ProductBannerImage = productsTagDetail?.FirstOrDefault(p => p.Pid.Equals(x.PID))?
                                .Values.FirstOrDefault(t => t.ColName.Equals("ImageUrl"))?.ColStringValue;
                        });
                    }
            ...
            IEnumerable<TiresActivityProductConfig> productInfo, IEnumerable<Models.SimpleTireProductInfo> tireInfo, IEnumerable<TireInstallmentModel> installmentInfo = null)
        {
            var result = (from fs in flashSaleInfo
                          ...
                          orderby pl.Position, pl.CreatedTime
        ...
        public static async Task<Models.ResultModel<bool>> InsertRebateApplyRecordNewAsync(RebateApplyRequest request)
        {
            ...
            #region 条件判断
            var order = await OrderServiceProxy.FetchOrderInfoByID(request.OrderId);
            var thirdPartyChannels = await OrderServiceProxy.Get3rdOrderChannel();
            var orderType = await OrderServiceProxy.CheckOrderProductTypeByOrderIdAsync(request.OrderId);
            ...
            if (!orderType.Contains("轮胎") && !orderType.Contains("保养") && !orderType.Contains("蓄电池")
                && !orderType.Contains("服务订单") && !orderType.Contains("车品订单"))
            {
                result.Code = 1004;
                result.Msg = "本活动仅限轮胎订单、保养订单、蓄电池订单、车品订单或钣喷服务订单";
                return result;
            }
            if (orderType.Contains("蓄电池"))
            {
            ...
            else if (orderType.Contains("车品订单"))
            {
                var vehicleCategroys = new List<string>
                {
                    "AudioModification",
                    "LED",
                    "Tachograph",
                ...
                var unionResult = vehicleCategroys.Intersect(order.Items?.Where(_ => !String.IsNullOrEmpty(_.Category))?.Select(x => x.Category.ToLower())?.ToList() ?? new List<string>());
                if (unionResult == null || !unionResult.Any())
                {
                    result.Code = 1012;
                    result.Msg = "车品订单返现仅限指定车品参与";
                    return result;
                }
                var relatedOrderIds = await OrderServiceProxy.GetRelatedSplitOrderIDsAsync(request.OrderId, SplitQueryType.Full);
                var codeInfo = await GetServiceCodeByOrderIds(relatedOrderIds);
                if (codeInfo == null || codeInfo.Status != KuaiXiu.Enums.ServiceCodeStatusType.Verified)
                {
                    result.Code = 1013;
                    result.Msg = "服务码未核销，请核销后再参加";
                    return result;
                }
            }
            else
            {
                if (orderType.Contains("服务订单"))
                {
                    if (order.Items?.Where(x => !String.IsNullOrEmpty(x.Category) && String.Equals(x.Category.ToUpper(), "PQFW"))?.Count() <= 0)
                    {
                        result.Code = 1011;
                        result.Msg = "服务订单仅限钣喷类型服务订单。";
                        return result;
                    }
                }
                else
            ...
            using (var helper = DbHelper.CreateDbHelper())
            {
                try
                {
                    ...
                    var existedData = await DalActivity.SelectRebateApplyConfigByParamV2(helper, request, "Rebate25");
                    if (existedData?.Where(x => String.Equals(x.Status, "Applying")
                                               || String.Equals(x.Status, "Approved")
                                               || String.Equals(x.Status, "Complete"))?.Count() > 0)
                    {
                        result.Code = 1007;
                    ...
                    else
                    {
                        ...
                        var pkid = await DalActivity.InsertRebateApplyRecordV2(helper, request, "Rebate25", order.InstallShopId ?? 0);
        ...
        /// <returns></returns>
        private static async Task<Tuhu.Service.KuaiXiu.Models.ServiceCode> GetServiceCodeByOrderIds(IEnumerable<int> orderIds)
        {
            Tuhu.Service.KuaiXiu.Models.ServiceCode serviceCode = null;
            if (orderIds != null && orderIds.Any())
            {
                foreach (var orderId in orderIds)
                {
                    var orderInfo = await KuaiXiuServiceProxy.FetchServiceCodeByTuhuOrderIdAsync(orderId);
                    if (orderInfo != null)
                    {
                        serviceCode = orderInfo;
                        break;
                    }
                }
            }
        ...
        /// <returns></returns>
        public static async Task<IEnumerable<RebateApplyResponse>> GetRebateApplyByOrderIdAsync(int orderId)
        {
            List<RebateApplyResponse> result = null;
            try
            {
                var rebateData = await DalActivity.GetAllRebateApplyByOrderId(orderId);
                if (rebateData != null && rebateData.Any())
                {
                    result = new List<RebateApplyResponse>();
                    var source25 = rebateData.Where(x => String.Equals(x.Source, "Rebate25")).OrderByDescending(o => o.PKID).FirstOrDefault();
                    var source58 = rebateData.Where(x => !String.Equals(x.Source, "Rebate25")).OrderByDescending(o => o.PKID).FirstOrDefault();
                    if (source25 != null && source25.PKID > 0)
                    {
                        result.Add(source25);
                    }
                    if (source58 != null && source58.PKID > 0)
                    {
                        source58.Source = "Rebate58";//爱卡/汽车之家渠道统一返回58元返现渠道
                        result.Add(source58);
                    }
                    if (result != null && result.Any())
                    {
            ...
            {
                Logger.Error("GetRebateApplyByOrderIdAsync 接口异常", ex);
            }
            return result ?? new List<RebateApplyResponse>();
        }
        ...
        public static async Task<List<string>> GetActivityAllPageWhiteListByUserIdWithMemoryAsync()
        {
            return await TuhuMemoryCacheNoJson.Instance.GetOrSetAsync("GetActivityAllPageWhiteListByUserIdWithMemoryAsync", GetActivityAllPageWhiteListByUserIdAsync, TimeSpan.FromMinutes(1));
        }
        private static async Task<List<string>> GetActivityAllPageWhiteListByUserIdAsync()
        {
            using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                var cacheResult = await client.GetOrSetAsync($"GetActivityAllPageWhiteListByUserIdAsync/ALL",
                    () => DalActivity.GetActivityAllPageWhiteListUserIdAsync(), TimeSpan.FromHours(1));
                if (cacheResult.Success)
                {
                    return cacheResult.Value?.ToList() ?? new List<string>();
                }
                else
                {
                    Logger.Error($"从redis中获取白名单信息报错接口GetActivityAllPageWhiteListByUserIdAsync");
                    var dbResult = await DalActivity.GetActivityAllPageWhiteListUserIdAsync();
                    return dbResult?.ToList() ?? new List<string>();
                }
            }
        }
        #endregion
...
(1, "成功"), null);
            ...
            if (product != null || flashSale != null)
            {
                ...
                if (product != null && flashSale != null)
                {
                    flashProducts = (from flash in (flashSale.Products ?? new List<FlashSaleProductModel>()).Where(r => r.IsShow == true)
                ...
                if (flashSale != null && product == null)
                {
                    ...
                    {
                        flashProducts = flashSale.Products.Where(r => r.IsShow == true).Select(i => new FlashSaleProductModel
                        {
                            PID = i.PID,
                            Position = i.Position
                        }).ToList();
                ...
                {
                    //去掉重复数据
                    flashProducts = flashProducts.Distinct(new FlashSaleProductModelCompare()).ToList();
                    //后台创建一个活动
...
(1, "成功"), cacheresult.Value.OrderBy(v => v.Value).Select(key => key.Key).ToList());
        ...
        /// </summary>
        /// <returns></returns>
        public static async Task<OperationResult<ActivityResponse>> GetActivityByTypeId(int activityTypeId)
        {
            try
            {
                using (var cacheClient = CacheHelper.CreateCacheClient(GlobalConstant.ActivityType))
                {
                    var cacheResult = await cacheClient.GetOrSetAsync($"GetActivityByTypeId/{activityTypeId}", async () =>
                    {
                        //AutoMapper初始化配置文件
                        var config = new MapperConfiguration(cfg =>
                        {
                        ...
                        //活动对象
                        var activityModel = await DalActivity.GetActivityByTypeId(activityTypeId);
                    ...
                    return OperationResult.FromResult<ActivityResponse>(null);
                }
            }
            catch (Exception e)
            {
                Logger.Error($"获取活动对象失败", e.InnerException ?? e);
                return OperationResult.FromError<ActivityResponse>("-1", Resource.Activity_Error);
            }
        ...
        public static async Task<OperationResult<IEnumerable<Models.Response.Question>>> SearchQuestionAsync(Guid userId, long activityId)
        {
            try
            {
                ...
                {
                    cacheFlag = configClient.GetOrSetRuntimeSwitch("ActivitySearchQuestionCache")?.Result?.Value ?? false;
                }
                        ...
                        await questions.ForEachAsync(async question =>
                        {
                            ...
                            if (cacheFlag)
                            {
                                ...
                                */
                                ...
                                {
                                    //用户是否已经答题
                                    var answerRecordInfo = await SearchQuestionnaireAnswerRecordInfo(true, userId,
                            ...
                            {
                                //用户是否已经答题
                                var answerRecordInfo = await DalQuestionnaireAnswerRecord.SearchQuestionnaireAnswerRecordInfo(true, userId,
                                    activityModel.QuestionnaireID,
                                    question.QuestionID);
                                //获取用户的答案
                                if (answerRecordInfo != null && question.QuestionOptionList != null)
        ...
        {
            var func = new Func<Task<List<Models.Response.Question>>>(async () =>
            {
                //AutoMapper初始化配置文件
                var config = new MapperConfiguration(cfg =>
                {
                ...
                questionArray.ForEach(p =>
                {
                    ...
                    //当前问题的选项
                    var questionOptions = questionnaireOptions
                        .Where(x => x.QuestionID == p.QuestionID)
        ...
        private static List<QuestionOption> RecursiveQuestionOption(List<QuestionOption> questionOptionModels)
        {
            ...
            foreach (var questionOptionModel in topOptions)
            {
                ...
                questionOptionModel.SubOptions = questionOptionModels
            ...
            try
            {
                ...
                using (var cacheClient = CacheHelper.CreateCacheClient(GlobalConstant.ActivityType))
                {
                    var cacheResult = await cacheClient.GetOrSetAsync($"/GetVictoryInfoAsync/{activityId}/{userId:N}", async () =>
                    {
                        ...
                        //获取用户兑换券
                        var userCoupon = await DalActivity.GetActivityCouponByUserId(true, userId, activityId);
        ...
        /// </summary>
        /// <returns></returns>
        public static async Task<OperationResult<UserObjectModel>> FetchUserByUserIdByCacheAsync(Guid userId)
        {
            using (var cacheClient = CacheHelper.CreateCacheClient(GlobalConstant.ActivityType))
            {
                var cacheKey = $"/Activity/FetchUserByUserIdByCache/{userId:N}";
        ...
        ///     排行榜 数据 脱敏
        /// </summary>
        private static string CouponRankNickNameDataMasking(string nickName)
        {
            ...
            long activityId, int pageIndex = 1, int pageSize = 20)
        {
            try
            {
                    ...
                    using (var userClient = new UserClient())
                    {
                        ...
                        await rankResponse.ForEachAsync(async p =>
                        {
                            ...
                            if (userInfo != null && userInfo.Success && userInfo.Result != null)
                            {
                                ...
                                p.NickName = CouponRankNickNameDataMasking(userInfo.Result.Nickname);
                            }
                ...
                {
                    var cacheResult =
                        await cacheClient.GetOrSetAsync($"SearchCouponRank/{activityId}/{pageIndex}/{pageSize}", func, TimeSpan.FromMinutes(1));
                    if (cacheResult.Success)
                    {
                        return cacheResult.Value;
                    }
        ...
        private static async Task<IEnumerable<ActivityPrizeOrderDetailModel>> SearchPrizeOrderDetailID(Guid userId, long activityId)
        {
            ...
            using (var cacheClient = CacheHelper.CreateCacheClient(GlobalConstant.ActivityType))
            {
                var result = await cacheClient.GetOrSetAsync($"/SearchPrizeOrderDetailID/{activityId}/{userId:N}", async () =>
                 {
                     var userPrizeListIds = await DalActivity.SearchPrizeOrderDetailID(true, userId, activityId);
                     return userPrizeListIds.ToList();
                 }, TimeSpan.FromMinutes(1));
                return result?.Value;
            }
        }
        ...
        /// <param name="activityId"></param>
        /// <returns></returns>
        private static async Task SyncPrizeOrderDetailIDCache(Guid userId, long activityId)
        {
            //读取缓存
            using (var cacheClient = CacheHelper.CreateCacheClient(GlobalConstant.ActivityType))
            {
                var userPrizeListIds = await DalActivity.SearchPrizeOrderDetailID(false, userId, activityId);

                cacheClient.Set($"/SearchPrizeOrderDetailID/{activityId}/{userId:N}", userPrizeListIds.ToList(), TimeSpan.FromMinutes(1));
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private static async Task SyncActivityCoupon(Guid userId, long activityId)
        {
            using (var cacheClient = CacheHelper.CreateCacheClient(GlobalConstant.ActivityType))
            {
                var userCoupon = await DalActivity.GetActivityCouponByUserId(false, userId, activityId);

                await cacheClient.SetAsync($"/GetUserActivityCoupon/{activityId}/{userId:N}", userCoupon, TimeSpan.FromMinutes(1));
            ...
            using (var db = DbHelper.CreateDbHelper())
            {
                ...
                if (promotionResult?.Success == true && promotionResult?.Result?.IsSuccess == true)
                {
                    try
                    {
                        ...
                        await Task.WhenAll(
                            //删除库存缓存
                            RemovePrizeStockCache(prizeId),
                            //同步用户已兑换
                            SyncPrizeOrderDetailIDCache(userId, activityId),
                            //同步用户兑换券
                            SyncActivityCoupon(userId, activityId)
                            );
                        return true;
                    }
                    catch (Exception e)
        ...
        /// <returns></returns>
        public static async Task<ActiveCustomerSettingResponse> GetCustomerSettingInfoAsync(string activityExclusiveId)
        {
            using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                var result = await client.GetOrSetAsync(string.Concat("CustomerSettingInfo/", activityExclusiveId), () => DalActivity.SelectCustomerExclusiveSettingInfo(activityExclusiveId, true), TimeSpan.FromHours(2));

                ActiveCustomerSettingResponse customerActivity;

                if (result.Success)
                {
                    customerActivity = result.Value;
                }
                else
                {
                    Logger.Warn($"GetCustomerSettingInfoAsync:{string.Concat("CustomerSettingInfo/", activityExclusiveId)};Error:{result.Message}", result.Exception);
                    // redis查询失败查数据库
                    customerActivity = await DalActivity.SelectCustomerExclusiveSettingInfo(activityExclusiveId, true);
                }
        ...
        /// <param name="userid">用户Id</param>
        /// <returns></returns>
        public static async Task<string> SelectUserCouponCode(string activityExclusiveId, string userid)
        {
            return await GetUserCouponCode(activityExclusiveId, userid);
        }
        ...
        public static async Task<int> CouponCodeBound(ActivityCustomerCouponRequests activityCustomerCouponRequests)
        {
            ...
            string couponCode = await GetUserCouponCode(activityCustomerCouponRequests.ActivityExclusiveId, activityCustomerCouponRequests.UserId + "");

            //当前用户未绑定优惠券时进行绑券操作
            if (string.IsNullOrWhiteSpace(couponCode))
            {
                int selectCouponCodeCount = await DalActivity.SelectCouponCode(activityCustomerCouponRequests.ActivityExclusiveId, activityCustomerCouponRequests.CouponCode);

                if (selectCouponCodeCount <= 0)
                {
                    result = 1; //该券码不存在或已经绑定其他账号，请填写正确券码
                }
                else
                {
                    if (await DalActivity.CouponCodeBound(activityCustomerCouponRequests) > 0)
                    {
                        using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
                        {
                            ///绑定券码成功后设置redis标记
                            string couponBoundKey = string.Concat("CouponBund", activityCustomerCouponRequests.ActivityExclusiveId, activityCustomerCouponRequests.UserId);

                            var cacheResult = await client.SetAsync(couponBoundKey, "true", TimeSpan.FromSeconds(12));

                            if (!cacheResult.Success)
                                Logger.Warn($"set redis缓存失败，接口==》CouponCodeBound");
                        }
                    }
                    else
                    {
        ...
        /// 客户专享活动下单验证（锦湖员工轮胎）
        /// </summary>
        /// <param name="activityOrderVerifyRequests">0:限时抢购ID在锦湖活动中配置，
        /// 且用户已绑定;1：标识此限时抢购ID未在活动中配置；2：此用户未在锦湖活动中绑定</param>
        /// <returns></returns>
        public static async Task<int> ActiveOrderVerify(ActivityOrderVerifyRequests activityOrderVerifyRequests)
        {
            int result = 0;

            ActiveCustomerSettingResponse activeCustomerSettingResponse = await DalActivity.SelectCustomerSettingActivityId(activityOrderVerifyRequests.ActivityId);

            if (activeCustomerSettingResponse == null)
            {
                result = 1; //标识此限时抢购ID未在活动中配置
            }
            else
            {
                string couponCode = await GetUserCouponCode(activeCustomerSettingResponse.ActivityExclusiveId, activityOrderVerifyRequests.UserId + "");

                if (string.IsNullOrEmpty(couponCode))
                    result = 2; //标识此用户未在锦湖活动中绑定
            }
            return result;
        }
        ...
        /// <param name="userid"></param>
        /// <returns></returns>
        private static async Task<string> GetUserCouponCode(string activityExclusiveId, string userid)
        {
            bool readOnly = true;
            ...
            {
                //获取绑券redis标记
                string couponBoundKey = string.Concat("CouponBund", activityExclusiveId, userid);
                var cacheResult = await client.GetAsync(couponBoundKey);
                if (cacheResult.Success)
                    readOnly = false; //redis标记存在 查询写库
            }
            return await DalActivity.SelectUserCouponCode(activityExclusiveId, userid, readOnly);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static async Task<List<ActivityCustomerInvalidOrderResponse>> GetExceptionalCustomerOrderInfo()
        {
            var resultList = new List<ActivityCustomerInvalidOrderResponse>();

            try
            {
                //1.获取大客户配置信息
                var customerSettings = await DalActivity.SelectValidCustomerSetting();
                var flashIds = customerSettings.Select(x => new Guid(x.ActivityId))?.Distinct()?.ToList();
                if (!(bool)(flashIds?.Any()))
                {
                    Logger.Warn($"GetExceptionalCustomerOrderInfo未获取到大客户配置的活动id");
                    return resultList;
                }

                //获取大客户活动最早开始绑定券码的时间,当作活动开始启用时间
                var firstCreateCoupon = await DalActivity.GetCouponFirstCreateTimes();
                foreach (var item in customerSettings)
                {
                    var firstTime = firstCreateCoupon?.FirstOrDefault(x => x.ActivityExclusiveId == item.ActivityExclusiveId);
                    if (firstTime == null)
                    {
                        item.CreateTime = new DateTime(2018, 9, 1);
                    }
                    else
                    {
                        item.CreateTime = firstTime.CreateTime;
                    }
                }

                //2.获取限时抢购活动信息
                var flashSaleInfo = await DalFlashSale.SelectFlashSaleListFromDBAsync(flashIds);
                if (flashSaleInfo?.Item1?.Count() < flashIds.Count())
                {
                    Logger.Warn($"GetExceptionalCustomerOrderInfo获取活动信息数据缺失:配置活动id数：{flashIds.Count()}，获取到活动数：{flashSaleInfo?.Item1?.Count()}");
                }

                //获取2年内活动id的下单记录信息
                var activityOrderInfos = await DalActivity.SelectActivityProductOrderRecords(flashIds);

                //3.获取在大客户配置券码后 活动下的单
                var afterCustomerOrderIDs = activityOrderInfos.Join(customerSettings, x => x.ActivityId.ToString(), y => y.ActivityId, (x, y) =>
                {
                    return new
                    {
                        x.ActivityId,
                        x.OrderId,
                        CustomerSettingCreateTime = y.CreateTime,
                        OrderCreateTime = x.CreateDateTime
                    };
                })?.Where(x => x.OrderCreateTime > x.CustomerSettingCreateTime)?.Select(x => x.OrderId);
                var afterCustomerOrderInfos = activityOrderInfos?.Where(x => (bool)afterCustomerOrderIDs?.Contains(x.OrderId));

                if (!(bool)(afterCustomerOrderInfos?.Any()))
                {
                    Logger.Warn($"GetExceptionalCustomerOrderInfo未获取到大客户配置的活动的订单记录");
                    return resultList;
                }

                //4.获取用户的券信息
                var orderUsers = afterCustomerOrderInfos.Select(x => x.UserId).Distinct()?.ToList();
                var userCouponInfoList = await DalActivity.SelectCustomerSettingByUserIds(orderUsers);
                if (!(bool)(userCouponInfoList?.Any()))
                {
                    Logger.Warn($"GetExceptionalCustomerOrderInfo未获取到下单用户的券信息");
                }

                //5.验证订单
                var userGroup = afterCustomerOrderInfos.GroupBy(x => x.UserId);//按userID分组
                foreach (var userItem in userGroup)
                {
                    var activityGroup = userItem.GroupBy(x => x.ActivityId);//按活动ID分组

                    foreach (var activityItem in activityGroup)
                    {
                        //检查 用户-活动 券码
                        //1.无活动券码 2.订单不在活动券码有效期：a.券码被删除 b.券码未删除
                        var userCouponList = userCouponInfoList.Where(x => x.UserId == userItem.Key && new Guid(x.ActivityId) == activityItem.Key);

                        if (!(userCouponList?.Count() > 0))
                        {
                            //用户没有该活动券码
                            var invalidModel = new ActivityCustomerInvalidOrderResponse()
                            {
                                UserId = userItem.Key,
                                InvalidType = "NOCOUPON",
                                OrderIDs = activityItem.Select(x => x.OrderId)?.ToList(),
                                ActivityId = activityItem.Key.ToString(),
                                IsCouponDeleted = 0,
                                Phone = userItem.FirstOrDefault()?.Phone
                            };
                            resultList.Add(invalidModel);
                        }
                        else
                        {
                            ...
                            //检查是否在券码有效期内下单的
                            var notInTime = new List<int>();//未删除无效
                            var deleteNotInTime = new List<int>();//已删除无效

                            foreach (var order in activityItem)//遍历订单信息
                            {
                                //未删除,有效:订单时间在券有效期
                                var coupon = userCouponList.FirstOrDefault(x => x.Status == 0 && x.CreateTime < order.CreateDateTime);
                                //未删除无效
                                var invalidCoupon = userCouponList.FirstOrDefault(x => x.Status == 0 && x.CreateTime > order.CreateDateTime);
                                //已删除有效
                                var deleteValidCoupon = userCouponList.FirstOrDefault(x => x.Status != 0 && x.UpdateDateTime != null
                                        && x.CreateTime < order.CreateDateTime && x.UpdateDateTime > order.CreateDateTime);

                                if (coupon == null && deleteValidCoupon == null)
                                {
                                    if (invalidCoupon != null)
                                    {
                                        //未删除无效
                                        notInTime.Add(order.OrderId);
                                    }
                                    else
                                    {
                                        //已删除无效
                                        deleteNotInTime.Add(order.OrderId);
                                    }
                                }
                            }

                            if (notInTime?.Count() > 0)
                            {
                                var invalidModel = new ActivityCustomerInvalidOrderResponse()
                                {
                                    UserId = userItem.Key,
                                    InvalidType = "COUPONOVERTIME",
                                    OrderIDs = notInTime,
                                    DetailInfo = $"券码:{userCouponList.FirstOrDefault(x => x.Status == 0)?.CouponCode}," +
                                                 $"券码创建时间:{userCouponList.FirstOrDefault(x => x.Status == 0)?.CreateTime}",
                                    ActivityId = activityItem.Key.ToString(),
                                    Phone = userItem.FirstOrDefault()?.Phone
                                };
                                resultList.Add(invalidModel);
                            ...
                            if (deleteNotInTime?.Count() > 0)
                            {
                                var invalidModel = new ActivityCustomerInvalidOrderResponse()
                                {
                                    UserId = userItem.Key,
                                    InvalidType = "COUPONOVERTIME",
                                    OrderIDs = deleteNotInTime,
                                    ActivityId = activityItem.Key.ToString(),
                                    DetailInfo = $"券码:{userCouponList.FirstOrDefault(x => x.Status != 0)?.CouponCode}," +
                                                 $"券码创建时间:{userCouponList.FirstOrDefault(x => x.Status != 0)?.CreateTime}" +
                                                 $"券码删除时间:{userCouponList.FirstOrDefault(x => x.Status != 0)?.UpdateDateTime}",
                                    IsCouponDeleted = 1,
                                    Phone = userItem.FirstOrDefault()?.Phone
                                };
                                resultList.Add(invalidModel);
                            ...
                            //检查用户购买数是否超出活动总限购
                            var flashModel = flashSaleInfo.Item1?.FirstOrDefault(x => x.ActivityID == activityItem.Key) ?? new FlashSaleModel();
                            flashModel.Products = flashSaleInfo.Item2?.Where(x => x.ActivityID == activityItem.Key) ?? new List<FlashSaleProductModel>();

                            var userActivityNum = activityItem.Sum(x => x.Quantity);
                            if (userActivityNum > flashModel.PlaceQuantity && flashModel.PlaceQuantity != null)
                            {
                                var info = $"活动限购数:{flashModel.PlaceQuantity},用户购买数量:{userActivityNum}";
                                var invalidModel = new ActivityCustomerInvalidOrderResponse()
                                {
                                    UserId = userItem.Key,
                                    InvalidType = "OVERACTIVITYLIMIT",
                                    OrderIDs = activityItem.Select(x => x.OrderId)?.ToList(),
                                    ActivityId = activityItem.Key.ToString(),
                                    DetailInfo = info,
                                    Phone = userItem.FirstOrDefault()?.Phone
                            ...
                            {
                                //检查用户的商品购买数是否超出PID限购
                                var pidGroup = activityItem.GroupBy(x => x.Pid);
                                foreach (var pidItem in pidGroup)//按PID分组
                                {
                                    var pidModel = flashModel.Products.FirstOrDefault(x => x.PID.ToUpper() == pidItem.Key.ToUpper());
                                    var userPidBuyNum = pidItem.Sum(x => x.Quantity);
                                    if (userPidBuyNum > pidModel?.MaxQuantity && pidModel?.MaxQuantity != null)
                                    {
                                        var orderIds = pidItem.Select(x => x.OrderId) ?? new List<int>();
                                        var info = $"PID:{pidItem.Key},PID限购数:{pidModel.MaxQuantity},用户购买数量:{userPidBuyNum}";
                                        var invalidModel = new ActivityCustomerInvalidOrderResponse()
                                        {
                                            UserId = userItem.Key,
                                            InvalidType = "OVERPIDLIMIT",
            ...
            {
                Logger.Error($"GetExceptionalCustomerOrderInfo异常,ex:{ex}");
            }
        ...
        /// <param name="request"></param>
        /// <returns></returns>
        public static async Task<OperationResult<bool>> AddStarRatingStoreAsync(AddStarRatingStoreRequest request)
        {
            var config = new MapperConfiguration(cfg =>
            {
                cfg.CreateMap<Models.Requests.Activity.AddStarRatingStoreRequest, StarRatingStoreModel>();
            });
            var mapper = config.CreateMapper();
            StarRatingStoreModel starModel = mapper.Map<StarRatingStoreModel>(request);
            //判断用户手机号是否已经登记
            int count = await DalActivity.GetStarRatingStoreByPhone(starModel.Phone);
            if (count > 0)
            {
                return OperationResult.FromError<bool>("0", Resource.ParameterError_Phone_IsRegisted);
            }
            else if (count == 0)
            {
                int result = await DalActivity.AddStarRatingStoreAsync(starModel);
                if (result > 0)
        ...
        /// <returns></returns>
        public static async Task<ActiveCustomerSettingResponse> GetVipCustomerSettingInfoByActivityId(string activityId)
        {
            using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                var result = await client.GetOrSetAsync(string.Concat("CustomerSettingInfo/", activityId), () => DalActivity.SelectCustomerSettingInfoByActivityId(activityId), TimeSpan.FromHours(2));
                ...
                {
                    Logger.Warn($"GetVipCustomerSettingInfoByActivityId:{string.Concat("CustomerSettingInfo/", activityId)};Error:{result.Message}", result.Exception);
                    // redis查询失败查数据库
                    customerActivity = await DalActivity.SelectCustomerSettingInfoByActivityId(activityId);
                }
        ...
        public static async Task<bool> AddRegistrationOfActivitiesDataAsync(RegistrationOfActivitiesRequest request)
        {
            try
            {
                return await DalActivity.AddRegistrationOfActivitiesDataAsync(request);
            }
            catch (Exception ex)
            {
                Logger.Error($"AddRegistrationOfActivitiesDataAsync -> {JsonConvert.SerializeObject(request)}", ex);
                throw ex;
            }
        }
        #endregion
    ...
    /// <summary>
    /// FlashSaleProductModel 按pid比较器类
    /// </summary>
    public class FlashSaleProductModelCompare : IEqualityComparer<FlashSaleProductModel>
    {
        public bool Equals(FlashSaleProductModel x, FlashSaleProductModel y)
        {
            if (!string.IsNullOrWhiteSpace(x?.PID) && !string.IsNullOrWhiteSpace(y?.PID))
            {
                return x.PID == y.PID;
            }
            else
        ...
        public int GetHashCode(FlashSaleProductModel obj)
        {
            if (obj == null)
                return 0;
