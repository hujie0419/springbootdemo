...
var domain_huodong = 'http://huodong.tuhu.cn/';
...
var IsWxForIosApp = false;              //客户端是否安装微信客户端 0 否 1是
...
function VersionForIos_v1(version) {
    isIosVersion = true;
...
function VersionForAndroid_v1(version) {
...
var InitNewsApi = {
    ...
    GlobalTrigger: function (version) {
        if (browser.versions.android) {
            VersionForAndroid_v1(version);
        }
        else if (
            (browser.versions.ios || browser.versions.iPhone || browser.versions.iPad)
            && browser.versions.webApp == false) {
            VersionForIos_v1(version);
        }
    }("6.6.6")
}
...
function Countloading(pkid) {
    ...
    if (!getCookie("ArticleloadCookie")) { SetCookie("ArticleloadCookie", GetDateTime1() + CreateNum()) }
    ...
    if (browser.versions.weixin) {
        ...
        uuidstr = "FWX|" + cookieId;
    }
    try {
        var e = new Image;
        e.id = "ArticleNewTemplateLoad";
        ...
        "Login": function () {
            if (userAgentInfo.indexOf("Android") >= 0) {
                window.WebViewJavascriptBridge.callHandler(
                    ...
                    , function (responseData) {
                        if (responseData != null) {
                            DeviceVersion.CallBackAppLoginResponse(responseData);
                        }
                    });
            }
            else {
                DeviceVersion.iosend("tuhuaction://getuserinfo#0#", "DeviceVersion.CallBackAppLoginResponse");
            }
        },
...
setTimeout(function () {
    if (browser.versions.android) {
        if ((AndroidVersion || "").length > 0 && AndroidVersion.localeCompare("4.9.8") >= 0) {
            DeviceVersion.Login();
        }
        else {
            Countloading(_PKID);
...
//javascript:AllGoTo.InvokeApp('TR-PR-P7C','15','tire');    //轮胎
//javascript:AllGoTo.InvokeH5('http://xxx/');               //h5
//javascript:AllGoTo.InvokeBY('xby;dby;pm;rx', 'upkeep');   //保养
//javascript:AllGoTo.InvokeNewsPKID(110);                   //跳转发现
//javascript:AllGoTo.InvokeHWAndQJD('http://xxx/');         //跳转旗舰店，与轮胎花纹
...
var AllGoTo = {
    ...
    InvokeNewsPKID: function (pkid) {
        pkid = pkid || 0;
        if (pkid > 0) {
            $.ajax({
                url: domain_huodong + "Advertise/SelectNewsUrl?callback=?",
                data: { "pkid": pkid },
                dataType: "jsonp",
                type: "get",
                success: function (data) {
                    console.log(['InvokeNewsPKID', pkid].join(","));
                    if ((data || "").length > 0) {
    ...
    InvokeHWAndQJD: function (url) {
        if (GetAndroidVersion >= 498 || GetIosVersion >= 343) {
...
//=================================统计 Begin=====================================//
var _hmt = _hmt || [];
(function () { var t = document.createElement("script"), n; t.async = !0; t.src = "//hm.baidu.com/hm.js?16afd1d01373c6f2bf619ff824868a5e"; n = document.getElementsByTagName("script")[0]; n.parentNode.insertBefore(t, n) })();
(function (i, s, o, g, r, a, m) { i['GoogleAnalyticsObject'] = r; i[r] = i[r] || function () { (i[r].q = i[r].q || []).push(arguments) }, i[r].l = 1 * new Date(); a = s.createElement(o), m = s.getElementsByTagName(o)[0]; a.async = 1; a.src = g; m.parentNode.insertBefore(a, m) })(window, document, 'script', 'https://www.google-analytics.com/analytics.js', 'ga'); ga('create', 'UA-24728550-3', 'auto'); ga('send', 'pageview');
//=================================统计 End======================================//