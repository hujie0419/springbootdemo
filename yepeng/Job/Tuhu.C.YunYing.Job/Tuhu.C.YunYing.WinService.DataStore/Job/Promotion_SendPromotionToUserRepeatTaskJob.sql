        ...
        BEGIN TRY
            ...
            WHILE ( @index < @rows )
                BEGIN
				  ...
				  --单用户多优惠券插入情况(一个用户的多个订单都满足塞券条件，需要多次塞券。每次循环对第一个订单进行塞券)
                    NOEMPTY:
                    ...
                    WHILE ( @index_rule < @rows_rule )
						...
						--【调用塞券PROC】 有记录并且自动发送开关为打开状态
                            ...
                            IF ( EXISTS ( SELECT    *
                                ...
                                BEGIN
							        DECLARE @PromotionReferKey NVARCHAR(200)= 'JOB:' +  convert(NVARCHAR(200),@PromotionTaskId) ;
