    ...
    public class PushManagerController : Controller
    {
        ...
        public async Task<ActionResult> UpdatePlanInfo()
        {
            ...
            try
            {
                ...
                {
                    var result =
                        await PushManagerNew.UpdateTemplatePlanInfoAsync(Convert.ToInt32(BatchID), PlanName,
                            templatestatus);
                    jr.Data = new { code = result > 0 ? 1 : 0, msg = result > 0 ? "更新成功" : "更新失败" };
                    return jr;
                }
        ...
        public async Task<ActionResult> SetPlanReadyPush(int batchid)
        {
            ...
            try
            {
                ...
                {
                    if (pushTemplate.PushStatus == PushStatus.Prepare)
                    {
        ...
        public async Task<JsonResult> QuerySinglePushTemplateAsync(TemplateQuery query)
        {
            ...
            query.PageIndex = query.PageIndex == 0 ? 1 : query.PageIndex;
            query.PageSize = 50;
            query.SendTypes = new List<SendType>() { SendType.Single, SendType.Groupcast };
            var datas = await PushManagerNew.SelectPushTemplatesAsync(query);
            if (datas != null)
            {
                jr.Data = new
        ...
        public async Task<JsonResult> SubmitSingleTemplateAsync(SinglePushSubmitModel model, string ispreview,
            string targets, string planname, string TemplateTag)
        {
            JsonResult jr = new JsonResult();
                ...
                if (pushtemplate.DeviceType == DeviceType.WeChat || pushtemplate.DeviceType == DeviceType.WeiXinApp)
                {
                    using (var client = new Tuhu.Service.Push.WeiXinPushClient())
                    {
                        var tasks = targets.Split(',')
                             .Select(x => client.PushByTemplateAsync(pushtemplate, new PushTemplateLog()
                             {
                                 Target = x
                             }));
                        var ss = await Task.WhenAll(tasks);
                        ss.ForEach(x => issuccess &= x.Result);
                        //var result = client.PushByTemplate(pushtemplate, new PushTemplateLog()
                        //{
                        //    Target = targets
                        //});
            ...
            else
            {
                ...
                if (batchid != 0)
                {
                    await PushManagerNew.UpdateTemplatePlanInfoAsync(batchid, planname, new Dictionary<int, PushStatus>());
                    int tagtemp;
        ...
        public async Task<JsonResult> SelectTemplatesByBatchIDAsync(int? batchid)
        {
            ...
            if (datas.Count != 5)
            {
                ...
                var templatetypes = datas.Select(x => x.DeviceType);
                var notexists = devicevalues.Except(templatetypes);
                if (notexists.Any())
                {
            ...
            Func<PushTemplate, string> GetTemplateWxID = t =>
           {
            ...
            jr.Data = new
            {
                ...
                datas = datas.OrderBy(x => x.DeviceType).Select(x => new
                {
                    ...
                    x.WxAppEmKeyWord,
                    WxTemplateID = GetTemplateWxID(x)//x.DeviceType != DeviceType.WeChat ? string.Empty : (!string.IsNullOrEmpty(x.WxTemplateID) ? x.WxTemplateID : allwxtemplates.FirstOrDefault()?.template_id)
                })
            };
            return jr;
        }
