...
app.controller('ProductVehicleTypeController', ['$scope', 'BusinessService', function ($scope, businessService) {
    ...
    $scope.getAllApiInfo = function () {
        ...
        var pid = decodeURI(urlParams[urlParams.length - 1]);
    ...
    ***************************************************************/
    ...
    $("#CheckAll").click(function () {
        if (confirm("确认要选中所有车型吗？")) {
            $("input[type='checkbox']").prop("checked", true);   
            $("dl a").attr("class", "seled");
            $scope.checkAll = true;
        }
    });
    $("#CancerAll").click(function () {
        if (confirm("确认要取消选中所有车型吗？")) {
            $("input[type='checkbox']").prop("checked", false);
            $("dl a").removeClass("seled");
            $scope.checkAll = false;
        }
    });
    $("#btnSave").click(function () {
        ...
        var paramItems = [];
        var checkItems = [];
        var configlevel = $("#hiddenLevel").val();
        ...
        $.ajax({
            url: '/ProductVehicleType/UpdateProductVehicleTypeConfigInfo',
            type: 'POST',
            dataType: 'JSON',
            data: { param: JSON.stringify(paramItems), level: configlevel, pid: pid, selectItems: JSON.stringify(selectParams) },
    ...
    $("#filter a").click(function () {
        ...
        var chexiStr = "";
        var chexiSelects = $("#checkAllBrand").parent().parent().parent().find("input[type='checkbox']");
        $.each(chexiSelects, function () {
            if ($(this).is(":checked")) {
                chexiStr += $(this).parent().text() + ",";
            }
        });
        console.log(chexiStr.substring(0, chexiStr.length - 1));

        var priceStr = "";
        var priceSelects = $("#checkAllPrice").parent().parent().parent().find("input[type='checkbox']");
        $.each(priceSelects, function () {
            if ($(this).is(":checked")) {
                priceStr += $(this).parent().text() + ",";
            }
        });
        console.log(priceStr.substring(0, priceStr.length - 1));

        var vehicleTypeStr = "";
        var vehicleTypeSelects = $("#checkAllVehicle").parent().parent().parent().find("input[type='checkbox']");
        $.each(vehicleTypeSelects, function () {
            if ($(this).is(":checked")) {
                vehicleTypeStr += $(this).parent().text() + ",";
            }
        });
        console.log(vehicleTypeStr.substring(0, vehicleTypeStr.length - 1));
        ...
        businessService.list(postData).success(function (response) {
            ...
            if ($scope.checkAll) {
                //var checkBoxArray = $(".search-list").find("input[type='checkbox']");
                //console.log(checkBoxArray);
                //for (var i = 0; i < checkBoxArray.length; i++) {
                //    $(checkBoxArray[i]).prop("checked", true);
                //}
                $(".search-list").find("input[type='checkbox']").prop("checked", true);
            }
    ...
    $("#checkAllBrand").on('change', function () {
        if ($(this).is(":checked")) {//品牌全选
            $(this).parent().parent().parent().find("input[type='checkbox']").prop("checked", true);//全选
        } else {
            $(this).parent().parent().parent().find("input[type='checkbox']").prop("checked", false);//反选
        }
    })

    $("#checkAllPrice").on('change', function () {
        if ($(this).is(":checked")) {//品牌全选
            $(this).parent().parent().parent().find("input[type='checkbox']").prop("checked", true);//全选
        } else {
            $(this).parent().parent().parent().find("input[type='checkbox']").prop("checked", false);//反选
    ...
    $("#checkAllVehicle").on('change', function () {
        if ($(this).is(":checked")) {//品牌全选
            $(this).parent().parent().parent().find("input[type='checkbox']").prop("checked", true);//全选
        } else {
            $(this).parent().parent().parent().find("input[type='checkbox']").prop("checked", false);//反选
    ...
    $("#filter input[type='checkbox']").on('change', function () {
        ...
        //    show: true
        //});
        //当筛选条件中的 checkbox 选中状态变化时，向后台重新请求车型数据
        var chexiStr = "";
        var chexiSelects = $("#checkAllBrand").parent().parent().parent().find("input[type='checkbox']");
        $.each(chexiSelects, function () {
            if ($(this).is(":checked")) {
...
app.factory('BusinessService', ['$http', function ($http) {
    var list = function (postData) {
        return $http.post('/ProductVehicleType/GetAllVehicleTypeInfoByParams', postData);
    };
