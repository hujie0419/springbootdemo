...
namespace Tuhu.Provisioning.Controllers
{
    public class UnivRedemptionCodeController : Controller
    {
        ...
        public JsonResult GetGroupSettings(int PageSize, int PageIndex, string KeyWord)
        {
            ...
            var appIds = manager.GetAllBigCustomerOpenAppIds();
            return Json(new { result = list, appIdList = appIds, total = count });
        }
        [HttpPost]
        ...
        #region 第三方appId配置
        public ActionResult ThirdAppSetting()
        {
            return View();
        }
        public JsonResult GetThirdAppSettings(int PageSize, int PageIndex, string KeyWord)
        {
            int count = 0;
            var list = manager.GetThirdAppSettings(out count, PageIndex, PageSize, KeyWord);
            return Json(new { result = list, total = count });
        }
        public JsonResult GetThirdAppSettingsDetail(int pkid)
        {
            if (pkid <= 0)
                return Json(new { result = new OpenAppModel() { BigCustomerStatus = "", EncryptType = "", OrderChannel = "", SignType = "" } });

            var model = manager.GetThirdAppSettingDetail(pkid);
        ...
        private void GetAppInfo(OpenAppModel model)
        {
            ...
            while (count > 0)
            {
                var appId = $"th{RandomHelper.Instanse.GetStringFromRandomString(16).Trim()}";
        ...
        public JsonResult DeleteThirdAppSetting(int pkid)
        {
            if (pkid <= 0)
                return Json(new {result = false, msg = "pkid参数不能为空"});
            var result = manager.DelThirdAppSetting(pkid, User.Identity.Name);
            var msg = result ? "" : "服务端执行异常";
            return Json(new {result = result, msg = msg});
        }
        #endregion

        #region RedemptionQuery
        public ActionResult QueryRedemptionCode()
        {
            return View();
        }

        /// <summary>
        /// 根据兑换码或手机号查询通用兑换码和美容兑换码信息
        /// </summary>
