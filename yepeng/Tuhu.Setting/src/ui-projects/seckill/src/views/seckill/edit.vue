...
export default {
    ...
    methods: {
        loadEditData () {
            ...
            if (this.strStatus === "新建" || this.strStatus === "已发布" || this.strStatus === "售卖中" || this.strStatus === "已驳回") {
                this.disabled = false;
            ...
            if (this.activityId != null) {
                ...
                this.ajax
                    ...
                    .then(response => {
                        ...
                        // _this.origiondata = response.data.slice(0);
                    });
            }
        },
        formatPrice (price) {
                    ...
                    _this.ajax.get("/Seckill/FetchProductWithCostPriceByPID", {
                        ...
                        .then(response => {
                            ...
                            if (!(returnVaule.length)) {
                                ...
                                _this.data[index].CostPrice = response.data.CostPrice;

                                _this.data[index].OriginalPrice = response.data.cy_list_price;

                                if (!(_this.data[index].Price.length > 1)) {
                                    _this.data[index].Price = response.data.cy_list_price;
                                }
                                ...
                                if (originPrice > 0) {
                                    if (parseFloat(price) <= 0 || parseFloat(price) > parseFloat(originPrice)) {
                                        _this.data[index].Error += "秒杀价高于原价，请查看是否配置错误";
                                    }
        ...
        toSave () {
            ...
            if (this.origiondata.length !== this.data.length) {
                NeedExam = true;
            }
            if (!NeedExam) {
                console.log("1" + this.origiondata)
                console.log("2" + this.data)
                this.origiondata.forEach(element => {
                    this.data.forEach(e => {
                        if (element.PID === e.PID) {
                            count++;
                            if (element.Price > e.Price) {
                                count--;
                            }
                        }
                    })
                });
                if (count !== this.origiondata.length) {
                    NeedExam = true;
                }
                console.log(count);
            }
            ...
            if (!_this.data.length) {
                ...
                _this.data.forEach(function (e, i) {
                    console.log("i: ", i, e)
                    var msgInfo = "";
                    if (e.Position != null && e.Position.length && parseInt(e.Position) + "" === "NaN") { msgInfo += "排序填写不规范<br/>"; }
                    if (!e.Price.toString().length) {
                        msgInfo += '秒杀价不允许为空<br/>';
                    } else if (isNaN(e.Price)) { msgInfo += "秒杀价请输入正确的数字格式<br/>"; } else if (parseFloat(e.Price) <= 0 || parseFloat(e.Price) > parseFloat(e.OriginalPrice)) { msgInfo += "秒杀价高于原价，请查看是否配置错误<br/>"; }
                    if (!e.FalseOriginalPrice.toString().length) { msgInfo += '伪原价不允许为空<br/>'; } else if (isNaN(e.FalseOriginalPrice)) { msgInfo += "伪原价请输入正确的数字格式<br/>"; } else if (parseFloat(e.FalseOriginalPrice) <= 0 || parseFloat(e.FalseOriginalPrice) < parseFloat(e.Price)) { msgInfo += "伪原价低于产品价，请查看是否配置错误<br/>"; }
                    if (e.MaxQuantity.length && parseInt(e.MaxQuantity) + "" === "NaN") { msgInfo += "个人限购数量请输入正确的数字格式<br/>"; }
                    if (!e.TotalQuantity.toString().length) { msgInfo += "秒杀库存不允许为空<br/>"; }
                    if (e.TotalQuantity.length && parseInt(e.TotalQuantity) + "" === "NaN") { msgInfo += "秒杀库存请输入正确的数组格式<br/>"; }
                    if (e.TotalQuantity < e.MaxQuantity) { msgInfo += "每人限购的数量高于秒杀库存，请确认<br/>"; }
                    if (msgInfo.length) {
                        _this.wrongMsg.push({
                            PID: e.PID,
                            msgInfo: msgInfo
        ...
        checkPrice (index) {
            ...
            if (originPrice > 0) {
                if (parseFloat(price) <= 0 || parseFloat(price) > parseFloat(originPrice)) {
                    this.data[index].Error = "秒杀价高于原价，请查看是否配置错误";
                }
                var degree = parseFloat(Math.abs(parseFloat(originPrice) - parseFloat(price)) / parseFloat(originPrice)).toFixed(2);
                this.data[index].DecreaseDegree = this.toPercent(degree);
            }
