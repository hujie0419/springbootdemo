...
WHERE  PKID=@PKID";
  ...
  ([PackageProductId]
   ...
   ,GetDate()
        ...
        /// <returns></returns>
        public static async Task<List<GeneralBeautyServerCodes>> GetGeneralBeautyServerCodes(string phone, string serviceCode, int pageIndex, int pageSize)
        {
            const string sql = @"
            SELECT  pbpr.Phone ,
                pbpr.PackageId ,
                bprd.ServiceCode ,
                bppc.SettlementMethod ,
                bppc.SettlementPrice ,
                bstc.PID ,
                bstc.AdapterVehicle ,
                muc.CooperateName 
        FROM    Tuhu_groupon..ThirdPartyBeautyPackageRecord AS pbpr WITH ( NOLOCK )
                LEFT JOIN Tuhu_groupon..ThirdPartyBeautyPackageRecordDetail AS bprd
                WITH ( NOLOCK ) ON pbpr.PKID = bprd.PackageRecordId
                LEFT JOIN Tuhu_groupon..ThirdPartyBeautyPackageConfig AS bpc
                WITH ( NOLOCK ) ON pbpr.PackageId = bpc.PackageId
                LEFT JOIN Tuhu_groupon..MrCooperateUserConfig AS muc WITH ( NOLOCK ) ON bpc.CooperateId = muc.PKID
                LEFT JOIN Tuhu_groupon..ThirdPartyBeautyPackageProductConfig
                AS bppc WITH ( NOLOCK ) ON bprd.PackageProductId = bppc.PKID
                LEFT JOIN Tuhu_groupon..BeautyServiceCodeTypeConfig AS bstc
                WITH ( NOLOCK ) ON bppc.CodeTypeConfigId = bstc.PKID
        WHERE   pbpr.IsDeleted = 0
                AND pbpr.Status = 1
                AND ( @Phone = ''
                      OR @Phone IS NULL
                      OR pbpr.Phone = @Phone
                ...
                AND ( @ServiceCode = ''
                      ...
                      OR bprd.ServiceCode = @ServiceCode
                    )
        ORDER BY pbpr.PKID DESC
            ...
            using (var dbHelper = new SqlDbHelper(strConn_OnlyRead))
            {
        ...
        {
            const string sql = @"SELECT PackageId ,PackageName
             FROM   [Tuhu_groupon].[dbo].[ThirdPartyBeautyPackageConfig] WITH ( NOLOCK )
             ORDER BY PKID DESC";
           
            using (var conn = new SqlConnection(strConn_OnlyRead))
            {
                return SqlHelper.ExecuteDataTable(conn, CommandType.Text, sql)?.ConvertTo<ThirdPartyBeautyPackageConfigModel>()?.ToList();
            }
        }
    }
}
