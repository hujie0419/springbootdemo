...
namespace Tuhu.C.YunYing.WinService.JobSchedulerService.BaoYangCoupon.Job
{
    ...
    internal class BaoYangFreeQuan0413Job : IJob
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(BaoYangFreeQuan0413Job));

        public void Execute(IJobExecutionContext context)
        {
            try
            {
                if (DateTime.Now < new DateTime(2017, 4, 14) || DateTime.Now > new DateTime(2017, 6, 16))
                {
                    //                    返券条件：
                    //①使用了(优惠券规则编号：3295 / 优惠券规则Guid：aa04dd4d - ed8a - 4ee1 - b886 - e1e12a73fc89)的优惠券的订单

                    //②销售渠道：u门店，订单内包含胜牌 / 海湾 / 出光任意品牌机油的保养订单

                    //满足以上任一条件即需返券

                    //订单状态：已安装
                    //发券时间：订单安装完成后5天
                    //每天跑一次，给符合要求的用户塞券（每个用户只返一次，返券金额以第一个符合返券条件的已安装完成订单为准），同时发送短信通知，短信不要夜里发送
                    //短信内容：感谢您参与指定品牌保养，机油买一送一活动，活动返券已发至您的账户，可下载手机APP查看 dwz.cn / Beul5

                    //发券要求：
                    //名称：机油抵用券
                    //说明：机油买一送一活动返券，胜牌 / 海湾 / 出光品牌到店保养专用
                    //优惠券RuleID：2006
                    //有效期：2017.5.1 - 2017.10.31
                    //面值：与原订单内所有机油产品实付金额相同
                    //最低使用金额：与面值相同
                    Logger.Info("2017-4-14~2017-6-15发放机油买一送一券");
                    return;
                }
                Guid id = Guid.NewGuid();
                Logger.Info("启动任务机油买一送一券" + id);
                var dt = DalCoupon.Get0413BaoYangFreeOrderList();
                ...
                using (var orderClient = new OrderQueryClient())
                {
                    dic.GroupBy(g => g.UserID).ForEach(f =>
                    {
                        ...
                        if (!DalCoupon.GetHasGetPromotionCodeFor0413(new Guid(temp.UserID)))
                        {
                            ...
                            {
                                var orderList = DalCoupon.SelectOrderListByOrderIdFor0413(orderIds.Result.Result);
                                if (orderList != null && orderList.Any())
                                {
                                    ...
                                    if (result > 0)
                                    {
                                        ...
                                        BLL.Business.SendMarketingSms(userTel, "指定品牌保养，机油买一送一");
                                    }
                                }
                                else
                                {
                            ...
                            {
                                Logger.Error("OrderID:" + temp.OrderID + "查询拆单订单数据出错,数据为空");
                            }
                        }
                        else
                        {
        ...
        public static int CreateOrYzPromotion(string userId, int money, string orderID)
        {
            try
            {
                ...
                promotionCodeModel.Description = "机油买一送一活动返券，胜牌/海湾/出光品牌到店保养专用";
                ...
                promotionCodeModel.CodeChannel = "机油买一送一券";
                promotionCodeModel.BatchId = Convert.ToInt32(orderID);
                promotionCodeModel.RuleId = 2006;
                promotionCodeModel.Number = 1;
                var result = DalCoupon.CreatePromotionCode(promotionCodeModel, null, "BaoYangFreeQuan0413Job");
                return result;
            }
            catch (Exception ex)
            {
