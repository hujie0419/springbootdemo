...
";
        ...
        {
            var pagedList = new PagedModel<UserRegistrationResponse> { Pager = new PagerModel() };
            ...
            string sql = @"SELECT   R.[PKID] ,
                                    ...
                                    [Status] ,
                                    R.[CreateDateTime]
                            FROM    Activity.[dbo].[T_UserRegistrations] AS R WITH ( NOLOCK ) LEFT JOIN
                                    Activity.[dbo].[T_UserActivities] AS A ON R.[ActivityId] = A.[PKID]
                            WHERE   [Area] LIKE @Area AND IsDeleted = 0
                            ORDER BY R.[PKID] DESC
            ...
            using (var cmd = new SqlCommand(sql))
            {
                ...
                pagedList.Source = (await DbHelper.ExecuteSelectAsync<UserRegistrationResponse>(true, cmd)).ToList();
            }

            string sqlCount = @"SELECT  COUNT(*)
                                FROM    Activity.[dbo].[T_UserRegistrations] WITH ( NOLOCK )
                                WHERE   [Area] LIKE @Area AND IsDeleted = 0;";

            using (var cmd = new SqlCommand(sqlCount))
            {
                cmd.Parameters.AddWithValue("@Area", $"{area}%");
                pagedList.Pager.Total = (int)await DbHelper.ExecuteScalarAsync(true, cmd);
            }
        ...
        {
            string sql = @"SELECT [PKID] ,[Content]
                            FROM Activity.[dbo].[T_UserActivities] WITH ( NOLOCK )";

            using (var cmd = new SqlCommand(sql))
            {
                return (await DbHelper
                    .ExecuteSelectAsync<UserActivityResponse>(true, cmd)).ToList();
            }
        }
        ...
        {
            string sql = @"SELECT [PKID] ,[Name] ,[Phone] ,[Area] ,[ActivityId]
                            FROM Activity.[dbo].[T_UserRegistrations] WITH ( NOLOCK )
            ...
            {
                cmd.Parameters.AddWithValue("@PKID", pkid);
                return (await DbHelper.ExecuteSelectAsync<UserRegistrationResponse>(true, cmd)).FirstOrDefault();
            }
        }
        ...
        /// 新增用户报名信息
        /// </summary>
        public static async Task<int> AddUserRegistrationAsync(UserRegistrationRequest request)
        {
            var sql = @"INSERT INTO [Activity].[dbo].[T_UserRegistrations] WITH ( ROWLOCK )
                         ([Name] ,[Phone] ,[Area] ,[ActivityId] ,[Status] ,[CreateDateTime] ,[IsDeleted])
			             VALUES(@Name ,@Phone ,@Area ,@ActivityId ,0 ,GETDATE() ,0)";
        ...
        /// 修改用户报名信息
        /// </summary>
        public static async Task<int> ModifyUserRegistrationAsync(UserRegistrationRequest request)
        {
            var sql = @"UPDATE [Activity].[dbo].[T_UserRegistrations] WITH ( ROWLOCK )
                                SET [Name] = @Name ,
                                    [Phone] = @Phone ,
                                    [Area] = @Area ,
            ...
            using (var cmd = new SqlCommand(sql))
            {
                ...
                cmd.Parameters.AddWithValue("@Area", request.Area);
                cmd.Parameters.AddWithValue("@ActivityId", request.ActivityId);
                cmd.Parameters.AddWithValue("@PKID", request.PKID);
                return await DbHelper.ExecuteNonQueryAsync(false, cmd);
            }
        }
        ...
        public static async Task<int> UpdateUserRegistrationStatusAsync(string area, RegistrationStatus status)
        {
            var sql = @"UPDATE [Activity].[dbo].[T_UserRegistrations] WITH ( ROWLOCK )
                         SET [Status] = @Status
                         WHERE Area = @Area AND Status = 0 AND IsDeleted = 0";
