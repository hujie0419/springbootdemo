...
namespace Tuhu.Provisioning.Business
{
    public class SE_HomePageConfigManager
    {
        ...
        public IEnumerable<SE_HomePageModuleContentConfig> GetHomePageContentListV2(int? moduleID, bool isMoreCity, int? moduleHelperID)
        {
            using (var c = ProcessConnection.OpenGungnirReadOnly)
            {
                return new DALSE_HomePageConfig().GetHomePageContentListV2(c, moduleID, isMoreCity, moduleHelperID);
            }
        }
        ...
        public int AddHomePageContent(SE_HomePageModuleContentConfig model, int pkid)
        {
            using (var c = ProcessConnection.OpenGungnir)
            {
                var list = new List<SE_HomePageModuleTagConfig>();
                if (!string.IsNullOrEmpty(model.ModuleTags))
                {
                    var tags = model.ModuleTags.Split(';');
                    foreach (var tag in tags)
                    {
                        if (string.IsNullOrWhiteSpace(tag))
                            continue;

                        var tagCfg = new SE_HomePageModuleTagConfig
                        {
                            FKTagIDs = tag
                        };
                        list.Add(tagCfg);
                ...
                return new DALSE_HomePageConfig().AddHomePageContent(c, model, pkid, list);
            }
        }
        ...
        public bool UpdateHomePageContent(SE_HomePageModuleContentConfig model, int pkid)
        {
            using (var c = ProcessConnection.OpenGungnir)
            {
                ...
                return new DALSE_HomePageConfig().UpdateHomePageContent(c, model, pkid, list);
            }
        }
        ...
        /// <param name="pkid"></param>
        /// <returns></returns>
        public bool DeleteHomePageContent(SE_HomePageModuleContentConfig model, SE_HomePageModuleConfig pageModel)
        {
            using (var c = ProcessConnection.OpenGungnir)
            {
                return new DALSE_HomePageConfig().DeleteHomePageContent(c, model, pageModel);
            }
        }
        ...
        /// <returns></returns>
        public List<SE_HomePageConfigTags> SelectModuleTags(int parentId = 0)
        {
            using (var c = ProcessConnection.OpenGungnirReadOnly)
            {
                return new DALSE_HomePageConfig().SelectModuleTags(c, parentId);
            }
        }
        ...
        /// <returns></returns>
        public List<SE_HomePageConfigTags> SelectModuleTagsById(string ids)
        {
            using (var c = ProcessConnection.OpenGungnirReadOnly)
            {
                return new DALSE_HomePageConfig().SelectModuleTagsById(c, ids);
            }
        }
        ...
        /// <param name="ids"></param>
        /// <returns></returns>
        public List<SE_HomePageModuleTagConfig> GetModuleTagsConfigList(int moduleId)
        {
            using (var c = ProcessConnection.OpenGungnirReadOnly)
            {
                return new DALSE_HomePageConfig().GetModuleTagsConfigList(c, moduleId);
            }
        }
    }
}
