...
using Tuhu.Provisioning.DataAccess.Entity.Paint;
...
VALUES  ( @PID , 
          ...
          @ServerType 
            ...
            var parameters = new SqlParameter[]
            {
                ...
                new SqlParameter("@ServerType",config.ServerType)
            };
            using (var dbhelper = new SqlDbHelper(writeConnStr))
            {
                return dbhelper.ExecuteNonQuery(sql, CommandType.Text, parameters) > 0;
            ...
            var parameters = new SqlParameter[]
{
        ...
        /// <returns></returns>
        public static UnivBeautyRedemptionCodeResult FetchBeautyServicePackageCodeByPackageCode(string packageCode)
        {
        ...
        B.CooperateId ,
        C.CooperateName ,
        A.PackageCode AS RedemptionCode ,
        A.StartTime ,
        A.EndTime ,
        A.ExchangeTime ,
        A.CreateTime ,
        B.CreateUser ,
        'BeautyServicePackageCode' AS RedemptionCodeType ,
        CASE A.IsActive
          WHEN 1 THEN '0New'
          WHEN 0 THEN '3Invalidated'
        END AS Status ,
        A.PackageId AS BatchCode
FROM    Tuhu_groupon..BeautyServicePackageCode A WITH ( NOLOCK )
        JOIN Tuhu_groupon..BeautyServicePackage B WITH ( NOLOCK ) ON A.PackageId = B.PKID
        LEFT JOIN Tuhu_groupon..MrCooperateUserConfig C WITH ( NOLOCK ) ON B.CooperateId = C.PKID
WHERE   A.PackageCode = @PackageCode;";
            using (var dbHelper = new SqlDbHelper(readConnStr))
            {
                return dbHelper.ExecuteDataTable(sql, CommandType.Text, new SqlParameter("@PackageCode", packageCode)).ConvertTo<UnivBeautyRedemptionCodeResult>().FirstOrDefault();
            }
        }
        /// <summary>
        /// 根据美容兑换码查询服务码
        ...
        /// <param name="packageCode"></param>
        /// <returns></returns>
        public static IEnumerable<RedemptionCodeAndServiceCodeMap> GetBeautyServicePackageDetailCodeByPackageCode(string packageCode)
        {
            const string sql = @"
            SELECT  ServiceCode ,
                    PackageCode AS RedemptionCode ,
                    CreateTime
            FROM    Tuhu_groupon..BeautyServicePackageDetailCode WITH ( NOLOCK )
            WHERE   PackageCode = @PackageCode;";
            using (var dbHelper = new SqlDbHelper(readConnStr))
            {
                return dbHelper.ExecuteDataTable(sql, CommandType.Text, new SqlParameter("@PackageCode", packageCode)).ConvertTo<RedemptionCodeAndServiceCodeMap>();
            }
        }
        /// <summary>
        /// 根据兑换码获取服务码
        ...
        /// <returns></returns>
        public static IEnumerable<RedemptionCodeAndServiceCodeMap> GetServiceCodesByRedemptionCode(string redemptionCode)
        {
            const string sql = @"
              SELECT   RedemptionCode ,
                        ServiceCode ,
                        CreateTime
                FROM    Tuhu_groupon..RedeemMrCodeRecord WITH ( NOLOCK )
                WHERE   RedemptionCode = @RedemptionCode;";
            using (var dbHelper = new SqlDbHelper(readConnStr))
            {
                return dbHelper.ExecuteDataTable(sql, CommandType.Text, new SqlParameter("@RedemptionCode", redemptionCode)).ConvertTo<RedemptionCodeAndServiceCodeMap>();
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static Tuple<IEnumerable<UnivBeautyRedemptionCodeResult>, int> SelectBeautyServicePackageCodesByUserId(Guid userId, int pageIndex, int pageSize)
        {
...
ORDER BY A.CreateTime DESC
        ...
        ONLY;";
            ...
            using (var dbHelper = new SqlDbHelper(readConnStr))
            {
                var list = dbHelper.ExecuteDataTable(sql, CommandType.Text, parameters).ConvertTo<UnivBeautyRedemptionCodeResult>();
                var total = Convert.ToInt32(parameters.Last().Value);
        ...
        /// <param name="redemptionCode"></param>
        /// <returns></returns>
        public static IEnumerable<RedeemPromotionRecord> GetRedeemPromotionRecordByRedemptionCode(string redemptionCode)
        {
            const string sql = @"SELECT * FROM Tuhu_groupon..RedeemPromotionRecord  WITH (NOLOCK) WHERE RedemptionCode=@RedemptionCode";
            using (var dbHelper = new SqlDbHelper(readConnStr))
            {
                return dbHelper.ExecuteDataTable(sql, CommandType.Text, new SqlParameter("@RedemptionCode", redemptionCode)).ConvertTo<RedeemPromotionRecord>();
            }
        }
        /// <summary>
        /// 获取保养兑换码得信息
        ...
        /// <param name="redemptionCode"></param>
        /// <returns></returns>
        public static IEnumerable<VipBaoYangGiftPackCouponDetail> GetVipBaoYangGiftPackCouponDetail(string redemptionCode)
        {
            const string sql = @"
            SELECT  A.RedemptionCode ,
                    A.Carno
            FROM    BaoYang..VipBaoYangGiftPackCouponDetail AS A WITH ( NOLOCK )
            WHERE   A.RedemptionCode = @RedemptionCode";
            using (var dbHelper = new SqlDbHelper(ConfigurationManager.ConnectionStrings["Gungnir_AlwaysOnRead"].ConnectionString))
            {
                return dbHelper.ExecuteDataTable(sql, CommandType.Text, new SqlParameter("@RedemptionCode", redemptionCode)).ConvertTo<VipBaoYangGiftPackCouponDetail>();
            }
        }
        /// <summary>
        /// 获取喷漆优惠券得兑换信息
        ...
        /// <returns></returns>
        public static IEnumerable<VipPaintPackagePromotionDetail> GetVipPaintPackagePromotionDetail(IEnumerable<string> promotionIds)
        {
            const string sql = @"
            SELECT  A.BatchCode ,
                    A.CarNo ,
                    A.PromotionId
            FROM    Configuration..VipPaintPackagePromotionDetail AS A WITH ( NOLOCK )
            WHERE   A.Status = 'SUCCESS'
                    AND EXISTS ( SELECT 1
                                 FROM   Configuration..SplitString(@PromotionIdStr, ',', 1)
                                        AS t
                                 WHERE  A.PromotionId = t.Item )";
            using (var dbHelper = new SqlDbHelper(ConfigurationManager.ConnectionStrings["Gungnir_AlwaysOnRead"].ConnectionString))
            {
                return dbHelper.ExecuteDataTable(sql, CommandType.Text, new SqlParameter("@PromotionIdStr", string.Join(",", promotionIds))).ConvertTo<VipPaintPackagePromotionDetail>();
            }
        }
    }
}
