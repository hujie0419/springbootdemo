    ...
    public class SalePromotionActivityController : Controller
    {
        ...
        private Tuple<List<SalePromotionActivityProduct>, List<SalePromotionActivityProduct>> CheckInsertProduct(string activityId, List<SalePromotionActivityProduct> productList)
        {
            ...
            if (productList?.Count > 0)
            {
                ...
                if (productInfoList?.Count > 0)
                {
                    ...
                    {
                        var infoModel = productInfoList.FirstOrDefault(p => p.Pid == item.Pid);
                        if (infoModel != null && !string.IsNullOrWhiteSpace(infoModel.Pid))
                        {
                            var model = new SalePromotionActivityProduct()
                            {
                                ActivityId = activityId,
                                Pid = item.Pid,
                                ProductName = infoModel.DisplayName,
            ...
            if (tempSuccessList.Count > 0)
            {
                var stockList = Manager.GetProductStockAndCostList(tempSuccessList.Select(p => p.Pid).ToList());
                if (stockList?.Count > 0)
                {
                    foreach (var item in tempSuccessList)
                    {
                        var stockModel = stockList.FirstOrDefault(p => p.Pid == item.Pid);
        ...
        private Tuple<List<string>, List<SalePromotionActivityProduct>> GetValidStockList(List<SalePromotionActivityProduct> products, int stock)
        {
            ...
            foreach (var item in products)
            {
                ...
                else if (item.TotalStock < stock)
                {
                    item.FailMessage = $"限购库存超出商品总库存,总库存{item.TotalStock}";
                    item.LimitQuantity = stock;
                    failList.Add(item);
                }
                else
        ...
        public ActionResult ValidAddPid(string activityId, string pid)
        {
            ...
            //3.验证商品是否存在
            var productInfoList = Manager.GetProductPriceAndName(new List<string>() { pid });
            if (!(productInfoList?.Count > 0))
            {
                return Json(new { Status = false, Msg = $"Pid不存在" });
            }
            //4.验证商品是否已有库存
            var stockList = Manager.GetProductStockAndCostList(new List<string>() { pid });
            if (!(stockList?.Count > 0))
            {
                return Json(new { Status = false, Msg = $"商品库存不存在" });
            }
            else if (stockList[0].TotalStock == 0)
            {
                return Json(new { Status = false, Msg = $"商品库存为0" });
            }
        ...
        {
            var priceList = Manager.GetProductPriceAndName(pids);
            var costList = Manager.GetProductStockAndCostList(pids);
            //  var activityProductList = Manager.GetProductInfo(activityId);
            var result = (from p in priceList
                          join c in costList on p.Pid equals c.Pid into temp
                          ...
                          select new SalePromotionActivityProduct()
                          {
                              ...
                              SalePrice = p.Price,
                              CostPrice = tt != null ? tt.CostPrice : 0,
                              TotalStock = tt != null ? tt.TotalStock : 0
                          })?.ToList();
            return result;
        }
