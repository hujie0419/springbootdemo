    ...
    public class FlashSaleUnitTest
    {
        ...
        public void SelectFlashSaleDataByActivityIdsAsync()
        {
            using (var client = new FlashSaleClient())
            {
                ...
                {
                    activityID = new Guid("b6540440-d125-400c-8b5e-d9a7a4ff6246"),
                    pids = new List<string>() { "TR-DP-ST-T1|25", "TR-DP-ST-T1|26" }
                };
                listFlashSaleDataByActivityRequest.Add(flashSaleDataByActivityRequest);


                var result = client.SelectFlashSaleDataByActivityIdsAsync(listFlashSaleDataByActivityRequest);
                Assert.IsNotNull(result.Result);
            }
        }
    }
