...
namespace Tuhu.Service.Activity.Server.Manager
{
    public class FlashSaleManager
    {
            ...
            DateTime? scheduleDate = null, bool needProducts = true)
        {
            ...
            {
                var result = await cacheClient.GetOrSetAsync($"SecondKillPrefix{year}/{month}/{day}/{activityType}",
                ...
                {
                    Logger.Warn(
                        $"获取当日秒杀产品redis数据失败SelectSecondKillTodayDataAsync:{$"SecondKillPrefix{year}/{month}/{day}/{activityType}"};Error:{result.Message}",
                        result.Exception);
