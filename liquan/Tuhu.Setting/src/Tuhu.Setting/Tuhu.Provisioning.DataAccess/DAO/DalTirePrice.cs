...
FROM    ( SELECT    T.*, ( CASE WHEN T.cost IS NULL THEN NULL
                    ...
                    ( CASE WHEN @SingleCustomPrice = 'list' THEN T.Price
                           ...
                           WHEN @SingleCustomPrice = 'coupon' THEN T.CouponPrice
                           WHEN @SingleCustomPrice = 'tmlpj' THEN T.TMLPJPrice
                           ELSE T.Price
                      END ) AS SingleCustomPrice
          FROM      ( SELECT    VP.CP_Brand AS Brand, VP.PID,VP.cy_marketing_price AS MarketingPrice,
                                VP.DisplayName AS ProductName,VP.OnSale,VP.CP_Tire_ROF as Rof,
                                ( ISNULL(( SELECT TOP 1
                                                    TPGP.Value
                                ...
                                JDJY.Price AS JDJYPrice, JDJY.SKUID AS JDJYPID, 
                                JDFW.Price AS JDFWPrice, JDFW.SKUID AS JDFWPID,
                                CT.NewPrice AS CouponPrice, DPSD.CaigouZaitu,
                                TMLPJ.Price as TMLPJPrice, TMLPJ.ItemID as TMLPJId
                      FROM      Tuhu_productcatalog.dbo.vw_Products AS VP WITH ( NOLOCK )
                                ...
                                LEFT JOIN Tuhu_productcatalog..ProductPriceMapping
                                AS JDJY WITH ( NOLOCK ) ON JDJY.ShopCode = N'5京东机油店'
                                                          AND VP.PID = JDJY.Pid
                                LEFT JOIN Tuhu_productcatalog..ProductPriceMapping
                                AS JDFW WITH ( NOLOCK ) ON JDFW.ShopCode = N'5京东服务店'
                                                          AND VP.PID = JDFW.Pid
                                LEFT JOIN Tuhu_productcatalog..ProductPriceMapping
                                AS TMLPJ WITH ( NOLOCK ) ON TMLPJ.ShopCode = N'天猫养车零配件官方直营'
                                                          AND VP.PID = TMLPJ.Pid
                      ...
                      WHERE     ( ( VP.PID LIKE '%' + @PID + '%'
                                ...
                                AND ( @IsShow = 2
                                 ...
                                 AND ( @CP_Tire_ROF = N'不限'
                                      OR @CP_Tire_ROF = N'非防爆'
                                      AND VP.CP_Tire_ROF = N'非防爆'
                                      OR @CP_Tire_ROF = N'防爆'
                                      AND VP.CP_Tire_ROF =N'防爆'
                                    )
                                AND ( @MaoLiE IS NULL
                                ...
                                AND ( @QplMaoLiE IS NULL
                                      OR ( @QplMaoLiE IS NOT NULL
                                           AND QPL.Price > 0
                                           AND DPSD.cost > 0
                                           AND ( @MatchQplMaoLiE = 1
                                                 AND ISNULL(QPL.Price,
                                                            0)
                                                 - ISNULL(DPSD.cost, 0) >= @QplMaoLiE
                                                 OR @MatchQplMaoLiE = -1
                                                 AND ISNULL(QPL.Price,
                                                            0)
                                                 - ISNULL(DPSD.cost, 0) <= @QplMaoLiE
                                ...
                                AND ( @QplMaoLiLv IS NULL
									  OR ( @QplMaoLiLv IS NOT NULL
										   AND QPL.Price > 0
                                           AND DPSD.cost > 0
										   AND ISNULL(QPL.Price, 0) - ISNULL(DPSD.cost, 0) > 0
										   AND ( @MatchQplMaoLiLv = 1
												 AND ( ISNULL(QPL.Price, 0) - ISNULL(DPSD.cost, 0) ) / IIF(QPL.Price <= 0, 1, QPL.Price) >= @QplMaoLiLv * 1.0 / 100
												 OR @MatchQplMaoLiLv = -1
												 AND ( ISNULL(QPL.Price, 0) - ISNULL(DPSD.cost, 0) ) / IIF(QPL.Price <= 0, 1, QPL.Price) <= @QplMaoLiLv * 1.0 / 100
											   )
										 )
									)
                                AND ( @ShopMaoLiE IS NULL
                                      OR ( @ShopMaoLiE IS NOT NULL
                                           AND VP.cy_list_price > 0
                                           AND QPL.Price > 0
                                           AND ( @MatchShopMaoLiE = 1 
										   AND ISNULL(VP.cy_list_price, 0) - ISNULL(QPL.Price, 0) >= @ShopMaoLiE
										   OR @MatchShopMaoLiE = -1
                                           AND ISNULL(VP.cy_list_price, 0) - ISNULL(QPL.Price, 0) <= @ShopMaoLiE
                                               )
                                         )
                                    )
                                AND ( @ShopMaoLiLv IS NULL
									  OR ( @ShopMaoLiLv IS NOT NULL
										   AND VP.cy_list_price > 0
                                           AND QPL.Price > 0
										   AND ISNULL(VP.cy_list_price, 0) - ISNULL(QPL.Price, 0) > 0
										   AND ( @MatchShopMaoLiLv = 1
												 AND ( ISNULL(VP.cy_list_price, 0) - ISNULL(QPL.Price, 0) ) / IIF(VP.cy_list_price <= 0, 1, VP.cy_list_price) >= @ShopMaoLiLv * 1.0 / 100
												 OR @MatchShopMaoLiLv = -1
												 AND ( ISNULL(VP.cy_list_price, 0) - ISNULL(QPL.Price, 0) ) / IIF(VP.cy_list_price <= 0, 1, VP.cy_list_price) <= @ShopMaoLiLv * 1.0 / 100
											   )
										 )
									)
                         ...
                         AND ( ( U.Price
                               ...
                               OR ( U.CouponPrice
                                ...
                                OR ( U.TMLPJPrice
                                    / IIF(ISNULL(U.SingleCustomPrice, 0) = 0, 1, U.SingleCustomPrice) = @Proportion
                                    AND U.TMLPJPrice > 0
                                    AND CHARINDEX('tmlpj', @ManyCustomPrice) > 0
                                  )
                              )
                           )
                   )
...
FETCH NEXT @PageSize ROWS ONLY;";
                ...
                using (var cmd = new SqlCommand(sql))
                {
                    ...
                    cmd.Parameters.AddRange(new SqlParameter[] {
                        ...
                        new SqlParameter("@QplMaoLiLv",model.QplMaoLiLv),
                        new SqlParameter("@MatchQplMaoLiLv",model.MatchQplMaoLiLv),
                        new SqlParameter("@QplMaoLiE",model.QplMaoLiE),
                        new SqlParameter("@MatchQplMaoLiE",model.MatchQplMaoLiE),
                        new SqlParameter("@ShopMaoLiE",model.ShopMaoLiE),
                        new SqlParameter("@MatchShopMaoLiE",model.MatchShopMaoLiE),
                        new SqlParameter("@ShopMaoLiLv",model.ShopMaoLiLv),
                        new SqlParameter("@MatchShopMaoLiLv",model.MatchShopMaoLiLv),
                        ...
                        new SqlParameter("@StockOut",model.StockOut),
                        new SqlParameter("@CP_Tire_ROF",model.Rof)
                    });
...
FROM    ( SELECT    T.* ,
          ...
          FROM      ( SELECT    VP.CP_Brand AS Brand ,
                                VP.PID ,
                                VP.DisplayName AS ProductName ,VP.OnSale,
                                ( ISNULL(( SELECT TOP 1
                                                    TPGP.Value
                                           FROM     Tuhu_productcatalog.dbo.tbl_TirePriceGuidePara
                                                    AS TPGP WITH ( NOLOCK )
                                ...
                                DPSD.CaigouZaitu,TMLPJ.Price as TMLPJPrice
                      FROM      Tuhu_productcatalog.dbo.vw_Products AS VP WITH ( NOLOCK )
                                LEFT JOIN Tuhu_bi.dbo.dm_Product_SalespredictData
                                AS DPSD WITH ( NOLOCK ) ON VP.PID = DPSD.PID
                                LEFT JOIN Tuhu_productcatalog..ProductPriceMapping
...
WHERE   ( @PCPricePer IS NULL
        ...
        /// <returns></returns>
        public static int InsrtCouponPriceHistory(CouponPriceHistory model)
        {
            string sql = @"INSERT Tuhu_log..tbl_CouponPriceHistory (PID,OldPrice,NewPrice,ChangeUser,ChangeReason) VALUES(@PID,@OldPrice,@NewPrice,@ChangeUser,@ChangeReason)";
            using (var dbHelper1 = new SqlDbHelper(ConnectionHelper.GetDecryptConn("Tuhu_log")))
            {
                return dbHelper1.ExecuteNonQuery(sql, CommandType.Text, new SqlParameter[] {
                            new SqlParameter("@PID",model.PID),
...
@WeekSaleCount,@MonthSaleCount,@GuidePrice,@JDPrice,@CarPrice,@TuhuPrice,@GrossProfit,@PoductName,@NowPrice,@NewPrice,@ApplyPerson,@ApplyReason)";
        ...
        /// </summary>
        /// <returns></returns>
        public static IEnumerable<QPLPriceApplyModel> GetQplPriceApplyList(int Type, string PID)
        {
            string sqlwhere = string.IsNullOrEmpty(PID) ? "  " : " AND vp.PID=@PID ";
            if (Type == 1)
            {
                sqlwhere += " and  qpl.ApplyStatus =0";
            }
            if (Type == 2)
            {
                sqlwhere += " and  qpl.ApplyStatus <> 0 ";
            ...
            if (Type == 3)
            {
                sqlwhere += " and qpl.ApplyStatus =1 ";
            }

            string sql = $@"SELECT 
                                CP_Brand AS Brand,
                                vp.PID,DisplayName ProductName ,
                                DPSD.cost StandPrice,
                                VP.cy_list_price AS TuHuPrice,
                                qpl.NowPrice,
                                qpl.OldPrice,
                                qpl.ApplyReason,
                                qpl.ApplyDateTime,
                                ...
                                '' ApplyStatusName,
                                qpl.CreateDateTime,
                                qpl.UpdateDateTime,qpl.Remark,qpl.PKID ApplyId
                                FROM Tuhu_productcatalog..vw_Products   vp WITH ( NOLOCK )
                                INNER JOIN Configuration..QPLPriceApply qpl WITH (NOLOCK) ON VP.PID = qpl.PID
                                LEFT JOIN Tuhu_bi..dm_Product_SalespredictData AS DPSD WITH ( NOLOCK ) ON VP.PID = DPSD.PID 
                                WHERE 1=1  {sqlwhere}
                                ORDER BY qpl.CreateDateTime desc";
            using (var dbHelper = new SqlDbHelper(ConnectionHelper.GetDecryptConn("Tuhu_Gungnir_BI")))
            {
                ...
                return dbHelper.ExecuteDataTable(sql, CommandType.Text, paras).ConvertTo<QPLPriceApplyModel>();
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static int AddQPLPriceApply(string Pid,decimal NowPrice,decimal OldPrice,string ApplyReason,string ApplyUser,int ApplyStatus, string Remark)
        {
            try
            {
                string sql = @"INSERT INTO Configuration..[QPLPriceApply]
                           ([PID]
                           ,[NowPrice]
                           ,[OldPrice]
                           ,[ApplyReason]
                ...
                using (var dbHelper = new SqlDbHelper(ConnectionHelper.GetDecryptConn("Gungnir")))
                {
                    var sqlParameter=new List<SqlParameter>()
                    {
                        new SqlParameter("@PID", Pid),
                        new SqlParameter("@NowPrice", NowPrice),
                        new SqlParameter("@OldPrice", OldPrice),
                    ...
                    return dbHelper.ExecuteNonQuery(sql, CommandType.Text, sqlParameter);
                }
        ...
        public static int UpdateQPLPriceApply(int PKID,int ApplyStatus,string Auditor)
        {
            try
            {
                var sql =@"UPDATE Configuration..[QPLPriceApply] SET ApplyStatus=@ApplyStatus,Auditor=@Auditor,UpdateDateTime=GETDATE()
                       WHERE PKID=@PKID";
                using (var dbHelper = new SqlDbHelper(ConnectionHelper.GetDecryptConn("Gungnir")))
                {
                    var sqlParameter = new List<SqlParameter>()
