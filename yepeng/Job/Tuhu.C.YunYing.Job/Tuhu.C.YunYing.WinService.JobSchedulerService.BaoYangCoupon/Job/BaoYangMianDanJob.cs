...
{
    /// <summary>保养免单券 huhengxing 2016-7-25 已过期</summary>
    [DisallowConcurrentExecution]
    public class BaoYangMianDanJob : IJob
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(BaoYangMianDanJob));

        public void Execute(IJobExecutionContext context)
        {
            try
            {
                if (DateTime.Now < new DateTime(2016, 7, 25))
                {
                    Logger.Info("2016-7-25开始发放保养免单券");
                    return;
                }
                if (DateTime.Now > new DateTime(2016, 9, 10))
                {
                    Logger.Info("2016-9-10后结束发放保养免单券");
                    return;
                }
                Logger.Info("启动任务");
...
WHERE	P.GetRuleID=500
        ...
        AND NOT EXISTS ( SELECT	1
						 ...
						 WHERE	PC.UserId = O.UserID
								...
								AND PC.CodeChannel = N'击穿低价保养免单券活动' )"))
                {
                    ...
                    foreach (var i in dic)
                    {
                        if (!string.IsNullOrWhiteSpace(i.SumMoney))
                        {
                            ...
                            if (result > 0)
                            {
                                BLL.Business.SendMarketingSms(i.UserTel, "途虎保养免单券", m, "保养现金券");
                            }
                        }
                    }
                }
        ...
        public static int CreateOrYzPromotion(string userId, int money, string orderID)
        {
            try
            {
                ...
                promotionCodeModel.Description = "保养免单（击穿低价）活动返券，限到店保养使用";
                ...
                promotionCodeModel.CodeChannel = "击穿低价保养免单券活动";
