...
export default {
                    ...
                    render: (h, params) => {
                        return h('Input', {
                            ...
                            on: {
                                ...
                                'on-blur': (event) => {
                                    this.data[params.index].PID = event.target.value;
                                    this.checkPidAndPrice(params.index)
                                }
                            }
                        })
                    }
    ...
    methods: {
                ...
                if (!_this.data.length) {
                    ...
                    if (_this.wrongMsg.length === 0) {
                        ...
                        _this.data.forEach(element => {
                            if (element.CostPrice === undefined || element.CostPrice === "") {
                                element.CostPrice = 0;
                            }
                            if (element.OriginalPrice === undefined || element.OriginalPrice === "") {
                                element.OriginalPrice = 0;
                            }
                        });
                        this.ajax.post('/Seckill/SpikeSave', {
        ...
        toApprovePass () {
            var _this = this;
            this.ajax.post('/Seckill/SpikeExamActivity', {
        ...
        toRefresh () {
            var _this = this;
            this.ajax.post('/Seckill/SpikeRefreshCache', {
                ...
                this.ajax.get("/Seckill/FetchProductWithCostPriceByPID", {
                    ...
                    .then(response => {
                        ...
                        if (returnVaule.length) {
                            ...
                            _this.checkPrice(index);
                        }                        
                    });
            }
        },
        checkPrice (index) {
            if (this.data[index].Error === undefined || this.data[index].Error === "") {
                ...
                if (originPrice > 0) {
                    if (parseFloat(price) <= 0 || isNaN(parseFloat(price)) || parseFloat(price) > parseFloat(originPrice)) {
                        this.data[index].Error = "秒杀价不能为0或者或者不能为空或者高于原价，请查看是否配置错误";
                    }
                    var degree = parseFloat(Math.abs(parseFloat(originPrice) - parseFloat(price)) / parseFloat(originPrice)).toFixed(2);
