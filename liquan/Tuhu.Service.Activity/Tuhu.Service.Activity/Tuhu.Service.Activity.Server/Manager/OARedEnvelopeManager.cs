...
namespace Tuhu.Service.Activity.Server.Manager
{
    ...
    public class OARedEnvelopeManager
    {
            ...
            OARedEnvelopeUserReceiveRequest request, OARedEnvelopeActivityInfoResponse activity)
        {
            ...
            if (conditionCarModelFlag == 1)
            {
                taskUserVehicle = GetUserVehicleAsync(userId);
                taskList.Add(taskUserVehicle);
            }

            if (conditionPriceFlag == 1 && activity.ConditionPrice > 0)
            {
        ...
        private static async Task<Tuple<bool, string, string>> ValidateOARedEnvelopeUserReceive(
            OARedEnvelopeUserReceiveRequest request, OARedEnvelopeActivityInfoResponse activityInfo,
            bool isVerify = false)
        {
            // 验证参数
            ...
            var validateOaRedEnvelopeUserReceiveDoneTask =
                ValidateOARedEnvelopeUserReceiveDoneAsync(request.UserId, request.OpenId, request.OfficialAccountType,
                    isVerify);
            ...
            OARedEnvelopeUserVerifyRequest request)
        {
            try
            {
                ...
                {
                    Logger.Info(
                        $"OARedEnvelopeManager -> OARedEnvelopeUserVerifyAsync -> {result.Item2} {result.Item3} {JsonConvert.SerializeObject(request)}");
                ...
                {
                    Logger.Info(
                        $"OARedEnvelopeManager -> OARedEnvelopeUserVerifyAsync -> {validateResult.Item2} {validateResult.Item3} {JsonConvert.SerializeObject(request)}");
            ...
            try
            {
                ...
                using (var zkLock = new ZooKeeperLock($"/OARedEnvelope/{request.OpenId}"))
                {
                    if (await zkLock.WaitAsync(20000)) //如果锁释放 会立即执行，正常逻辑不会等待5秒钟
                    {
                        ...
                        {
                            Logger.Info(
                                $"OARedEnvelopeManager -> OARedEnvelopeUserReceiveAsync -> {result.Item2} {result.Item3} {JsonConvert.SerializeObject(request)}");
                            return OperationResult.FromError<bool>(result.Item2, result.Item3);
                        }
                        ...
                        try
                        {
                            ...
                            {
                                Logger.Info(
                                    $"OARedEnvelopeManager -> OARedEnvelopeUserReceiveAsync ->  LockRedEnvelopAsync -> {lockResult.Item2} {lockResult.Item3} ->  {JsonConvert.SerializeObject(request)}");
                                //  回滚
                            ...
                            {
                                Logger.Info(
                                    $"OARedEnvelopeManager -> OARedEnvelopeUserReceiveAsync ->  SendRedEnvelopAsync -> {sendResult.Item2} {sendResult.Item3} ->  {JsonConvert.SerializeObject(request)}");

                                //  回滚
                                await oaRedEnvelopeUserReceiveManager.RollBackAsync();
                                return OperationResult.FromError<bool>(sendResult.Item2, sendResult.Item3);
                            }
                        }
                        catch (Exception e)
                        {
            ...
            OARedEnvelopeDailyDataInitRequest request)
        {
            ...
            try
            {
                using (var zkLock = new ZooKeeperLock($"{nameof(OARedEnvelopeDailyDataInitAsync)}New"))
                {
                    ...
                    var redEnvelopList = CalueHB(dayMaxMoney, oaRedEnvelopeSettingModel.PerMaxMoney,
                        ...
                        .ToList();
                    Logger.Info(
                        $"OARedEnvelopeDailyDataInitAsync -> OARedEnvelopeDailyDataInitAsync -> before SaveOARedEnvelopeObjectListAsync -> {redEnvelopList.Count} -> min:{redEnvelopList.Select(p => p.Money).Min()} -> max:{redEnvelopList.Select(p => p.Money).Max()} -> avg:{redEnvelopList.Select(p => p.Money).Average()}");
        ...
        /// </summary>
        /// <returns></returns>
        public static async Task<OperationResult<IDictionary<string, OARedEnvelopeObjectResponse>>>
            GetAllOARedEnvelopeDailyDataAsync(
                GetAllOARedEnvelopeDailyDataRequest request)
        {
            // 初始化
            var date = request.Date.Date;
            ...
            var result = await oaRedEnvelopeCacheManager.GetAllOARedEnvelopeObjectAsync();

            var returnResult =
                (IDictionary<string, OARedEnvelopeObjectResponse>)
                (new Dictionary<string, OARedEnvelopeObjectResponse>());

            if (result != null)
            {
                result = result.OrderBy(p => int.Parse(p.Key)).ToDictionary(p => p.Key, o => o.Value);
                foreach (var redEnvelopeObjectModel in result)
                {
                    returnResult[redEnvelopeObjectModel.Key] = new OARedEnvelopeObjectResponse()
                    {
                        Money = redEnvelopeObjectModel.Value?.Money ?? 0,
                        Position = redEnvelopeObjectModel.Value?.Position ?? 0
                    };
                }
            }

            return OperationResult.FromResult(returnResult);
        }
