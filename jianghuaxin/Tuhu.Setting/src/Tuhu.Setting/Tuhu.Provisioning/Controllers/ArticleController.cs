        ...
        public JsonResult AddArticleImg2()
        {
            ...
            if (Request.Files.Count > 0)
            {
                ...
                try
                {
                    ...
                    var client = new WcfClinet<IFileUpload>();
                    _BImage = swc.WebConfigurationManager.AppSettings["UploadDoMain_image"] + Guid.NewGuid().ToString() + ext;
                    _SImage = swc.WebConfigurationManager.AppSettings["UploadDoMain_image"] + Guid.NewGuid().ToString() + "_s" + ext;
                    var buffer = new byte[Imgfile.ContentLength];
                    Imgfile.InputStream.Read(buffer, 0, buffer.Length);

                    var result_1 = client.InvokeWcfClinet(w => w.UploadImage(_BImage, buffer, 0, 0));
            ...
            return Json(new
            {
                BImage = swc.WebConfigurationManager.AppSettings["DoMain_image"] + _BImage,
                SImage = swc.WebConfigurationManager.AppSettings["DoMain_image"] + _SImage,
        ...
        public JsonResult AddArticleImgForBeautyBanner()
        {
            ...
            if (Request.Files.Count > 0)
            {
                ...
                try
                {
                    ...
                    using (var image = Image.FromStream(Imgfile.InputStream))
                    {
                        if (image.Width * 1.0 / image.Height != 5.0 / 2)
                            return Json(new
                            {
                                BImage = "",
                                Msg = "图片宽高尺寸比例应为(5:2)"
