...
FROM Tuhu_groupon..BeautyServicePackageCode A WITH (NOLOCK)
    JOIN Tuhu_groupon..BeautyServicePackage B WITH (NOLOCK)
        ON A.PackageId = B.PKID
    LEFT JOIN Tuhu_groupon..MrCooperateUserConfig C WITH (NOLOCK)
        ON B.CooperateId = C.PKID
...
FROM Tuhu_groupon..BeautyServicePackageCode A WITH (NOLOCK)
    ...
    LEFT JOIN Tuhu_groupon..MrCooperateUserConfig C WITH (NOLOCK)
        ON B.CooperateId = C.PKID
WHERE A.UserId = @UserId
ORDER BY A.CreateTime DESC OFFSET (@PageIndex - 1) * @PageSize ROWS FETCH NEXT @PageSize ROWS ONLY;";
