<template>
    <div class="content-wrapper" ref="main">
        ...
        <!-- 编辑分类限购数量 end -->
        <!-- 编辑产品限购数量 start -->
        <Modal v-model="productModal.visible" :loading="productModal.loading" title="修改限购数量" okText="保存" @on-ok="submitProductLimitCarProduct">
        ...
        <!-- 编辑产品限购数量 end -->
        <!-- 批量编辑产品限购数量 start -->
        <Modal v-model="batchModal.visible" :loading="batchModal.loading" title="修改限购数量" okText="保存" @on-ok="submitBatchProductLimitCarProduct">
...
export default {
    ...
    methods: {
        ...
        getRootTree() {
            this.ajax
                .get("/ProductLimitChePin/GetCatrgoryTree?category=AutoProduct")
                .then(res => {
                    if (res.data.Success) {
                    ...
                    [
                        ...
                        h(
                            "a",
                            {
                                on: {
                                    click: () => {
                                        this.setProductLimitCarProductCnt(data);
                                    }
                                }
                            },
                            "[设置产品]"
        ...
        submitCategory() {
            ...
            this.ajax
                .post("/ProductLimitChePin/SaveCategoryLimitCount", {
                    ...param
                })
                .then(res => {
                    if (res.data.success) {
         ...
         * 修改产品限购
         */
        submitProductLimitCarProduct() {
            var that = this;
            let param = that.productModal.model;
            param.CategoryCode="AutoProduct";

            that.ajax
                .post("/ProductLimitChePin/SaveProductLimitCount", {
                    ...param
                })
                .then(res => {
                    if (res.data.success) {
        ...
        loadData() {
            ...
            this.ajax
                .post("/ProductLimitChePin/GetProductListByCagegoryCode", {
        ...
        loadBrandList(val) {
            this.ajax
                .post("/ProductLimitChePin/GetBrandsByCategory", {
                    category: val
                })
                .then(res => {
                    if (res.data.Success) {
         ...
         *  根据条件查询类目下产品数据
         */
        setProductLimitCarProductCnt(data) {
            this.categoryModal.model.CategoryLevel = data.level;
            this.categoryModal.model.CategoryCode = data.value;
            this.categoryModal.model.CategoryName = data.title;
        ...
        handleShowCategoryLog(category, level) {
            this.logmodal.loading = true;
            this.util.ajax
                .post(`/ProductLimitChePin/GetCommonConfigLogs`, {
                    category,
                    level
                })
                .then(res => {
         ...
         * 提交批量设置
         */
        submitBatchProductLimitCarProduct() {
            ...
            this.selectModel.forEach(item => {
                (item.LimitCount = this.batchModal.model.LimitCount),
                    (item.CategoryName = this.categoryModal.model.CategoryName);
                item.CategoryCode = 'AutoProduct';
                item.CategoryLevel = this.categoryModal.model.CategoryLevel;
            });

            this.ajax
                .post("/ProductLimitChePin/BatchSaveProductLimitCount", {
         ...
         */
        refreshCache() {
            this.util.ajax.post("/ProductLimitChePin/RefreshCache").then(res => {
                if (res.data.success) {
