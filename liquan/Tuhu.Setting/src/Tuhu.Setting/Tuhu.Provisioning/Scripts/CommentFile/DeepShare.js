...
var APPShare = {
    ...
    Init: function () {
        var source=APPShare.GetQueryString("source");
        //判断参数
        if (source != "" && source != undefined) {
            ...
            $(".foot-top").css("margin-bottom", "3px");
            //相关阅读链接加source
            $("#xgyd").find(".relateds").each(function () {
                var thisObj = $(this);
                var link = thisObj.attr("href") + "&source=" + source;
            ...
            var articleUrl = location.href.split("?")[0] + "?id=" + articleId;
            var data = {};
            data.Type = 2;
            data.URL = articleUrl;
