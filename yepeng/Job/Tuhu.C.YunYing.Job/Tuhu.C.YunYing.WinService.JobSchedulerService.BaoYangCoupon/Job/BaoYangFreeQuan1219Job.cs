...
namespace Tuhu.C.YunYing.WinService.JobSchedulerService.BaoYangCoupon.Job
{
    ...
    public class BaoYangFreeQuan1219Job : IJob
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(BaoYangFreeQuan1219Job));
        ...
        public void Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                Logger.Info("启动任务" + id);
                var dt = DalCoupon.Get1219BaoYangFreeOrderList();
                        ...
                        try
                        {
                            if (items != null && items.Any())
                            {
                                ...
                                if (result > 0)
                                {
                                    ...
                                    BLL.Business.SendMarketingSms(userTel, "途虎养车保养买一送一");
                                }
                            }
                            else
                            {
        ...
        public static int CreateOrYzPromotion(string userId, int money, string orderID)
        {
            try
            {
                ...
                promotionCodeModel.CodeChannel = "保养买一送一返券活动";
                promotionCodeModel.BatchId = Convert.ToInt32(orderID);
                promotionCodeModel.RuleId = 23;
                promotionCodeModel.Number = 1;
                var result = DalCoupon.CreatePromotionCode(promotionCodeModel, null, "BaoYangFreeQuan1219Job");
                return result;
            }
            catch (Exception ex)
            {
