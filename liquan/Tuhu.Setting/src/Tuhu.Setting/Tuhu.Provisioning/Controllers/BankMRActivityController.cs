    ...
    public class BankMRActivityController : Controller
    {
        ...
        public ActionResult UpsertMrCooperateUserConfig(MrCooperateUserConfig config)
        {
            ...
            if (config != null)
            {
                ...
                if (config.PKID > 0)
                {
                    result = manager.UpdateMrCooperateUserConfig(config, user);
                }
                else
                {
                    config.CreateUser = user;
                    result = manager.InsertMrCooperateUserConfig(config, user);
                }
            }
        ...
        /// <param name="companyId"></param>
        /// <returns></returns>
        public ActionResult GetAllCompanyUsersByCompanyId(int companyId)
        {
            List<CompanyUserModel> result = new List<CompanyUserModel>();
            var allUserInfo = UserAccountService.GetCompanyUsersByCompanyId(companyId);
            if (allUserInfo != null && allUserInfo.Any())
            {
                foreach (var user in allUserInfo)
                {
                    var companyInfo = UserAccountService.SelectCompanyInfoById(user.CompanyId);
                    result.Add(new CompanyUserModel
                    {
                        UserId = user.UserId,
                        UserMobile = user.UserMobile,
                        CompanyId = user.CompanyId,
                        CompanyName = companyInfo?.FirstOrDefault()?.Name,
                        UserName = user.UserName,
                        IsParent = companyInfo?.FirstOrDefault()?.ParentId > 0 ? false : true
                    });
                }
            }
            return Json(result, JsonRequestBehavior.AllowGet);
        ...
        public FileResult GetImportBankMRActivityUsers(string batchCode,string otherVerifyName)
        {
            ...
            int index2 = 1;
            temp.Value.Value.ToArray()?.ToList().ForEach(f =>
            {
        ...
        public JsonResult GetImportBankMRActivityUsersRecords(string activityId, int pageIndex, int pageSize)
        {
            ...
            result?.Where(w => !string.IsNullOrWhiteSpace(w.BatchCode)).GroupBy(g => g.BatchCode).ToDictionary(x => x.Key, v => v)?.ToList().ForEach(f =>
              {
