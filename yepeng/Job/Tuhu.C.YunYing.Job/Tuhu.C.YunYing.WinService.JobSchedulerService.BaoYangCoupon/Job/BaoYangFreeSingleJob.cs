...
{
    /// <summary>保养免单券 huhengxing 2016-9-18 (长期)</summary>
    [DisallowConcurrentExecution]
    public class BaoYangFreeSingleJob : IJob
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(BaoYangFreeSingleJob));

        public void Execute(IJobExecutionContext context)
        {
            try
            {
                if (DateTime.Now < new DateTime(2016, 9, 20))
                {
                    Logger.Info("2016-9-20开始发放胜牌海湾机油免单券");
                    return;
                }
...
WHERE	P.GetRuleID=814
        ...
        AND NOT EXISTS ( SELECT	1
						 ...
						 WHERE	PC.UserId = O.UserID
								...
								AND PC.CodeChannel = N'胜牌海湾机油免单券活动' )"))
                {
                        ...
                        if (!string.IsNullOrWhiteSpace(i.SumMoney))
                        {
                            ...
                            try
                            {
                                ...
                                if (result > 0)
                                {
                                    ...
                                    BLL.Business.SendMarketingSms(userTel, "途虎养车网胜牌&海湾机油免单券", m, "保养现金券");
                                }
                            }
                            catch (Exception ex)
                            {
        ...
        public static int CreateOrYzPromotion(string userId, int money, string orderID)
        {
            try
            {
                ...
                promotionCodeModel.Description = "胜牌/海湾机油免单活动返券，限到店保养使用";
                ...
                promotionCodeModel.CodeChannel = "胜牌海湾机油免单券活动";
