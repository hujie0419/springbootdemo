...
";
        ...
        /// <returns></returns>
        public static async Task<int> AddEnrollInfo(TEnrollInfoModel infomodel)
        {
            using (var cmd = new SqlCommand(@"
         INSERT INTO Activity..T_EnrollInfo
        ( UserName ,
          Tel ,
          Area ,
		  EnrollStatus,
...
VALUES  ( @UserName ,
          ...
          @ActivityId
            ...
            {
                cmd.Parameters.AddWithValue("@UserName", infomodel.UserName);
                cmd.Parameters.AddWithValue("@Tel", infomodel.Tel);
                cmd.Parameters.AddWithValue("@Area", infomodel.Area);
                cmd.Parameters.AddWithValue("@EnrollStatus", infomodel.EnrollStatus);
        ...
        /// <param name="infomodel"></param>
        /// <returns></returns>
        public static async Task<int> UpdateEnrollInfo(TEnrollInfoModel infomodel)
        {
            using (var cmd = new SqlCommand("UPDATE	Activity..T_EnrollInfo WITH(ROWLOCK) SET UserName = @UserName,Tel =@Tel,Area=@Area,EnrollStatus=@EnrollStatus,UpdateTime=@UpdateTime WHERE Id = @Id; "))
            {
                    cmd.Parameters.AddWithValue("@UserName", infomodel.UserName);
                    cmd.Parameters.AddWithValue("@Tel", infomodel.Tel);
                    cmd.Parameters.AddWithValue("@Area", infomodel.Area);
                    ...
                    cmd.Parameters.AddWithValue("@Id", infomodel.Id);
                return await DbHelper.ExecuteNonQueryAsync(cmd);
            }
        }
        ...
        /// <returns></returns>
        public static async Task<int> UpdateUserEnrollStatus(string area)
        {
            using (var cmd = new SqlCommand("UPDATE	Activity..T_EnrollInfo WITH(ROWLOCK) SET EnrollStatus=@EnrollStatus,UpdateTime=GETDATE() WHERE Area = @Area; "))
            {
                cmd.Parameters.AddWithValue("@EnrollStatus", 1);
        ...
        {
            string sql = @"SELECT TOP 1 *  FROM [Activity].[dbo].T_EnrollInfo WITH(NOLOCK) WHERE Id = @id";
                using (var cmd = new SqlCommand(sql))
                {
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.AddWithValue("@id", id);
                    return (await DbHelper.ExecuteSelectAsync<TEnrollInfoModel>(cmd)).FirstOrDefault();
                }
        }
        ...
        {
            string sql = @"SELECT TOP 1 *  FROM [Activity].[dbo].T_EnrollInfo WITH(NOLOCK) WHERE Tel = @tel";
            using (var cmd = new SqlCommand(sql))
            {
                cmd.CommandType = CommandType.Text;
        ...
        {
            var sql = @"SELECT  EI.Id, UserName, Tel, Area, EnrollStatus, CreatTime, UpdateTime,ActivityId,At.ActivityName as ActivityName
FROM [Activity].[dbo].T_EnrollInfo as EI with (nolock)  left join  Activity.[dbo].T_Activity as At WITH ( NOLOCK )  on EI.ActivityId=At.Id where 1=1";

            if (!string.IsNullOrWhiteSpace(area))
            {
                sql += " and Area= @Area";
            }
            sql += " order by CreatTime  desc OFFSET (@pageIndex -1) * @pageSize ROWS FETCH NEXT @pageSize ROWS ONLY";

            var sqlCount = @"SELECT  COUNT(*)
FROM [Activity].[dbo]. T_EnrollInfo as EI with (nolock) where 1=1";
            if (!string.IsNullOrWhiteSpace(area))
            {
                sqlCount += " and Area= @Area";
            }

            var result = new PagedModel<TEnrollInfoModel>();
            using (var cmd = new SqlCommand(sql))
            {
                if (!string.IsNullOrWhiteSpace(area))
                {
                ...
                var list = await DbHelper.ExecuteSelectAsync<TEnrollInfoModel>(true, cmd);
                result.Source = list;
            }
            using (var cmd = new SqlCommand(sqlCount))
            {
