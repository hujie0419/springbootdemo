...
function validateAddCouponsType() {
    ...
    var PromotionType = $('#UseMoney').attr('PromotionType');
    if (PromotionType!=3) {
...
$(document.forms["PromotionForm"]).validate({
    ...
    submitHandler: function (form) {
        ...
        debugger
        var PromotionType  = $("[id=UseMoney]").attr("PromotionType");
    ...
    $.ajax({
        ...
        success: function (jsonObj) {
            if (jsonObj.IsSuccess) {
                ...
                debugger
                if (jsonObj.ObjectData.PromotionType == 3) {
                    $("#lblUseMoney").hide();
                    $("#lblUseMoney").prev().hide();
                }
                else {
                    $("#lblUseMoney").show();
                    $("#lblUseMoney").prev().show();
                }
