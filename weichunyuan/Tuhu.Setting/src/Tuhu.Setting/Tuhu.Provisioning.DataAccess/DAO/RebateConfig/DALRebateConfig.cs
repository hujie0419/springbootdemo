...
namespace Tuhu.Provisioning.DataAccess.DAO.RebateConfig
{
    public static class DALRebateConfig
    {
        public static List<RebateConfigModel> GetRebateConfig(SqlConnection conn, Status status, string orderId, string phone,
            ...
            string principalPerson, string rebateMoney,string source, int pageIndex, int pageSize)
        {
            const string sql = @"
            SELECT  rac.PKID ,
            rac.OrderId ,
    ...
    ORDER BY rac.PKID DESC
        ...
        public static int InsertRebateApplyPageConfig(SqlConnection conn, string backgroundImg, string activityRules, string user)
        {
            const string sql = @"
