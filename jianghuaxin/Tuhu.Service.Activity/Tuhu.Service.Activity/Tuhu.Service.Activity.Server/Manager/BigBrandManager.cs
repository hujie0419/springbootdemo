...
namespace Tuhu.Service.Activity.Server.Manager
{
    ...
    {
        private static readonly string DefaultClientName = "BigBrandRewardService";
        ...
        public static async Task<BigBrandRewardListModel> GetBigBrand(string keyValue)
        {
            ...
            {
                var result = await client.GetOrSetAsync<BigBrandRewardListModel>(keyValue, async () => await GetEntity(keyValue));
                if (result.Success)
                    return result.Value;
                else
    ...
    public class BigBrandLogic
    {
        ...
        public BigBrandLogic(string keyValue, Guid userId, string phone, string deviceId, string channel,string openId=default(string))
        {
            ...
            if (entity.PeriodType != 1 && entity.PeriodType != 2 && entity.PeriodType != 3)
                throw new Exception("获取领取记录的周期类型不符合配置");
            ...
            if (DateTime.Now < this.entity.StartDateTime)
                throw new Exception("不在抽奖时间范围内，不能抽奖");
            ...
            {
                int month = (DateTime.Now.Year - this.entity.StartDateTime.Value.Year) * 12 + (DateTime.Now.Month - this.entity.StartDateTime.Value.Month);
                this.StartDateTime = Convert.ToDateTime(this.entity.StartDateTime.Value.AddMonths(month).ToString("yyyy-MM-dd HH:mm:ss"));
                this.EndDateTime = Convert.ToDateTime(this.StartDateTime.AddMonths(this.entity.Period.Value).ToString("yyyy-MM-dd HH:mm:ss"));
                while (DateTime.Now.Date < this.StartDateTime.Date && this.entity.Period.Value > 0)
                {
                    this.StartDateTime = this.StartDateTime.AddMonths(-this.entity.Period.Value);
                    this.EndDateTime = this.EndDateTime.AddMonths(-this.entity.Period.Value);
                }
            }
        }
