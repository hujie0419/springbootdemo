...
namespace Tuhu.Provisioning.Business.ProductInfomation
{
    public class CarProductPriceManager
    {
        ...
        static async Task<List<CarProductPriceModel>> FullPriceData(List<string> pids, SqlConnection conn)
        {
            ...
            var activityPrices = await DalCarProductPrice.GetActivityPricesByPids(conn, pids);
            ...
            foreach (var item in list)
            {
                ...
                var tempCarigouPrices = carigouPrices?.Where(x => x.PID == item.PID).FirstOrDefault();
                //采购相关价格信息
                item.PurchasePrice = tempCarigouPrices?.PurchasePrice ?? 0;
                item.ContractPrice = tempCarigouPrices?.ContractPrice ?? 0;
                item.OfferPurchasePrice = tempCarigouPrices?.OfferPurchasePrice ?? 0;
                item.OfferContractPrice = tempCarigouPrices?.OfferContractPrice ?? 0;
