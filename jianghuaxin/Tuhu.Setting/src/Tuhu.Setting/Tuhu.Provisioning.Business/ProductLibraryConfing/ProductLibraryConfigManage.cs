...
namespace Tuhu.Provisioning.Business.ProductLibraryConfing
{
    public class ProductLibraryConfigManage
    {
        ...
        {
            List<QueryProductsModel> result;
            try
            {
                System.Diagnostics.Stopwatch watcher = new System.Diagnostics.Stopwatch();
                result = handler.QueryProductsForProductLibrary(model).ToList();
                if (result.Any())
                {
                    //改为循环一次把该做的事情做完，因为是逻辑分页，这一个list有几万条数据，频繁循环损耗太大
                    var pids = new List<string>(result.Count);
                    var rulePkIds = new Dictionary<int, bool>(result.Count);
                    result.ForEach(r =>
                    {
                        pids.Add(r.PID);
                        r.CouponIds?.Split(',').ForEach(t =>
                        {
