...
namespace Tuhu.Service.Activity.Server.Manager
{
    public static class ActivityManager
    {
        ...
        private static async Task<ActivePageListModel> ActivePageListModelAssembly(ActivePageListModel activePageList, ActivtyPageRequest request)
        {
            ...
            if (allPids.Any())
            {
                var products = (await SelectSimpleProductModels(request.HashKey, allPids)).ToList();
                //比较手动配置的产品跟推荐产品有重复的时候去重

                Guid[] activityGuid = pageContents.Where(o => o.ActivityId.HasValue && (o.Type != (int)ActivityPageContentType.ProductPool && o.Type != (int)ActivityPageContentType.NewProductPool)).Select(r => r.ActivityId.Value).Distinct().ToArray();
        ...
        public static async Task<Models.ResultModel<bool>> InsertRebateApplyRecordNewAsync(RebateApplyRequest request)
        {
            ...
            if (!orderType.Contains("轮胎") && !orderType.Contains("保养") && !orderType.Contains("蓄电池") && !orderType.Contains("服务订单"))
            {
                result.Code = 1004;
                result.Msg = "本活动仅限轮胎订单、保养订单、蓄电池订单或钣喷服务订单";
                return result;
            }
            ...
            else
            {
                ...
                {
                    if (order.Items?.Where(x => String.Equals(x.Category, "PQFW"))?.Count() <= 0)
                    {
                        result.Code = 1011;
...
(1, "成功"), cacheresult.Value.OrderBy(v => v.Value).Select(key => key.Key).ToList());
        ...
        /// 分页获取报名参加活动的用户列表
        /// </summary>
        public static async Task<PagedModel<UserRegistrationResponse>> GetUserRegistrationsByAreaAsync(string area, int pageSize = 20, int pageIndex = 1)
        {
            using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                var result = await client.GetOrSetAsync($"UserRegistration/{area}/{pageIndex}-{pageSize}",
                    () => DalActivity.GetUserRegistrationsByAreaAsync(area, pageSize, pageIndex), TimeSpan.FromHours(2));

                PagedModel<UserRegistrationResponse> userRegistrations;

                if (result.Success)
                {
                    userRegistrations = result.Value;
                }
                else
                {
                    Logger.Warn($"GetUserRegistrationsByArea:UserRegistration/{area}/{pageIndex}-{pageSize};Error:{result.Message}", result.Exception);
                    // redis查询失败查数据库
                    userRegistrations = await DalActivity.GetUserRegistrationsByAreaAsync(area, pageSize, pageIndex);
                }
        ...
        /// 获取用户活动列表
        /// </summary>
        public static async Task<List<UserActivityResponse>> GetUserActivitiesAsync()
        {
            using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                var result = await client.GetOrSetAsync($"UserActivities",
                    () => DalActivity.GetUserActivitiesAsync(), TimeSpan.FromHours(12));

                List<UserActivityResponse> userActivities;

                if (result.Success)
                {
                    userActivities = result.Value;
                }
                else
                {
                    Logger.Warn($"GetUserActivities:UserActivities;Error:{result.Message}", result.Exception);
                    // redis查询失败查数据库
                    userActivities = await DalActivity.GetUserActivitiesAsync();
                }
        ...
        /// 获取用户报名信息
        /// </summary>
        public static async Task<UserRegistrationResponse> GetUserRegistrationByIdAsync(long pkid)
        {
            return await DalActivity.GetUserRegistrationByIdAsync(pkid);
        }

        /// <summary>
        /// 新增用户报名信息
        /// </summary>
        public static async Task<bool> AddUserRegistrationAsync(UserRegistrationRequest request)
        {
            int count = await DalActivity.AddUserRegistrationAsync(request);
            if (count < 1) return false;

            using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                // TODO: 为了避免混淆，缓存Key需要统一管理
                await client.RemoveAsync("UserRegistration//1-10");
                await client.RemoveAsync($"UserRegistration/{request.Area}/1-10");
                return true;
            }
        }
        ...
        /// 修改用户报名信息
        /// </summary>
        public static async Task<bool> ModifyUserRegistrationAsync(UserRegistrationRequest request)
        {
            var oldArea = (await DalActivity.GetUserRegistrationByIdAsync(request.PKID))?.Area;
            int count = await DalActivity.ModifyUserRegistrationAsync(request);
            if (count < 1) return false;

            using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                await client.RemoveAsync("UserRegistration//1-10");
                await client.RemoveAsync($"UserRegistration/{oldArea}/1-10");
                await client.RemoveAsync($"UserRegistration/{request.Area}/1-10");
                return true;
            }
        }
        ...
        /// <param name="status">报名状态</param>
        public static async Task<bool> UpdateUserRegistrationStatusAsync(string area, RegistrationStatus status)
        {
            int count = await DalActivity.UpdateUserRegistrationStatusAsync(area, status);
            if (count < 1) return false;

            using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                await client.RemoveAsync("UserRegistration//1-10");
                await client.RemoveAsync($"UserRegistration/{area}/1-10");
                return true;
            }
        }
    }
