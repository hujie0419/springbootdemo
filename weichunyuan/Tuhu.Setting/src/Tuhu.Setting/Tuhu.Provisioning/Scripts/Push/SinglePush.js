        ...
        methods: {
            ...
            PreModifyTemplate: function (template) {
                ...
                if (template.DeviceType == 4) {
                    ...
                    if (vm.WeiXinTemplates.length == 0&& template.IsUseOtherUser == true) {
                        vm.CanSave = false;
                        vm.ErrorMessage = "当前微信公众号模板不可用，不可选择 其余用户";
                        return template;
                    }

                    if (vm.WeiXinTemplates.length == 0 && template.IsUseCurrentActiveUser == true && template.CurrentActiveUserOptionItem == 1 ) {
                        vm.CanSave = false;
                    ...
                    if (template.IsUseCurrentActiveUser == true && template.CurrentActiveUserOptionItem != 1 && template.WeixinMediaId != "" && vm.WeiXinTemplates.length == 0) {
                        template.Title = "微信图文消息-" + vm.WeixinMediaId;
                        return template;
                    }
                  
                    //勾选了近期积极互动用户 + 选择了图文消息+ 没有填写微信图文消息
                    if (template.IsUseCurrentActiveUser == true && template.CurrentActiveUserOptionItem != 1 && template.WeixinMediaId == "") {
                        vm.CanSave = false;
