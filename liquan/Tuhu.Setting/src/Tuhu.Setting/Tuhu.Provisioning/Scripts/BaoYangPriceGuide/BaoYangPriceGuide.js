...
var Func = {
    ...
    UpdatePrice: function (pid, guidePrice, price, upperLimit, lowerLimit, marketingPrice, qplPrice) {
            ...
            buttons: {
                "保存": function () {
                    ...
                    if (isNaN(price_t)) {
                        ...
                        if (price_t < qplPrice) {
                            alert("该价格小于汽配龙价格,请先修改汽配龙价格");
                            return false;
                        }
                        //if (result > 0) {
                        if (price > 0) {
                            if (guidePrice == '-') {
                                Func.ApplyUpdatePrice(dialog, pid, price_t, "Tuhu");
                            }
                            else {
    ...
    UpdateQPLPrice: function (pid, guidePrice, price, upperLimit, lowerLimit, marketingPrice, qplPrice, cost) {
        $("#PriceHistory").attr("href", "/Tire/CouponPriceChangeLog?pid=" + pid);
        $("#UpdateQPLPrice .price").text(price + '元');
        $("#UpdateQPLPrice .costPrice").text(cost + '元');
        if (qplPrice === "") {
            qplPrice = "0.00";
        }
        $("#UpdateQPLPrice .QPLprice").val(qplPrice); 
        $("#UpdateQPLPrice .jyprice").text(cost+"-" + price); 
        $("#UpdateQPLPrice").dialog({
            title: "修改汽配龙价格",
            width: 300,
            height: 300,
            modal: true,
            buttons: {
                "保存": function () {
                    var dialog = $(this);
                    var price_t = dialog.find(".QPLprice").val();
                    if (isNaN(price_t)) {
                        alert("价格格式有误");
                        return false;
                    }
                        ...
                        if (price_t > 0) {
                            if (price_t >= parseFloat(cost) && price_t <= price) {
                                Func.QPLPrice(dialog, pid, qplPrice, price_t);
                                //进入修改
                            } else if (price_t < parseFloat(cost)) {
                                //审核
                                Func.ApplyUpdatePrice(dialog, pid, price_t, "QPLPrice");
                            }
                        }  
                    }
                },
    ...
    QPLPrice: function (dialog, pid, originalPrice,price) {
        $.post("UpdateQPLPrice", { pid: pid, price: price, originalPrice: parseFloat(originalPrice) }, function (data) {
            if (data.Status) {
                alert("修改成功");
                dialog.dialog("close");
    ...
    ApplyUpdatePrice: function (dialogold, pid, price, type="") {
        ...
        $("#ApplyUpdatePrice").dialog({
            ...
            buttons: {
                "保存": function () {
                    ...
                    $.post("ApplyUpdatePrice", { PID: pid, ApplyPrice: price, ApplyReason: reason, Type: type }, function (result) {
                        if (result > 0) {
                            alert("申请成功");
                            dialog.dialog("close");
