...
ORDER BY log.PKID DESC
        ...
        public static IEnumerable<ProductPriorityRegular> GetProductPriorityRegularByPartName(SqlConnection conn, IEnumerable<string> partNames)
        {
            const string sql = @"WITH partNames
                                    AS (SELECT  Item
                                        FROM    BaoYang..SplitString(@PartNames, N',', 1) )
                                    SELECT  a.PartName ,
                                            a.Brand ,
                                            a.Series ,
                                            a.Seq as Priority
                                    FROM    BaoYang..ProductPriority_Regular AS a WITH (NOLOCK)
                                    WHERE   EXISTS (
                                        SELECT  1 FROM  partNames AS p WHERE a.PartName = p.Item
                                    );";
            var parameters = new SqlParameter[]
            {
            ...
            var dt = SqlHelper.ExecuteDataTable(conn, CommandType.Text, sql, parameters);
            return dt.ConvertTo<ProductPriorityRegular>();
        }
...
VALUES  ( @PartName ,
          ...
          GETDATE()
            ...
            var parameters = new SqlParameter[]
            {
                new SqlParameter("@PartName", setting.PartName),
                new SqlParameter("@PriorityType", setting.PriorityType),
                new SqlParameter("@Brand", setting.Brand ?? string.Empty),
                new SqlParameter("@Series", setting.Series ?? string.Empty),
                new SqlParameter("@PID", setting.PID),
                new SqlParameter("@Priority", setting.Priority),
                new SqlParameter("@IsEnabled", setting.IsEnabled),
            };
        ...
        public static int InsertProductPriorityRegular(SqlConnection conn, BaoYangProductPriorityModel model,string partName)
        {
            const string sql = @"INSERT  INTO BaoYang..ProductPriority_Regular
        ( PartName ,
          Brand ,
          Series ,
          Seq ,
...
VALUES  ( @PartName ,
        ...
        /// <param name="conn"></param>
        /// <returns></returns>
        public static IEnumerable<OilBaoYangProductPriority> GetOilBaoYangProductPriority(SqlConnection conn)
        {
            const string sql = @" SELECT  o.Viscosity ,
                                                o.Brand ,
                                                o.Series ,
                                                o.Seq as Priority ,
                                                o.Grade
                                        FROM    BaoYang.dbo.ProductPriority_Regular_Oil AS o WITH (NOLOCK);";
