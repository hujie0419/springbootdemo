...
namespace Tuhu.Provisioning.Business.BaoYangPriceGuide
{
    public class BaoYangPriceGuideManager
    {
        ...
        public Tuple<bool, List<BaoYangPriceGuideList>> SelectBaoYangPriceGuide(BaoYangPriceSelectModel param, PagerModel pager)
        {
            ...
            try
            {
                ...
                Func<BaoYangPriceGuideList, bool> func = (list) =>
                {
                    ...
                    //勾选是否显示
                    if (param.IsShow != null) {
                        filter = filter && list.IsShow == param.IsShow.Value;
                    }
                    //勾选价格对比
        ...
        /// 修改汽配龙价格
        /// </summary>
        /// <param name="dic">key = pid value = price</param>
        /// <param name="userName"></param>
        /// <returns></returns>
        public async Task<string> UpdateQPLProductPriceByPidAsync(Dictionary<string, decimal> dic, string userName)
        {
            ...
            try
            {
                if (dic != null && dic.Any())
                {
                    List<T_ProductModel> items = new List<T_ProductModel>();
                    foreach (var item in dic)
                    {
                        items.Add(new T_ProductModel() { Price = item.Value, UserSKU = item.Key });
                    }
                    var serviceResult = ServiceHelper<ITuHuTaskService>.Invoke(x => x.UpdateUserProductPreiceByUseridAndSkuList(string.Empty, items, userName));
                    if (serviceResult.Success)
                    {
                        var isSuccess = (await serviceResult.Result)?.Success;
            ...
            {
                Logger.Log(Level.Error, ex, "UpdateQPLProductPriceByPid");
            }
            return result;
        }
        ...
        /// <returns></returns>
        public decimal GetQPLPriceBypid(string pid)
        {
            decimal result = 0;
            try
            {
                var pidData = ServiceHelper<ITuHuTaskService>.Invoke(x => x.GetUserProductPreiceByUseridAndSkuList("970d5eabb66c42d0aeaf4b360c42e451", new List<string> { pid }));
                if (pidData.Success && pidData.Result.Result != null)
                {
                    result = pidData.Result.Result.Data?.FirstOrDefault().Price ?? result;
                }
            }
            catch (Exception ex)
            {

                Logger.Log(Level.Error, ex, "GetQPLPriceBypid");
            }
            return result;
        }
        ...
        public Tuple<bool, List<Product_SalespredictData>> SelectStock(IEnumerable<string> pids)
        {
            var result = new List<Product_SalespredictData>();
            var flag = false;
            try
            {
                result = DalBaoYangPriceGuide.SelectProductSalespredictData(pids);
                flag = true;
            }
            catch (Exception ex)
            {
        ...
        public static IEnumerable<PriceUpdateAuditModel> SelectAuditLogByPID(string pID, PagerModel pager, string type) => DalBaoYangPriceGuide.SelectAuditLogByPID(pID, pager, type);
