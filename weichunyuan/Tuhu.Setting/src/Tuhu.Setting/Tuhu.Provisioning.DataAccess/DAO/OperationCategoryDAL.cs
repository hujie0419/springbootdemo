...
{
    public class OperationCategoryDAL
    {
        public static IEnumerable<OperationCategoryModel> Select(SqlConnection connection, int? id, int? parentId, int? type)
        {
            ...
            {
                string sql = "SELECT * FROM [Activity].[dbo].[tbl_OperationCategory] WITH(NOLOCK) ORDER BY sort";
            ...
            {
                string sql = "SELECT * FROM [Activity].[dbo].[tbl_OperationCategory] WITH(NOLOCK) WHERE ParentId = @parentId ORDER BY sort";
            ...
            {
                string sql = "SELECT * FROM [Activity].[dbo].[tbl_OperationCategory] WITH(NOLOCK) WHERE Type = @type ORDER BY sort";
            ...
            {
                string sql = "SELECT * FROM [Activity].[dbo].[tbl_OperationCategory] WITH(NOLOCK) WHERE Id = @id ORDER BY sort";
        ...
        {
            string sql = @"
                INSERT INTO [Activity].[dbo].[tbl_OperationCategory]
                           ([ParentId]
                           ,[CategoryCode]
                           ,[DisplayName]
                           ,[Icon]
            ...
            return SqlHelper.ExecuteNonQuery(connection, CommandType.Text, sql, sqlparams) > 0 ? true : false;
        }

        public static bool Update(SqlConnection connection, OperationCategoryModel model)
        {
            string sql = @"
                    UPDATE [Activity].[dbo].[tbl_OperationCategory]
        ...
        public static bool Delete(SqlConnection connection, int id)
        {
            string sql = @"
                ...
                with childTable as(
                select * from [Activity].[dbo].[tbl_OperationCategory] WITH(NOLOCK) where ParentId = @Id
                UNION ALL
                select op.* from [Activity].[dbo].[tbl_OperationCategory] op WITH(NOLOCK) JOIN childTable on op.ParentId=childTable.id
                ...
                ELSE
	                BEGIN
		                DELETE [Activity].[dbo].[tbl_OperationCategory] WHERE id = @Id
		                DELETE [Activity].[dbo].[tbl_OperationCategory_Products] WHERE OId = @Id
		                SELECT 1 AS 'result'
	                END
            ...
            return (int)SqlHelper.ExecuteScalar(connection, CommandType.Text, sql, sqlparams) > 0 ? true : false;
        }
        ...
        {
            string sql = " SELECT * FROM [Activity].[dbo].[tbl_OperationCategory_Products] WITH(NOLOCK) WHERE OId = @OId ORDER BY Sort";
        ...
        public static bool UpdateOperationCategoryProducts(SqlConnection connection, int oid, int type, List<OperationCategoryProductsModel> lists)
        {
            ...
            string templateForDel = "DELETE FROM [Activity].[dbo].[tbl_OperationCategory_Products] WHERE OId = @OId";
            string templateForIns = @"INSERT INTO [Activity].[dbo].[tbl_OperationCategory_Products]
            ...
            return SqlHelper.ExecuteNonQuery(connection, CommandType.Text, sbSql.ToString(), listParas.ToArray()) > 0 ? true : false;
        }
        ...
        {
            string sql = " SELECT oid AS 'id',ParentOid AS 'pId',DisplayName AS 'name',0 AS 'open',0 AS 'chkDisabled',CategoryName,NodeNo FROM [Tuhu_productcatalog]..vw_ProductCategories WITH(NOLOCK) ";
        ...
        {
            string sql = string.Format("SELECT CategoryName FROM [Tuhu_productcatalog]..vw_ProductCategories WITH(NOLOCK) WHERE oid IN({0})", string.Join(",", oids));
        ...
        public static IEnumerable<VW_ProductsModel> SelectProductCategories(SqlConnection connection, IEnumerable<string> tags, bool isOid = false)
        {
            ...
            if (isOid)
            {
                sql = string.Format(@"SELECT oid,PID,DisplayName,OnSale,stockout,cy_list_price,cy_marketing_price,CP_Brand,CP_Place,CP_Tab,TireSize,Image 
                                         FROM[Tuhu_productcatalog].[dbo].[vw_Products] WITH(NOLOCK)
                                         WHERE oid IN({0})", string.Join(",", tags));
            }
            else {
                sql = string.Format(@"SELECT oid,PID,DisplayName,OnSale,stockout,cy_list_price,cy_marketing_price,CP_Brand,CP_Place,CP_Tab,TireSize,Image 
                                         FROM[Tuhu_productcatalog].[dbo].[vw_Products] WITH(NOLOCK)
                                         ...
                                         (
                                             SELECT CategoryName FROM[Tuhu_productcatalog]..vw_ProductCategories WITH(NOLOCK) WHERE oid IN({0})
                                         ) OR Category IN
                                         (
                                             SELECT CategoryName FROM[Tuhu_productcatalog]..vw_ProductCategories WITH(NOLOCK) WHERE oid IN({0})
                                         )", string.Join(",", tags));
        ...
        public static IEnumerable<VW_ProductsModel> FetchPrProductsModelByProductIds(IEnumerable<string> pids)
        {
            ...
            sql = string.Format(@"SELECT oid,PID,DisplayName,OnSale,stockout,cy_list_price,cy_marketing_price,CP_Brand,CP_Place,CP_Tab,TireSize,Image 
                                            FROM[Tuhu_productcatalog].[dbo].[vw_Products] WITH(NOLOCK)
                                            WHERE PID IN ({0})", string.Join(",", pids));
