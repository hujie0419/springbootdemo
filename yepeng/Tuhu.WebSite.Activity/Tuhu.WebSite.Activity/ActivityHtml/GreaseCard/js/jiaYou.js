$(function () {
    ...
    function getCodeTip() {
        ...
        if (getCodeSecond < 1) {
            ...
            $("#getCode").removeClass("disabled").val("获取验证码");
        } else {
            $("#getCode").addClass("checkcode disabled").val(getCodeSecond + "秒后重试");
        }
    }
    //获取验证码
    $("#getCode").tap(function () {
        ...
        if (validPhone(phone) == true) {
            if (getCodeSecond == 60) {
                ...
                $.ajax({
                    url: "http://wx.tuhu.cn/Wxgwbt/GetGreaseCardCode?callback=?",
                    data: {
                        "phone": phone
                    },
                    dataType: "jsonp",
    ...
    $("#getCoupon").tap(function () {
        ...
        if (validPhone(phone) == true && code.length > 0) {
            if (getCouponSecond == 3) {
                ...
                $.ajax({
                    url: "http://wx.tuhu.cn/Wxgwbt/GetGreaseCardCoupon?callback=?",
                    data: {
                        "phone": phone,
                        "code": code
                    },
    ...
    function InvokeRandomData() {
        $.ajax({
            url: "http://wx.tuhu.cn/Wxgwbt/GetRandomGreaseCard?callback=?",
            data: {
            },
            dataType: "jsonp",
            type: "get",
