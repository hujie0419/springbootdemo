  ...
  <div>
    ...
    <div class="page-form-header">
      ...
      <div style="display:inline-block;">
        <Button type="info" @click="pushMsg()">发消息推送</Button>
    ...
    <Modal
        ...
        title="发送推送消息"
        @on-ok="pushDialog.pushMsg" :closable="false" :mask-closable="false">
        <table class="popup">
          <tr>
          ...
          <tr>
            <th>推送消息ID</th>
            <td>
              <Select transfer v-model="pushDialog.pushMsgId">
          ...
          <tr>
            <th>用户范围</th>
            <td>
              <span v-if="pushDialog.userCount > 0">所选用户数量：{{pushDialog.userCount}}</span>
              <Select transfer v-model="pushDialog.pushUserScope" @on-change="pushDialog.onUserScopeChange">
...
var viewModel = {
  ...
  pushDialog: {
    ...
    pushMsg: function () {
      ...
      util.modal.confirm({
        ...
        content: `确定要给推送消息吗？`,
        onOk: () => {
            util.ajax.post("/GroupBuyingV2/PushLotteryMessage", null, {
            headers: {
            ...
            params: {
              ...
              batchId: viewModel.pushDialog.pushMsgId
            }
          })
          .then(function (response) {
            if (response.data.Code === 1) {
  ...
  couponDialog: {
    ...
    issueCoupon: function () {
      ...
      var params = new URLSearchParams();
      params.append('productGroupId', viewModel.groupId);
      params.append('tag', viewModel.couponDialog.userScope);
      params.append('orderId', viewModel.couponDialog.couponOrderId);
      params.append('couponList', couponList);

      util.modal.confirm({
        title: "提示",
        content: `确定要给用户塞券吗？`,
        onOk: () => {
            util.ajax.post("/GroupBuyingV2/SendLotteryCoupon", params, {
              headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
              }
            })
