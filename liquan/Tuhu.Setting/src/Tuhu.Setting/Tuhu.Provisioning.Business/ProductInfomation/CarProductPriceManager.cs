...
namespace Tuhu.Provisioning.Business.ProductInfomation
{
    ...
    {
        private static readonly string conn_Gungnir = ConfigurationManager.ConnectionStrings["Gungnir"].ConnectionString;
        private static readonly string conn_Gungnir_AlwaysOnRead = ConfigurationManager.ConnectionStrings["Gungnir_AlwaysOnRead"].ConnectionString;
        private static readonly Common.Logging.ILog logger = LogManager.GetLogger(typeof(CarProductPriceManager));
        ...
        /// <returns></returns>
        public static async Task<List<CarProductPriceModel>> GetCarProductMutliPriceByCategory(int categoryId,int onSale,
            int isDaifa, int stockOut, int hasPintuanPrice, string keyWord, int keyWordSearchType, int page, int pageSize, string brand = "")
        {
            var list = new List<CarProductPriceModel>();
            var pids = new List<string>();
            try
            {
                var res = await GetPidsByFilter(categoryId, onSale, isDaifa, stockOut, hasPintuanPrice, keyWord, keyWordSearchType, page, pageSize, brand);
                ...
                {
                    // SqlServer最多支持2100个参数
                    if (pids.Count > 2100)
                    {
                        int index = 1, size = 100;
                        var stepPids = pids.Skip(size * (index - 1)).Take(size).ToList();
                        while (stepPids.Any())
                        {
                            list.AddRange(await FullPriceData(stepPids, conn));
                            index = index + 1;
                            stepPids = pids.Skip(size * (index - 1)).Take(size).ToList();
                        }
                    }
                    else
                    {
        ...
        /// <returns></returns>
        public static async Task<int> GetCarProductMutliPriceCountByCategory(int categoryId, int onSale,
            int isDaifa, int stockOut, int hasPintuanPrice, string keyWord, int keyWordSearchType, string brand = "")
        {
            var res = await GetPidsByFilter(categoryId, onSale, isDaifa, stockOut, hasPintuanPrice, keyWord, keyWordSearchType, 1, 10, brand);
            if (!res.Success)
                return 0;

            return res.Result.Pager.Total >= 10000 ? 9999 : res.Result.Pager.Total;
        }
        ...
        /// <param name="conn"></param>
        /// <returns></returns>
        private static async Task<List<CarProductPriceModel>> FullStockData(List<string> pids, SqlConnection conn, IDBScopeManager mana)
        {
            var list = await DalCarProductPrice.GetProductBaseInfoByPids(conn, pids);
            if (list == null || list.Count < 1)
                return list;
        ...
        /// <param name="brand"></param>
        /// <returns></returns>
        private static async Task<OperationResult<PagedModel<string>>> GetPidsByFilter(int categoryId, int onSale,
            int isDaifa, int stockOut, int hasPintuanPrice, string keyWord, int keyWordSearchType, int page, int pageSize, string brand = "")
        {
            // ProductCommonQueryAsync接口限制必须小于1w条数据（ES深度分页）
            while (page * pageSize >= 10000)
            {
                pageSize = pageSize - 1;
            }
            ...
            using (var client = new ShopProductClient())
            {
                ...
                #region 查询条件 
                dics.Add(nameof(ProductCommonQueryModel.IsTuhuProduct), new ProductCommonQuery
                {
                ...
                {
                    dics.Add(nameof(ProductCommonQueryModel.Brand), new ProductCommonQuery
                    {
                        CompareType = CompareType.Equal,
                        Value = brand
                    });
