    ...
    public class TuhuActivityMonitorJob : IJob
    {
        ...
        public string MonitorH5Activity()
        {
            ...
            if (activityConfigs.Any())
            {
                ...
                htmlStr = GenerateH5andLightAppTextString(activityConfigs);
        ...
        public string MonitorSpecificDaysExpireActivites(int days)
        {
            ...
            if (overdueActivities.Any())
            {
                ...
                htmlStr = GenerateTextTableString(overdueActivities);
        ...
        public string MonitorAppActivity()
        {
            ...
            if (OverdueActiveConfigList.Any())
            {
                ...
                htmlString = GenerateTextString(OverdueActiveConfigList);
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(appresult) || !string.IsNullOrWhiteSpace(h5result) || !string.IsNullOrWhiteSpace(lightAppresult) || !string.IsNullOrWhiteSpace(allactivitiesresult))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(appresult))
                    {
                        ...
                        appmessage.AppendLine("App首页活动过期列表:");
                        appmessage.AppendLine("消息样式:名称_类型_地址_所在首页名称_所在模块名称_所在帮助模块名称_结束时间_剩余过期时间");
                        appmessage.AppendLine(appresult);
                        MailService.SendWxFileMessage(ConfigurationManager.AppSettings["ActivityMonitorJob:To"], appmessage.ToString());
                    }
                    if (!string.IsNullOrWhiteSpace(h5result))
                    {
                        StringBuilder h5message = new StringBuilder();
                        h5message.AppendLine("华为轻应用首页活动过期列表:");
                        h5message.AppendLine("消息样式:名称_类型_地址_所在模块名称_所在帮助模块名称_结束时间_剩余过期时间");
                        h5message.AppendLine(h5result);
                        MailService.SendWxFileMessage(ConfigurationManager.AppSettings["ActivityMonitorJob:To"], h5message.ToString());
                    }
                    if (!string.IsNullOrWhiteSpace(lightAppresult))
                    {
                        StringBuilder lightmessage = new StringBuilder();
                        lightmessage.AppendLine("小程序首页活动过期列表:");
                        lightmessage.AppendLine("消息样式:名称_类型_地址_所在模块名称_所在帮助模块名称_结束时间_剩余过期时间");
                        lightmessage.AppendLine(lightAppresult);
                        MailService.SendWxFileMessage(ConfigurationManager.AppSettings["ActivityMonitorJob:To"], lightmessage.ToString());
                    }
                    if (!string.IsNullOrWhiteSpace(allactivitiesresult))
                    {
                        StringBuilder allactivitiesmessage = new StringBuilder();
                        allactivitiesmessage.AppendLine("4天内活动过期列表:");
                        allactivitiesmessage.AppendLine("消息样式:名称_类型_地址_结束时间_剩余过期时间");
                        allactivitiesmessage.AppendLine(allactivitiesresult);
                        MailService.SendWxFileMessage(ConfigurationManager.AppSettings["ActivityMonitorJob:To"], allactivitiesmessage.ToString());
                    }
                  //  MailService.SendWxFileMessage(ConfigurationManager.AppSettings["ActivityMonitorJob:To"], message.ToString());
                    /*
                    ...
                    //string filename = string.Format("ActivityMonitor_{0}.html", DateTime.Now.ToString("yyyy-MM-dd"));
                    ...
                    //MailService.SendWxFileMessage(ConfigurationManager.AppSettings["ActivityMonitorJob:To"], path);
                    //if (File.Exists(path))
                    //{
                    //    File.Delete(path);
                    //}
        ...
        public string GenerateTextTableString(List<OverdueActivityConfig> OverdueActiveConfigList)
        {
            ...
            foreach (var item in OverdueActiveConfigList)
            {
                ...
                sb.AppendFormat(string.IsNullOrWhiteSpace(item.Title) ? "无数据":item.Title);
                sb.Append("_");
                sb.AppendFormat(item.Type);
                sb.Append("_");
                sb.AppendFormat(string.IsNullOrWhiteSpace(item.Url) ? "无数据":item.Url);
                sb.Append("_");
        ...
        public string GenerateTextString(List<OverdueActivityConfig> OverdueActiveConfigList)
        {
            StringBuilder sb = new StringBuilder();         
            foreach (var item in OverdueActiveConfigList.OrderBy(p => p.EndDate))
            {
                ...
                sb.AppendFormat(string.IsNullOrWhiteSpace(item.Url) ? "无数据":item.Url);
                sb.Append("_");
                sb.AppendFormat(string.IsNullOrWhiteSpace(item.HomePageName) ? "无数据":item.HomePageName);
                sb.Append("_");
                sb.AppendFormat(string.IsNullOrWhiteSpace(item.ModuleName) ? "无数据":item.ModuleName);
                sb.Append("_");
                sb.AppendFormat(string.IsNullOrWhiteSpace(item.HelperModuleName) ? "无数据":item.HelperModuleName);
                sb.Append("_");
                sb.AppendFormat(item.EndDate.ToString("yyyy-MM-dd"));
                sb.Append("_");               
                sb.AppendFormat(days.ToString());
        ...
        public string GenerateH5andLightAppTextString(List<OverdueActivityConfig> OverdueActiveConfigList)
        {
            StringBuilder sb = new StringBuilder();
            foreach (var item in OverdueActiveConfigList.OrderBy(p => p.EndDate))
            {
                ...
                sb.Append(item.Type);
                sb.Append("_");
                sb.Append(string.IsNullOrWhiteSpace(item.Url)? "无数据": item.Url);
                sb.Append("_");
                sb.Append(string.IsNullOrWhiteSpace(item.ModuleName) ? "无数据":item.ModuleName);
                sb.Append("_");
                sb.Append(string.IsNullOrWhiteSpace(item.HelperModuleName) ? "无数据":item.HelperModuleName);
                sb.Append("_");
                sb.Append(item.EndDate.ToString("yyyy-MM-dd"));
                sb.Append("_");               
                sb.Append(days);
                sb.AppendLine();
            }
