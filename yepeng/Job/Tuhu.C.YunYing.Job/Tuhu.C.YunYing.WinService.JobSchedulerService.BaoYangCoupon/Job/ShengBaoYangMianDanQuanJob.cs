...
{
    /// <summary>胜牌机油免单券 huhengxing 2016-11-1 </summary>
    [DisallowConcurrentExecution]
    public class ShengBaoYangMianDanQuanJob : IJob
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(ShengBaoYangMianDanQuanJob));

        public void Execute(IJobExecutionContext context)
        {
            try
            {
                if (DateTime.Now < new DateTime(2016, 11, 4))
                {
                    Logger.Info("2016-11-4开始发放保养免单券");
                    return;
                }
                Logger.Info("启动任务");
                                                    ...
                                                    WHERE	P.GetRuleID = 1171
                                                            ...
                                                            AND NOT EXISTS ( SELECT	1
						                                                     ...
						                                                     WHERE	PC.UserId = O.UserID
								                                                    ...
								                                                    AND PC.CodeChannel = N'胜牌机油免单活动返券'
                    ...
                    foreach (var i in dic)
                    {
                        if (!string.IsNullOrWhiteSpace(i.SumMoney))
                        {
                            ...
                            if (result > 0)
                            {
                                ...
                                BLL.Business.SendMarketingSms(userTel, "途虎养车网胜牌机油免单券", m, "保养现金券");
                            }
                        }
                    }
                }
        ...
        public static int CreateOrYzPromotion(string userId, int money, string orderID)
        {
            try
            {
                ...
                promotionCodeModel.Description = "胜牌机油免单活动返券，限到店保养使用";
                ...
                promotionCodeModel.CodeChannel = "胜牌机油免单活动返券";
