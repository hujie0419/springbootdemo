...
";
        ...
        public static List<BeautyHomePageConfig> SelectBeautyBannerConfig(int status, string keyWords, int pageIndex, int pageSize,  out int count)
        {
            #region sql
...
Offset @begin Rows Fetch NEXT @pageSize Rows Only";
            ...
            if (count > 0)
            {
                paraList.Add(new SqlParameter("@begin", (pageIndex - 1) * pageSize));
                paraList.Add(new SqlParameter("@pageSize", pageSize));
