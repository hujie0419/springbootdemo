...
namespace Tuhu.Provisioning.DataAccess.DAO
{
    public class DALWebSiteHomeAd
    {
        ...
        public static DataTable SelectAllAdDetail(string idstart)
        {
            ...
            using (var dbhelper = new SqlDbHelper(conn))
            {
                var cmd = new SqlCommand(@"SELECT  ADC.ID ,
                ADC.Name AS ADCName,
                    ADC.Remark,
                ADC.Width,
                ADC.Height,
                ADC.Capacity,
                ADC.SwitchMillisecond,
                ADC.DefaultImage,
                ADC.DefaultUrl,
                ADC.IsShow,
                ADC.CreateDateTime AS AdcCreateDateTime,
                    ADC.LastUpdateDateTime AS AdcUpdateTime,
                    AD.AdColumnID,
                AD.Name AS AdName,
                    AD.Position,
                AD.BeginDateTime,
                ...
                AD.bgColor,
                AD.PKID,
                AD.CreateDateTime AS AdCreateDateTime,
                    AD.LastUpdateDateTime AS AdUpdateTime
                    FROM    dbo.tbl_AdColumn AS ADC WITH(NOLOCK)
                LEFT JOIN dbo.tbl_Advertise AS AD WITH(NOLOCK) ON ADC.ID = AD.AdColumnID
                WHERE   ADC.ID LIKE @IDStart");
        ...
        public static int UpdateModuleSetting(string id, int isEnabled)
        {
            ...
            using (var dbhelper = new SqlDbHelper(conn))
            {
                var cmd = new SqlCommand($"update  dbo.tbl_AdColumn set IsShow={isEnabled} where ID='{id}'");
                cmd.CommandType = CommandType.Text;
