<template>
  <div>
      ...
      <div>
        <label>商品PID</label>
        <Input type="text" placeholder="商品PID" style="width: 200px" v-model="filters.PID" />
      </div>
      ...
      <Modal v-model="modal.visible" :loading="modal.loading" title="拼团优惠券配置（编辑）" okText="提交" :transfer="false" cancelText="取消" @on-ok="SaveProductCouponConfig"   width="50%">
        <Form ref="modal.productConfig" :model="modal.productConfig" :rules="modal.rules" :label-width="90">
            <FormItem label="PID" prop="PID">
                <input v-model="modal.productConfig.PID" :disabled="isAdd===2" placeholder="自动生成"  @blur="FetchProductInfo(modal.productConfig.PID)" />
            </FormItem>
            <FormItem label="商品名称">
            ...
            <FormItem label="优惠券类型">
                <label><input type="radio" v-model="couponType" value="rate" @click="ChangeCouponType('rate')" >配置虚拟成本比例</label>
                <label><input type="radio" v-model="couponType" value="value" @click="ChangeCouponType('value')">配置项目原价</label>
            </FormItem>
            <FormItem v-for="(item, index) in modal.productConfig.Coupons" :label="'优惠券-'+(index+1)" :key="index" :prop="'executors.' + index">
                <Row>
                    <input style="display:inline-block;width:200px;height:30px;margin-right:10px"  type="text"  :key="index" @blur="FetchCouponInfo(item.CouponId,index)" v-model="item.CouponId" placeholder="优惠券PID" />
                    <Input style="display:inline-block;width:200px;margin-right:10px"  type="text" v-model="item.CouponName" :disabled="true" placeholder="优惠券名称" />
                    <label style="display:inline-block;width:100px;font-size: 10px;" v-if="couponType=='rate'">虚拟成本比例(%)</label>
                    <Input style="display:inline-block;width:80px" v-if="couponType=='rate'" type="text"  v-model="item.CouponRate" placeholder="1-100" />
                    <label style="display:inline-block;width:70px;font-size: 10px;" v-if="couponType=='value'" >优惠券价格</label>
                    <Input style="display:inline-block;width:80px" v-if="couponType=='value'"  type="text"  v-model="item.CouponValue" placeholder="大于0的整数" />
                ...
                <Row>
                    ...
                    <label style="display:inline-block;width:100px;font-size: 10px;" >面额(元)</label>
                    <Input style="display:inline-block;width:200px" type="text" v-model="item.CouponPrice" :disabled="true"/>
                </Row>
                <Row>
    ...
    methods: {
        ...
        if (pid) {
          ...
          .then(response => {
              if (response.data.status === 2) {
                ...
                that.modal.productConfig = response.data.data;
                that.modal.visible = true;
                that.isAdd = 2;
                if (that.modal.productConfig.Coupons.length > 0) {
                    that.couponType = that.modal.productConfig.Coupons[0].CouponRate > 0 ? 'rate' : 'value'
                }  
              }  
          });
        } else {
          that.modal.productConfig = {
              PID: "",
              ProductName: "",
              ProductPrice: 0,
              IsActive: true,
      ...
      FetchCouponInfo (PID, index) {
        ...
        if (PID) {
          ...
          .then(response => {
            if (response.data) {
              that.modal.productConfig.Coupons[index].PKID = response.data.PKID;
        ...
        for (var i = 0; i < that.modal.productConfig.Coupons.length; i++) {
            if (that.couponType === 'rate') {
                if (that.modal.productConfig.Coupons[i].CouponId) {
                    ...
                    that.$Message.warning("优惠券的成本比例只能是大于0的整数");
                    that.$nextTick(
                        () => {
                                that.modal.loading = true;
                            }
                    ...
                    that.modal.productConfig.Coupons[i].CouponValue = 0
                }   
            } else if (that.couponType === 'value') {
                that.modal.productConfig.Coupons[i].CouponRate = 0;
                if (that.modal.productConfig.Coupons[i].CouponValue > 0) {
                } else {
                    that.$Message.warning("优惠券价格只能是大于0的整数");
                    that.$nextTick(
                        () => {
                                that.modal.loading = true;
                            }
            ...
            submitData.push({
                ...
                CouponValue: that.modal.productConfig.Coupons[i].CouponValue
            });
        }
        if (that.couponType === 'rate' && sumCouponRate !== 100) {
