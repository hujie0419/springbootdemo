...
ORDER BY PKID DESC
        ...
        internal async Task<IEnumerable<GFBankSmsTask>> SelectGFSmsTasks(DateTime startTime, int pageSize, int pageIndex, string status)
        {
            ...
            var sql = @"SELECT  A.PKID AS PromotionTaskId ,
        B.PKID AS RedemptionCodeTaskId ,
        A.Mobile
FROM    Tuhu_groupon..GFBankPromotionTask AS A WITH ( NOLOCK )
        JOIN Tuhu_groupon..GFBankRedemptionCodeTask AS B WITH ( NOLOCK ) ON A.Mobile = B.Mobile
WHERE   A.Status = @Status
        AND B.Status = @Status
        AND A.BusinessType='Activate'
		AND B.BusinessType='Activate'
        AND A.CreatedDateTime > @StartTime
ORDER BY A.PKID DESC
        ...
        /// <param name="status"></param>
        /// <returns></returns>
        internal async Task<bool> BatchUpdateGFBankSmsTasksStatus(IEnumerable<GFBankSmsTask> smsTask, string status)
        {
            var sql = @"UPDATE  A
SET     A.Status = @PromotionTaskStatus
FROM    Tuhu_groupon..GFBankPromotionTask AS A WITH ( NOLOCK )
        JOIN Tuhu_groupon..SplitString(@PromotionTaskIds, ',', 1) AS B ON A.PKID = B.Item
UPDATE  A
SET     A.Status = @RedemptionCodeTaskStatus
FROM    Tuhu_groupon..GFBankRedemptionCodeTask AS A WITH ( NOLOCK )
        JOIN Tuhu_groupon..SplitString(@RedemptionCodeTaskIds, ',', 1) AS B ON A.PKID = B.Item";
            ...
            using (var cmd = dbHelper.CreateCommand(sql))
            {
                ...
                cmd.Parameters.Add(new SqlParameter("@PromotionTaskIds", string.Join(",", smsTask.Select(s => s.PromotionTaskId))));
                cmd.Parameters.Add(new SqlParameter("@RedemptionCodeTaskStatus", status));
                cmd.Parameters.Add(new SqlParameter("@RedemptionCodeTaskIds", string.Join(",", smsTask.Select(s => s.RedemptionCodeTaskId))));
