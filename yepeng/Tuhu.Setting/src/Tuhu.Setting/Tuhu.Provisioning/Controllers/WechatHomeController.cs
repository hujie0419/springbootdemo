    ...
    public class WechatHomeController : Controller
    {
        ...
        public JsonResult SaveWxAppConfig(string configJson)
        {
            ...
            var bef = manager.FetchWxAppUserEventConfig(config.OriginId, config.EventType,
                config.UserData);
            if (bef.PKID > 0 && bef.PKID != config.PKID)
            {
                return Json($"该小程序{(config.EventType.Equals("user_enter_tempsession") ? "【用户通过客服消息按钮进入会话】" : $"用户消息【{config.UserData.Trim()}】")}已配置");
            }
        ...
        public ActionResult SubmitWX_QRCodeConfig(WX_QRCodeConfigModel model)
        {
            ...
            try
            {
                ...
                if (model.PKID == 0)
                {
                    if (model.action_name == "QR_STR_SCENE")//临时
                    {
                        if (model.expire_seconds < 30)
                        {
                            model.expire_seconds = 30;
                        }
                        else if (model.expire_seconds > 2592000)
                        {
                            return Content(JsonConvert.SerializeObject(new { state = "error", message = "" }));
                        }
                    }
                    else
                    {
                        model.expire_seconds = 0;
                    }
                }
