...
{
    public class MDBeautyApplyConfigController : Controller
    {
        ...
        public ActionResult List(MDBeautyApplyConfig model, int pageIndex = 1, int pageSize = 15)
        {
            ...
            var lists = MDBeautyApplyConfigManager.GetMDBeautyApplyConfigList(model, pageSize, pageIndex, "Apply", out count);
        ...
        {
            using (var stream = new MemoryStream(System.IO.File.ReadAllBytes(Server.MapPath(@"~/Content/Export/门店美容活动报名审核列表.xlsx"))))
            {
                int count = 0;
                var list = MDBeautyApplyConfigManager.GetMDBeautyApplyConfigList(model, int.MaxValue, 1, "Apply", out count);
                ...
                if (list.Count > 0)
                {
                    ...
                    Response.AppendHeader("Content-Disposition", "attachment;fileName=门店美容活动报名审核列表" + ".xlsx");
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult Edit(MDBeautyApplyConfig model, string type)
        {
            try
            {
                if (MDBeautyApplyConfigManager.UpdateMDBeautyApplyConfig(model, type))
                {
