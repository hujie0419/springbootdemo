    ...
    public class BankMRActivityController : Controller
    {
            ...
            string activityName = null, string serviceId = null, string settleMentMethod = null)
        {
            ...
            if (configs != null)
            {
                ...
                configs.ForEach(s => s.CooperateName = cooperateUsers.FirstOrDefault(t => t.PKID == s.CooperateId)?.CooperateName);
            }
        ...
        public FileResult DownLoadRulesTemplate(string verifyType)
        {
            var ms = new MemoryStream();
            if (verifyType != null)
            {
        ...
        private List<Tuple<string, string>> GetBankRuleExcelColumn(string verifyType)
        {
            var verifyTypeArray = verifyType.Split(',');
        ...
        [HttpGet]
        public FileResult GetImportBankMRActivityUsers(string batchCode)
        {
            ...
            var cloums = GetBankRuleExcelColumn(temp.Value.Value.FirstOrDefault().VerifyType);
            var row = sheet.CreateRow(0);
            int index = 0;
