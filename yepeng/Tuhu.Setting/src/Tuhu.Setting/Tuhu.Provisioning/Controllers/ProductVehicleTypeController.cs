...
namespace Tuhu.Provisioning.Controllers
{
                ...
                switch (productInfo.VehicleLevel)
                {
                    ...
                    case "五级车型":
                        ...
                        var dataLevel5 = vehicleTypeInfoList.GroupBy(item => item.Brand)
                            .ToDictionary(i => i.Key,
                                i => i.ToList().GroupBy(t => t.Vehicle).ToDictionary(j => j.Key, j => j.ToList().GroupBy(t => t.PaiLiang).ToDictionary(a => a.Key, a => a.ToList().GroupBy(c => c.Nian).ToDictionary(d => d.Key, d => d.ToList().GroupBy(e => e.SalesName).ToDictionary(f => f.Key, f => f.ToList())))));
    ...
    var entity = new ProductVehicleTypeConfigOpLog()
        ...
        public ActionResult FileImport(HttpPostedFileBase filebase)
        {
            ...
            var pid = System.Web.HttpUtility.UrlDecode(Request.UrlReferrer.Segments[Request.UrlReferrer.Segments.Length - 1]);
            string fileName;
            string ext;
            string noFileName;
            string savePath;
