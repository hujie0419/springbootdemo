...
using Tuhu.Provisioning.Models.Promotion;
    ...
    public class PromotionController : Controller
    {
        ...
        public ActionResult AddGetRule(int id, int? rid, string iscopy)
        {
            ...
            else
            {
                ...
                model = new GetPCodeModel() { RuleDescription = promotionInfo?.RuleDescription, PromotionType = promotionInfo.PromotionType };
            }
            var SettingTemp = PromotionManager.GetDepartmentUseSetting();
            if (SettingTemp != null && SettingTemp.Rows.Count > 0)
            {
        ...
        public ActionResult SaveGetRule(GetPCodeModel model)
        {
            ...
            //更新缓存
            var getCouponRulesInfo = PromotionManager.SelectGeCouponRulesByGetRuleID(model.GETPKID);
            if (getCouponRulesInfo != null)
            {
                using (var client = new CreatePromotionClient())
                {
                    client.RefreshGetCouponRulesCache(getCouponRulesInfo.GetRuleGUID);
                }
            }
...
SELECT	CR.PKID, Type, CR.Name,PromotionType 
...
EXEC Gungnir..PromotionCode_CreatePromotionCode_Cellphones @Cellphones, @StartDate, @EndDate, @Type, @RuleId, @Description, @Discount, @MinMoney, @CreateUser, @Channels;", sqlparamters.ToArray());
        ...
        public ActionResult CreatePromotionTask(int? id)
        {
            ...
            //非触发任务不做产品类型绑定
            if (!model.PromotionTask.TaskType.HasValue || model.PromotionTask.TaskType.Value != (int)PromotionTaskType.Repeat)
            {
                return View(model);
            }
            #region 产品分类默认值赋值
            //先获取关系内容
            var promotionTaskProductCategoryList = DalPromotionTaskProductCategory.GetList(new DataAccess.PromotionTaskProductCategory { PromotionTaskId = id.Value });
            if (promotionTaskProductCategoryList == null || promotionTaskProductCategoryList.Count <= 0)
            {
                return View(model);
            }
            var categoryIds = new StringBuilder();
            var parentCategoryIds = new StringBuilder();
            var categoryNames = new StringBuilder();
            var nodeNos = new StringBuilder();
            ...
            var pName = "";
            foreach (var categoryModel in promotionTaskProductCategoryList)
            {
                categoryIds.AppendFormat("{0},", categoryModel.ProductCategoryId);
                parentCategoryIds.AppendFormat("{0},", categoryModel.ParentCategoryId);
                categoryNames.AppendFormat("{0},", categoryModel.CategoryName);
                nodeNos.AppendFormat("{0},", categoryModel.NodeNo);
                if (categories != null)
                {
                    var treeModel = categories.Find(t => t.id == categoryModel.ProductCategoryId);
                    pName = treeModel?.name;
                }
                productNames.AppendFormat("{0},", pName);
            }
            if (categoryIds.Length > 0)
            {
                categoryIds = categoryIds.Remove(categoryIds.Length - 1, 1);
                parentCategoryIds = parentCategoryIds.Remove(parentCategoryIds.Length - 1, 1);
                categoryNames = categoryNames.Remove(categoryNames.Length - 1, 1);
                nodeNos = nodeNos.Remove(nodeNos.Length - 1, 1);
                productNames = productNames.Remove(productNames.Length - 1, 1);
            }
            ViewBag.ProductCategoryIds = categoryIds;
            ViewBag.ParentCategoryIds = parentCategoryIds;
            ViewBag.CategoryNames = categoryNames;
            ViewBag.NodeNos = nodeNos;
            ...
            IDictionary<string, string> vehicleInfos)
        {
            ...
            if (promotionTask != null)
            {
                ...
                //订单的产品类型
                if (promotionTask.ProductType > 0)
                {
                    string productTypeStr = string.Empty;
                    if (promotionTask.ProductType == 1)
                        productTypeStr = "全部";
                    else if (promotionTask.ProductType == 2)
                        productTypeStr = "套装产品";
                    else if (promotionTask.ProductType == 3)
                        productTypeStr = "非套装产品";

                    filterItems.Add(new FilterItem
                    {
                        Title = "订单类型：" + productTypeStr,
                        ElementName = "ProductType",
                        ElementValue = promotionTask.ProductType.ToString()
                    });
                }
            }
        ...
        public ActionResult CreatePromotionTask(PromotionTaskParamsModel promotionTaskParams)
        {
            try
            {
                ...
                var promotionTask = new PromotionTask()
                {
                    ...
                    ProductType = promotionTaskParams.ProductType,

                };

                if (promotionTaskParams.TaskType == PromotionTaskType.Repeat)
                {
                    if (!string.IsNullOrWhiteSpace(promotionTaskParams.ProductCategoryIds) && promotionTaskParams.ProductCategoryIds.IndexOf(',') > 0)
                    {
                        //清空原有逻辑，按新逻辑走
                        promotionTask.Category = "";
                    }

                    //触发任务
                    promotionTask.ProductCategoryIds = promotionTaskParams.ProductCategoryIds;
                    promotionTask.ParentCategoryIds = promotionTaskParams.ParentCategoryIds;
                    promotionTask.NodeNos = promotionTaskParams.NodeNos;
                    promotionTask.CategoryNames = promotionTaskParams.CategoryNames;
                }
        ...
        /// <returns></returns>
        [PowerManage]
        public ActionResult SetCouponForMaintenance()
        {
            return View();
        }
        /// <summary>
        ...
        /// <returns></returns>
        public JsonResult SendCouponForBaoYang(SendCouponForBaoYangModel couponModel)
        {
            var dicResult = new Dictionary<string, object>();
            //金额验证
            if (couponModel.Discount > couponModel.MinMoney)
            {
                dicResult.Add("Code", -1);
                dicResult.Add("Msg", "优惠券使用金额大于等于优惠券金额");
                return Json(dicResult, JsonRequestBehavior.DenyGet);
            }
            //优惠券描述限制字数
            if (couponModel.Description.Length > 50)
            {
                dicResult.Add("Code", -1);
                dicResult.Add("Msg", "优惠券描述不能超过50字符");
                return Json(dicResult, JsonRequestBehavior.DenyGet);
            }
            using (var userClient = new UserAccountClient())
            using (var client = new PromotionClient())
            {
                var userInfo = userClient.GetUserByMobile(couponModel.Phone);
                if (!userInfo.Success)
                {
                    dicResult.Add("Code", -1);
                    dicResult.Add("Msg", userInfo.ErrorMessage);
                    return Json(dicResult, JsonRequestBehavior.DenyGet);
                }
                couponModel.UserID = userInfo?.Result?.UserId;
                couponModel.PromotionName = "保养券（限到店订单）";
                couponModel.DepartmentName = "运营";
                couponModel.IntentionName = "常规促销";
                couponModel.Author = User.Identity.Name;
                couponModel.Issuer = User.Identity.Name;
                couponModel.Creater = User.Identity.Name;
                couponModel.Channel = "保养塞券";
                couponModel.IssueChannle = "手动塞券";
                couponModel.UserID = userInfo?.Result?.UserId;
                var model = ObjectMapper.ConvertTo<SendCouponForBaoYangModel, CreatePromotionModel>(couponModel);
                var response = client.CreatePromotionForYeWu(model);
                if (response.Success)
                {
                    dicResult.Add("Code", 1);
                    dicResult.Add("Msg", "发券成功");
                    return Json(dicResult, JsonRequestBehavior.DenyGet);
                }
                else
                {
                    dicResult.Add("Code", -1);
                    dicResult.Add("Msg", response.Result.ErrorMessage);
                    return Json(dicResult, JsonRequestBehavior.DenyGet);
                }
            }
        }
