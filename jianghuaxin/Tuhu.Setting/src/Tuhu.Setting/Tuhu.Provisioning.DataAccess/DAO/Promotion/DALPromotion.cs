...
FROM    Tuhu_productcatalog..vw_ProductCategories AS c WITH ( NOLOCK );");
        ...
        public static DataTable SelectGeCouponRulesByRuleID(int id)
        {
            ...
            using (var dbhelper = new SqlDbHelper(conn))
            {
                var cmd = new SqlCommand("SELECT CR.PKID,CR.Name,CR.InstallType,CR.RuleDescription,GCR.PKID AS GETPKID,GCR.GetRuleGUID, GCR.RuleID,GCR.SingleQuantity,GCR.PromtionName,GCR.Description,GCR.Discount,GCR.Minmoney,GCR.Term, GCR.Channel,GCR.ValiStartDate,GCR.ValiEndDate,GCR.AllowChanel,GCR.Quantity,GCR.RemindQuantity,GCR.RemindEmails,GCR.GetQuantity,GCR.DepartmentId,GCR.IntentionId,GCR.BusinessLineId,DXStartDate,DXEndDate,SupportUserRange,DetailShowStartDate,DetailShowEndDate,DeadLineDate,IsPush,PushSetting  FROM	Activity..tbl_CouponRules AS CR WITH(NOLOCK) LEFT JOIN Activity..tbl_GetCouponRules AS GCR WITH(NOLOCK) ON CR.PKID = GCR.RuleID WHERE CR.PKID = @PKID ");
                cmd.CommandType = CommandType.Text;
                cmd.Parameters.AddWithValue("@PKID", id);
                return dbhelper.ExecuteDataTable(cmd);
            }
