...
namespace Tuhu.Provisioning.Business.RebateConfig
{
    public class RebateConfigManager
    {
        ...
        public List<RebateConfigModel> SelectRebateConfig(Status status, string orderId, string phone,
            ...
            string wxName, string principalPerson, string rebateMoney, string source, int? installShopId,
            ...
            try
            {
                dbScopeReadManager.Execute(conn =>
                {
                    result = DALRebateConfig.GetRebateConfig(conn, status, orderId ?? string.Empty, phone, wxId, remarks,
                        timeType, startTime, endTime, wxName, principalPerson, rebateMoney, source, installShopId, pageIndex, pageSize);
                    if (result != null && result.Any() && pageSize != 9999999)
                    {
                        ...
                        var shopInfo = ShopService.SelectShopSimpleDetails(result.Select(x => x.InstallShopId).ToList());
                        foreach (var item in result)
                        {
                            ...
                            item.CarparName = shopInfo?.Where(_ => _.ShopId == item.InstallShopId)?.FirstOrDefault()?.CarparName;
                        }
                    }
                });
            }
                            ...
                            else
                            {
                                if (!string.IsNullOrEmpty(data.OpenId))
                                {
                                    if (data.Status == Status.Applying)
                                    {
                                        ...
                                        {
                                            var rebatePageConfig = ActivityService.SelectRebateApplyPageConfig();
                                            if (rebatePageConfig != null && !string.IsNullOrEmpty(rebatePageConfig.RedBagRemark))
                                            {
                                                var getResult = SendRedBag(new WxSendRedBagRequest()
                                                {
                                                    ...
                                                    Channel = "途虎朋友圈点赞返现",
                                                    Remark = rebatePageConfig?.RedBagRemark
                                                });
                                                if (getResult.Item1)
                                                {
                                                ...
                                                msg = getResult.Item1 ? "返现成功" : (getResult.Item2 ?? "返现失败");
                                            }
                                            else
                                            {
                                                msg = "未获取到红包零钱文案配置信息";
                                            }
                                            InsertLog(pkid.ToString(), "SendRedBag", msg, $"OpenId：{data.OpenId},Money：{decimal.Parse("25.00")}", string.Empty, string.Empty, user);
                                        }
                                    }
                                    else
                                    {
        ...
        public bool WxPayManualRetry(int pkid, string user)
        {
            ...
            try
            {
                ...
                if (info != null)
                {
                    ...
                    if (pay != null && pay.Any())
                    {
                        if (pay.FirstOrDefault().PayStatus == 1 || pay.FirstOrDefault().PayStatus == 2)
                        {
                        ...
                        else
                        {
                            ...
                            if (getResult.Success&& getResult.Result > 0)
                                {
                                    result = dbScopeManager.Execute(conn => DALRebateConfig.UpdateStatusForCompleteV2(conn, pkid)) > 0;
                                }
                            }
        ...
        public bool InsertRebateApplyPageConfig(RebateApplyPageConfig data, string user)
        {
            ...
            try
            {
                dbScopeManager.CreateTransaction(conn =>
                {
                    var pkid = DALRebateConfig.InsertRebateApplyPageConfig(conn, data, user);
        ...
        /// <returns></returns>
        public bool UpdateRebateRecordInstallShopId(int pageIndex, int pageSize)
        {
            var result = false;
            try
            {
                dbScopeManager.CreateTransaction(conn =>
                {
                    var data = DALRebateConfig.GetNotInstallShopIdRebateRecrd(conn, pageIndex, pageSize);
                    if (data != null && data.Any())
                    {
                        foreach (var item in data)
                        {
                            var orderInfo = OrderService.FetchOrderByOrderId(item.OrderId);
                            if (orderInfo != null && orderInfo.InstallShopId != null && orderInfo.InstallShopId > 0)
                            {
                                result = DALRebateConfig.UpdateInstallShopIdByPKID(conn, orderInfo.InstallShopId ?? 0, item.PKID) > 0;
                            }
                        }
                    }
                });
