...
namespace Tuhu.Service.Activity.UnitTest
{
    ...
    public class SalePromotionActivityUnitTest
    {
        ...
        /// </summary>
        [TestMethod]
        public void SetDiscountProductDetailImg()
        {
            var request = new SetDiscountProductDetailImgRequest()
            {
                ActivityId = "dfbb2252-298a-4104-8fb5-cc163bf6313e",
                DetailImgUrl = "123"
            };
            using (var client = new SalePromotionActivityClient())
            {
                var result = client.SetDiscountProductDetailImg(request).Result;
                Assert.IsNotNull(result.ResponseRow > 0);
            }
        }
