...
using Tuhu.Provisioning.Business.BatteryLevelUp;
...
namespace Tuhu.Provisioning.Controllers
{
    public class BaoyangController : Controller
    {
        ...
        public ActionResult PriorityVehicleSettingRemoveCache(string[] vehicleIds)
        {
            bool result = false;
            if (vehicleIds != null &&vehicleIds.Length > 0)
            {
                ...
                {
                    array.Add($"PriorityVehicleSettingNew/{vehicleIds[i]}");
                } 
                var manager = new BaoYangManager();
                result = manager.RemoveCache(array);
            }
            return Json(result);
        }
        ...
        public ActionResult GetProductPrioritySettings(string partName)
        {
            var manager = new BaoYangManager();
            var list = manager.GetProductPrioritySettingsNew(partName).Select(x => new
            {
                x.Brand,
                x.PartName,
                x.Priority,
            ...
            return Json(new { status = list != null, data = list }, JsonRequestBehavior.AllowGet);
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<ActionResult> GetBaoYangCategories()
        {
            var manager = new BaoYangManager();
            var categories = await manager.GetAllBaoYangPackageCategoriesAsync();
            var packages = await manager.GetBaoYangPackageDescriptionAsync();
            var settings = new BaoYangConfigManager().GetPrioritySettingMapConfig();
            var list = new List<BaoYangCategoryTreeModel>();
            if (categories != null && categories.Categories != null && categories.Categories.Any() && packages != null && packages.Any())
            {
                Func<string, string, string> getNameFunc = (byType, category) =>
                {
                    if (settings.ContainsKey(byType))
                    {
                        return settings[byType];
                    }
                    if (byType.Contains("ys") &&
                        string.Equals(category, "Wiper", StringComparison.OrdinalIgnoreCase) &&
                        settings.ContainsKey("ys"))
                    {
                        //雨刷
                        return settings["ys"];
                    }
                    if (byType.Contains("sa") &&
                        string.Equals(category, "JZQ", StringComparison.OrdinalIgnoreCase) &&
                        settings.ContainsKey("sa"))
                    {
                        //减震器
                        return settings["sa"];
                    }
                ...
                {
                    var items = new List<BaoYangCategoryLeave>();
                    if (category.Items != null && category.Items.Any())
                    {
                        foreach (var item in category.Items)
                        {
                            var package = packages.FirstOrDefault(p => string.Equals(p.PackageType, item.PackageType, StringComparison.OrdinalIgnoreCase));
                            if (package != null && package.Items != null && package.Items.Any())
                            {
                                items.AddRange(package.Items.Select(x => new BaoYangCategoryLeave
                                {
                                    Category = x.CatalogName,
                                    PartName = getNameFunc(x.BaoYangType, x.CatalogName),
                                }));
                            }
                        }
                    }
                    if (items.Any())
                    {
                        list.Add(new BaoYangCategoryTreeModel
                        {
                            CategoryName = category.CategoryName,
                            CategoryType = category.CategoryType,
                            //根据类别和类目名称去重复
                            Items = items.Where(item => !string.IsNullOrWhiteSpace(item.PartName)).Distinct().ToList()
                        });
                    }
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<ActionResult> GetBaoYangCategoriesInfo()
        {
            var result = new List<BaoYangCategoryTreeModel>();
            var configManager = new BaoYangConfigManager();
            var configInfo = configManager.GetRecommendConfigInfo();
            var baoyangManager = new BaoYangManager();
            var package = await baoyangManager.GetBaoYangPackageDescriptionAsync();
            var baoyaoTypeList = package.SelectMany(x => x.Items).Select(x => new { x.CatalogName, x.BaoYangType, x.DisplayName });

            result = configInfo.CategoryPartNameMap.Select(item => new BaoYangCategoryTreeModel()
            {
                CategoryName = item.category,
                CategoryType = string.Empty,
                Items = item.GetPartNames().Select(partName =>
                {
                    var baoYangCategoryLeaveItem = new BaoYangCategoryLeave();
                    baoYangCategoryLeaveItem.PartName = partName;
                    var mapTypes = configInfo.MaintenanceTypePartNameMap?.Where(x => string.Equals(x.partName, partName, StringComparison.OrdinalIgnoreCase))?.Select(x => x.type);
                    if (mapTypes != null && mapTypes.Any())
                    {
                        baoYangCategoryLeaveItem.Category = baoyaoTypeList.Where(x => mapTypes.Contains(x.BaoYangType)).Select(x => x.CatalogName)?.Distinct()?.FirstOrDefault() ?? string.Empty;
                    }
                    return baoYangCategoryLeaveItem;
                }).ToList()
            }).ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult SaveProductPrioritySettings(List<ProductPrioritySettingNew> settings)
        {
            ...
            Func<string> vaildFun = () =>
            {
                ...
                settings.ForEach(x =>
                {
                    x.PartName = x.PartName?.Trim() ?? string.Empty;
                    x.PriorityType = x.PriorityType?.Trim() ?? string.Empty;
                    x.Series = x.Series?.Trim() ?? string.Empty;
                    x.Brand = x.Brand?.Trim() ?? string.Empty;
                    x.IsEnabled = true;
                });
                ...
                if (settings.Any(s => string.IsNullOrEmpty(s.PartName) || string.IsNullOrEmpty(s.Brand)))
                {
                    return "配置有误！请刷新页面重试。";
                }

                if (settings.GroupBy(x => new { x.PartName, x.PriorityType, x.Brand, x.Series }).Count() < settings.Count)
                {
                    return "配置不允许重复";
                }
            ...
            settings = settings
                .GroupBy(x => new { x.PartName, x.PriorityType })
                .SelectMany(g =>
                    g.OrderBy(x => x.Priority).Select((x, i) => new ProductPrioritySettingNew
                    {
                        Priority = i + 1,
            ...
            var manager = new BaoYangManager();
            var success = manager.SaveProductPrioritySettings(settings, User.Identity.Name);
            return Json(new { status = success });
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult GetProductPrioritySettingLog(string partName, string piorityType, int pageIndex, int pageSize)
        {
            string identityID = string.Concat(partName, "-", piorityType);
            ...
            var manager = new BaoYangManager();
            string logType = "BaoYangProductPrioritySettingNew";
            int totalCount = 0;
            var logResult = manager.SelectBaoYangOprLog(logType, identityID, null, null, null, pageIndex, pageSize, out totalCount);
            return Json(new { status = true, totalCount = totalCount, data = logResult });
        }
        /// <summary>
        /// 保存产品推荐描述配置
        /// </summary>
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult SaveProductPrioritySettingDescribeConfig(ProductPriorityDescribeConfig config)
        {
            var status = false;
            if (config != null && !string.IsNullOrWhiteSpace(config.PartName))
            {
                var manager = new BaoYangConfigManager();
                status = manager.SaveProductPriorityDescribeConfig(config);
            }
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult GetProductPrioritySettingDescribeConfig(string partName)
        {
            if (!string.IsNullOrWhiteSpace(partName))
            {
                var manager = new BaoYangConfigManager();
                var list = manager.GetProductPriorityDescribeConfigs();
                if (list != null && list.Any())
                {
                    var config = list.FirstOrDefault(x => x.PartName == partName);
                    if (config != null)
                    {
                        return Json(new { Status = true, Data = config }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                        return Json(new { Status = false, Data = new { partName, Value = string.Empty } }, JsonRequestBehavior.AllowGet);
                    }
                }
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult GetBatteryLeveUpList(BatteryLevelUpRequest request)
        {
            BaoYangResultEntity<Tuple<int, List<BatteryLevelUpReslut>>> result = new BaoYangResultEntity<Tuple<int, List<BatteryLevelUpReslut>>>() { Status = false };
            ...
            if (request.OriginalPID != null && request.OriginalPID != string.Empty)
            {
                request.OriginalPID = request.OriginalPID.Trim();
            }
            result = new BatteryLevelUpManager().GetBatteryLeveUpList(request);
            return Json(result);
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult InsertOrUpdateBatteryLevelUp(BatteryLevelUpEntity request)
        {
            BaoYangResultEntity<bool> result = new BaoYangResultEntity<bool> { Status = false };
            result.Status = false;
            if (request == null || string.IsNullOrWhiteSpace(request.NewPID) || string.IsNullOrWhiteSpace(request.OriginalPID) || string.IsNullOrWhiteSpace(request.Copywriting))
            {
                result.Msg = "参数验证失败";
                return Json(result);
            }
            if (request.Copywriting.Length > 16)
            {
                result.Msg = "提示语不能超过16个中文字符";
                return Json(result);
            }
            request.NewPID = request.NewPID.Trim();
            request.OriginalPID = request.OriginalPID.Trim();
            request.Copywriting = request.Copywriting.Trim();
            if (request.NewPID == request.OriginalPID)
            {
                result.Msg = "不能填写一样的数据!";
                return Json(result);
            }
            var manager = new ProductManager();
            var productDetailDic = manager.SelectProductDetail(new List<string> { request.OriginalPID });

            if (productDetailDic == null || !productDetailDic.Any() || !productDetailDic.ContainsKey(request.OriginalPID))
            {
                result.Msg = "查询原始产品信息失败";
                return Json(result);
            }
            productDetailDic = manager.SelectProductDetail(new List<string> { request.NewPID });
            if (productDetailDic == null || !productDetailDic.Any() || !productDetailDic.ContainsKey(request.NewPID))
            {
                result.Msg = "查询升级购产品信息失败";
                return Json(result);
            }
            result.Status = true;
            if (request.PKID <= 0)
            {
                result = new BatteryLevelUpManager().InsertBatteryLevelUp(request, User.Identity.Name);
            }
            else
            {
                result = new BatteryLevelUpManager().UpdateBatteryLevelUp(request, User.Identity.Name);
            }
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<JsonResult> DeleteBatteryLevelUpByPkid(int pkid)
        {
            BaoYangResultEntity<bool> result = new BaoYangResultEntity<bool> { Status = false };
            if (pkid <= 0)
            {
            ...
            {
                result = await new BatteryLevelUpManager().DeleteBatteryLevelUpByPkid(pkid, User.Identity.Name);
            }
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        /// <summary>
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<JsonResult> RemoveBtaaeryLevelUpCache()
        {
            BaoYangResultEntity<bool> result = new BaoYangResultEntity<bool>();
            result = await new BatteryLevelUpManager().RemoveBtaaeryLevelUpCache();
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        /// <summary>
        /// 获取操作日志
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult GetBtaaertLevelUpSettingLog(string originalPID, int pageIndex, int pageSize)
        {
            BaoYangResultEntity<Tuple<int, List<BaoYangOprLog>>> result = new BaoYangResultEntity<Tuple<int, List<BaoYangOprLog>>>() { Status = false };

            if (originalPID == null || originalPID == string.Empty)
            {
                return Json(new { status = false, msg = "参数验证失败" });
            }
            if (pageIndex <= 0)
            ...
            var manager = new BaoYangManager();
            string logType = "BaoYangProductBatteryLevelUpSetting";
            int totalCount = 0;
            result.Status = true;
            var batteryLevelUpLog = manager.SelectBaoYangOprLog(logType, originalPID, null, null, null, pageIndex, pageSize, out totalCount).Item2;
            result.Data = Tuple.Create(totalCount, batteryLevelUpLog);
            return Json(result);
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<ActionResult> GetPackageTypeRelations()
        {
            var manager = new BaoYangManager();
            var packageTypeRelations = await manager.GetPackageTypeRelationsAsync();
            var packageConfig = await manager.GetBaoYangPackageDescriptionAsync();
            var result = packageTypeRelations.Select(x =>
            {
                var mainPackage = packageConfig.FirstOrDefault(p =>
                    string.Equals(p.PackageType, x.MainPackageType, StringComparison.OrdinalIgnoreCase)
                );

                var auxiliaryPackages = (x.RelatedPackageTypes ?? new List<string>())
                    .Distinct()
                    .Select(t => new
                    {
                        PackageType = t,
                        PackageName = packageConfig.FirstOrDefault(p =>
                            string.Equals(p.PackageType, t, StringComparison.OrdinalIgnoreCase)
                        )?.ZhName,
                    });

                return new
                {
                    PackageType = x.MainPackageType,
                    PackageName = mainPackage?.ZhName,
                    x.IsStrongRelated,
                    RelatedPackages = auxiliaryPackages.ToList(),
                };
            });
            return Json(result, JsonRequestBehavior.AllowGet);
        }
