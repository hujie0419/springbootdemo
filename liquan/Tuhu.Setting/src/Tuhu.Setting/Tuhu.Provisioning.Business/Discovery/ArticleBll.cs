...
namespace Tuhu.Provisioning.Business.Discovery
{
    public class ArticleBll : BaseBll
    {
        ...
        public static async Task<List<Article>> SearchArticle(ArticleStatus status, PagerModel pager, DateTime? startCreateDateTime, DateTime? endCreateDateTime, string pkid = null, string keyword = null, int type = 5, int category = -1)
        {
            if (string.IsNullOrWhiteSpace(keyword) || string.IsNullOrEmpty(keyword))
                keyword = string.Empty;
            return await articleDal.SearchArticle(ProcessConnection.OpenMarketingReadOnly, status, pager, startCreateDateTime, endCreateDateTime, pkid, keyword, type, category);
        }
