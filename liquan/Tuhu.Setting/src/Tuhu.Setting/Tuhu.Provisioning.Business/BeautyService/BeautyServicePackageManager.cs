        ...
        public static IEnumerable<BeautyServicePackageDetail> GetBeautyServicePackageDetails(int packageId)
        {
            ...
            try
            {
                ...
                if (result != null && result.Any())
                {
                    ...
                    result?.ToList().ForEach(s => s.CooperateName = cooperateUsers.FirstOrDefault(u => u.PKID == s.CooperateId)?.CooperateName);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                else if (!string.IsNullOrWhiteSpace(mobile))
                {
                    ...
                    if (user != null && user.UserId != Guid.Empty)
                    {
                        ...
                        beautyServicePackageCodes.Item1?.ToList().ForEach(i =>
                        {
