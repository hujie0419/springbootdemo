...
namespace Tuhu.Provisioning.Areas.Activity.Controllers
{
    public class ActivityUnityController : Controller
    {
        ...
        public ActionResult GetContentForm(string group, int FKActiveID)
        {
            ...
            Expression<Func<ActivePageListEntity, bool>> getactiveexpression = o => o.PKID == FKActiveID;

            var colentity = new CommonConfigColInfoEntity()
            {
                Description = "车型图片配置",
                ColName = "VehicleImage"
            };

            colentity = GetOrCreateCommonConfigColInfoByColName(colentity);

            var list = repository.GetEntityList<ActivePageContentEntity>(expression);
            List<CommonConfigColValueInfoEntity> valueentities = new List<CommonConfigColValueInfoEntity>();
            if (list.Any(x => x.Type == 29))
            {
                ActivePageListEntity activepagelist = repository.GetEntity(getactiveexpression);
                Expression<Func<CommonConfigColValueInfoEntity, bool>> getvaluesexpression = x => x.ActiveHashKey == activepagelist.HashKey;
                valueentities = repository.GetEntityList(getvaluesexpression)?.ToList();
            }
            list?.ToList().ForEach(_ =>
            {
                ...
                if (_.Type == 29)
                {
                    if (valueentities != null && valueentities.Any())
                    {
                        var configs = valueentities.Where(x => x.ConfigInfoPkId == colentity.PKID && x.GroupId == _.GROUP);
                        _.JsonContent = JsonConvert.SerializeObject(configs);
                        //item.ActiveHashKey = activepagelist.HashKey;
                        //item.ConfigInfoPkId = colentity.PKID;
                        //item.GroupId = entity.GROUP;
                    }
        ...
        private CommonConfigColInfoEntity GetOrCreateCommonConfigColInfoByColName(CommonConfigColInfoEntity colentity)
        {
            RepositoryManager colrepository = new RepositoryManager();

            Expression<Func<CommonConfigColInfoEntity, bool>> getcolexpression = o => o.ColName == colentity.ColName;

            using (var db = colrepository.BeginTrans())
            {
                var coltemp = colrepository.GetEntity(getcolexpression);
                if (coltemp == null)
                {
                    db.Insert(colentity);
                    db.Commit();
            ...
            return colrepository.GetEntity(getcolexpression);
        }
        ...
        public ActionResult SubmitContent(string list, string keyValue = "", bool copy = false)
        {
            ...
            try
            {
                ...
                RepositoryManager checkrepository = new RepositoryManager();
                var id = entities.FirstOrDefault().FKActiveID;
                Expression<Func<ActivePageContentEntity, bool>> checkexpression = o => o.FKActiveID == id;
                var datalists = checkrepository.GetEntityList<ActivePageContentEntity>(checkexpression)?.ToList();

                var temp = datalists?.Where(_ => _.Type == 25);
                var entitytemp = entities.Where(x => x.Type == 25);

                if (temp != null && temp.Count() >= 2)
                {
                    if (entities.Any(x => x.Type == 25 && x.PKID == 0))
                    {
                        throw new Exception("最多配置2个文字链模块");
                    }
                }
                if (entities.Count(x => x.Type == 27) > 1)
                {
                    throw new Exception("最多配置1个倒计时模块");
                }
                var checktemp = entities.FirstOrDefault(x => x.Type == 27);
                if (checktemp != null)
                {
                    if (checktemp.PKID == 0 && datalists.Count(x => x.PKID != checktemp.PKID && x.Type == 27) > 0)
                    {
                        throw new Exception("最多配置1个倒计时模块");
                    }
                }
                //var cutitemitem = entities.FirstOrDefault(x => x.Type == 27);
                //if (cutitemitem != null)
                //{
                //    var tempCountDownStyle = datalists?.Where(_ => _.Type == 27);
                //    if (tempCountDownStyle != null && tempCountDownStyle.Count(x => x.PKID != cutitemitem.PKID) > 2)
                //    {
                //        throw new Exception("最多配置2个文字链模块");
                //    }
                //}
                if (entities.Any(x => x.Type == 29))
                {
                    RepositoryManager colrepository = new RepositoryManager();
                    var colentity = new CommonConfigColInfoEntity()
                    {
                    ...
                    var fkpkid = entities.FirstOrDefault().FKActiveID;

                    Expression<Func<ActivePageListEntity, bool>> getactiveexpression = o => o.PKID == fkpkid;

                    colentity = GetOrCreateCommonConfigColInfoByColName(colentity);
                    using (var db = colrepository.BeginTrans())
                    {
                        ActivePageListEntity activepagelist = colrepository.GetEntity(getactiveexpression);

                        var activehashkey = entities.FirstOrDefault().HashKey;
                        Expression<Func<CommonConfigColValueInfoEntity, bool>> deleteexpression = o => o.ActiveHashKey == activepagelist.HashKey;

                        var datas = (colrepository.GetEntityList(deleteexpression))?.ToList()?.Where(o => o.ConfigInfoPkId == colentity.PKID
                        && o.GroupId == entities.FirstOrDefault().GROUP);
                        if (datas != null && datas.Any())
                        {
                            foreach (var item in datas)
                            {
                                db.Delete<CommonConfigColValueInfoEntity>(item);
                            }

                        }

                        foreach (var entity in entities.Where(x => x.Type == 29))
                        {
                            try
                            {
                                if (!string.IsNullOrEmpty(entity.JsonContent))
                                {
                                    var valuelist = JsonConvert.DeserializeObject<List<CommonConfigColValueInfoEntity>>(entity.JsonContent);
                                    if (valuelist != null && valuelist.Any())
                                    {
                                        foreach (var item in valuelist)
                                        {
                                            ...
                                            item.ConfigInfoPkId = colentity.PKID;
                                            item.GroupId = entity.GROUP;
                                            db.Insert<CommonConfigColValueInfoEntity>(item);
                                        }
                                    }
                                }
                                entity.JsonContent = "";
                ...
                if (string.IsNullOrWhiteSpace(keyValue) || copy == true)
                {
                    ...
                    LoggerManager.InsertLog("ActivityPage", new FlashSaleProductOprLog()
                    {
                        ...
                        AfterValue = JsonConvert.SerializeObject(entities),
                ...
                if (entities.Any(x => x.Type == 30))
                {
                    using (var client = new Tuhu.Service.Activity.ActivityClient())
                    {
                        RepositoryManager activityidrepository = new RepositoryManager();
                        var datas = activityidrepository.GetEntityList<ActivePageContentEntity>(checkexpression)?.ToList();
                        if (datas.Any(x => x.Type == 30))
                        {
                            foreach (var item in datas.Where(x => x.Type == 30))
                            {
                                Expression<Func<ActivePageListEntity, bool>> getactiveexpression = o => o.PKID == item.FKActiveID;
                                ActivePageListEntity activepagelist = activityidrepository.GetEntity(getactiveexpression);
                                var activehashkey = entities.FirstOrDefault().HashKey;

                                var request = new SortedPidsRequest()
                                {
                                    ProductType = item.ProductType.HasValue ? (ProductType)item.ProductType.Value : ProductType.Default,
                                    Brand = item.Brand,
                                    NeedUpdatePkid = item.PKID,

                                };
                                if (item.ActivityType.HasValue && item.ActivityType.Value != 0 && item.ActivityID.HasValue)
                                {
                                    request.DicActivityId = new KeyValuePair<string, ActivityIdType>(item.ActivityID.Value.ToString(), ActivityIdType.FlashSaleActivity);
                                }
                                else
                                {
                                    request.DicActivityId = new KeyValuePair<string, ActivityIdType>(null, ActivityIdType.FlashSaleActivity);
                                }
                                var result = client.GetOrSetActivityPageSortedPids(request);

                                if (!result.Success || result.ErrorCode == "5")
                                {
        ...
        public ActionResult SubmitForm(ActivePageListEntity entity, int keyValue = 0)
        {
            if (keyValue <= 0)
            {
                ...
                LoggerManager.InsertLog("ActivityPage", new FlashSaleProductOprLog()
                {
                    ...
                    LogType = "ActivityPage",
                    LogId = entity.PKID.ToString()?.ToString(),
                    Operation = "新增活动",
                });
            ...
            else
            {
                ...
                LoggerManager.InsertLog("ActivityPage", new FlashSaleProductOprLog()
                {
                    OperateUser = User.Identity.Name,
                    CreateDateTime = DateTime.Now,
                    BeforeValue = JsonConvert.SerializeObject(beforeEntity),
                    AfterValue = JsonConvert.SerializeObject(entity),
                    LogType = "ActivityPage",
                    LogId = entity.PKID.ToString()?.ToString(),
                    Operation = "修改活动",
        ...
        public ActionResult DeleteForm(int pkid)
        {
            ...
            try
            {
                ...
                LoggerManager.InsertLog("ActivityPage", new FlashSaleProductOprLog()
                {
                    OperateUser = User.Identity.Name,
                    CreateDateTime = DateTime.Now,
                    BeforeValue = JsonConvert.SerializeObject(entity),
                    AfterValue = "",
                    LogType = "ActivityPage",
                    LogId = entity.PKID.ToString()?.ToString(),
                    Operation = "删除活动",
                ...
                try
                {
                    using (var client = new Tuhu.Service.Activity.ActivityClient())
                    {
                        var result1 = client.RefreshActivePageListModelCache(new Service.Activity.Models.Requests.ActivtyPageRequest()
                        {
                            Channel = "wap",
                            HashKey = entity.HashKey,
                        });
                        result1.ThrowIfException(true);
                        var result2 = client.RefreshActivePageListModelCache(new Service.Activity.Models.Requests.ActivtyPageRequest()
                        {
        ...
        public ActionResult DeleteContent(string group, int? fkActivityID)
        {
            try
            {
                ...
                LoggerManager.InsertLog("ActivityPage", new FlashSaleProductOprLog()
                {
                    OperateUser = User.Identity.Name,
                    CreateDateTime = DateTime.Now,
                    BeforeValue = JsonConvert.SerializeObject(entities),
                    AfterValue = "",
                    LogType = "ActivityPage",
                    LogId = fkActivityID?.ToString(),
                    Operation = "删除活动内容",
                });
                ...
                RefreshSource(fkActivityID); //刷新缓存
            ...
            if (Request.UrlReferrer != null && Request.UrlReferrer.ToString().ToLower().Contains("wechathome"))
            {
                ...
                allMenu.ChildNodes = new List<ClientMenuEnitity>() {
                    ...
                    new ClientMenuEnitity(){
                     ...
                     new ClientMenuEnitity(){
                        F_FullName="公众号素材管理",
                        F_Id="574687E8-6B9B-4E75-A222-E1FEFE332123",
                        F_ParentId=allMenu.F_Id,
                        F_SortCode=9,
                        F_UrlAddress="/WechatHome/WX_MaterialList"
                    },
                    new ClientMenuEnitity() {
                         F_FullName="领红包活动配置",
                         F_Id="F406BCCD-B2DC-4092-A96A-3D7021D31C46",
                         F_ParentId=allMenu.F_Id,
                         F_SortCode=10,
                         F_UrlAddress="/WechatHome/RedEnvelopeActivity"
                    },
                    new ClientMenuEnitity() {
                        F_FullName="每日发放红包概况",
                        F_Id="FC060CCD-B2DC-4092-A96A-3D7021D31C46",
                        F_ParentId=allMenu.F_Id,
                        F_SortCode=11,
                        F_UrlAddress="/WechatHome/RedEnvelopeStatistic"
                    }
                };
                list.Add(allMenu);
        ...
        /// <returns></returns>
        public ActionResult RefreshSource(int? keyValue)
        {
            if (keyValue == null)
                return Content(JsonConvert.SerializeObject(new
                {
            ...
            try
            {
                ...
                if (lucy != null)
                {
                    using (var client = new Tuhu.Service.Activity.ActivityClient())
                    {
                        var result = client.RefreshLuckWheelCache(lucy.ActivityID.ToString());
                        result.ThrowIfException(true);
                    }
                }
                return Content(JsonConvert.SerializeObject(new
        ...
        [Obsolete("整合到了 WechatHomeController.GetProductGroupInfoByPIdAsync")]
        ...
        public JsonResult SelectProductBrandsByCategory(string category)
        {
            using (var client = new Tuhu.Service.Product.ProductClient())
            {
                var result = client.GetBrandsByCategoryName(category);
                result.ThrowIfException(true);
        ...
        public JsonResult SelectFlashSaleDataByActivityID(string activityid)
        {
            Guid temp;
            ...
            if (Guid.TryParse(activityid, out temp))
            {
                using (var client = new Tuhu.Service.Activity.FlashSaleClient())
                {
                    var result = client.SelectFlashSaleDataByActivityID(temp);
                    result.ThrowIfException(true);
                    jr.Data = result.Result.ActivityID == temp ? 1 : 0;
                }
            }
            else
            {
        ...
        public ViewResult GetActivityPageListLog(string logId)
        {
            //if (string.IsNullOrEmpty(logId))
            //    return Json(null);
            var result = LoggerManager.SelectFlashSaleHistoryByLogId(logId, "ActivityPage");

            return View("ListBoostrap", result);
        }

        public ActionResult ViewConfigLog(int id)
        {
            var result = LoggerManager.GetFlashSaleHistoryByPkid(id) ?? new FlashSaleProductOprLog();
            return View("ActivityPageLog", result);
        }
    }
}