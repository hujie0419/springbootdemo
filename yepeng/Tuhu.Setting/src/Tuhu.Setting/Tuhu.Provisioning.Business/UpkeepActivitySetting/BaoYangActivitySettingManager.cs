...
namespace Tuhu.Provisioning.Business
{
    public class BaoYangActivitySettingManager
    {
        ...
        public List<BaoYangActivitySettingItem> GetBaoYangActivitySettingItemByBaoYangActivityId(int id)
        {
            try
            {
                return DalBaoYangActivitySetting.GetBaoYangActivitySettingItemByBaoYangActivityId(id);
            }
            catch (TuhuBizException)
            {
                throw;
        ...
        /// <returns></returns>
        public List<BaoYangTypeDescription> GetBaoYangTypesConfig(string type)
        {
            var result = null as List<BaoYangTypeDescription>;
            try
            {
                using (var client = new BaoYangClient())
                {
                    ...
                    clientResult.ThrowIfException(true);
                    var packageDescription = clientResult.Result;
                    result = packageDescription?.FirstOrDefault(s => string.Equals(type, s.PackageType))?.Items?.ToList();
                }
            }
            catch (Exception ex)
            {
