...
namespace Tuhu.Service.Activity.Server.Manager
{
    public static class ZeroActivityManager
    {
        ...
        private static async Task<IEnumerable<ZeroActivityModel>> GetAllZeroActivityList(bool resetCache = false)
        {
            using (var client = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                if (resetCache)
                {
                    ...
                    var setResult = await client.SetAsync(ZeroActivityListKey, allZeroActivityList, TimeSpan.FromHours(1));
        ...
        public static async Task<bool> RefreshZeroActivityCache()
        {
            ...
            {
                var setUnfinishedZaKeyResult = await client.SetAsync(
                    GlobalConstant.UnfinishedZeroActivitiesForHomepage,
                    RedisHelper.GenerateZeroActivityCacheKeyPrefix(GlobalConstant.UnfinishedZeroActivitiesForHomepage),
                    GlobalConstant.ZeroActivitiesForHomepageExpiration);
                ...
                var setFinishedZaKeyResult = await client.SetAsync(GlobalConstant.FinishedZeroActivitiesForHomepage,
                    RedisHelper.GenerateZeroActivityCacheKeyPrefix(GlobalConstant.FinishedZeroActivitiesForHomepage),
                    GlobalConstant.ZeroActivitiesForHomepageExpiration);
                ...
                var setZaDetailKeyResult = await client.SetAsync(GlobalConstant.ZeroActivityDetail,
                    RedisHelper.GenerateZeroActivityCacheKeyPrefix(GlobalConstant.ZeroActivityDetail),
                    GlobalConstant.ZeroActivityDetailExpiration);

                if (!setZaDetailKeyResult.Success)
                {
                    Logger.Warn("更新零元购活动详情缓存Redis缓存失败", setZaDetailKeyResult.Exception);
                }
                result = setUnfinishedZaKeyResult.Success && setFinishedZaKeyResult.Success && setZaDetailKeyResult.Success;
            }

            // 获取 所有 未过期的 的 众测配置
            var zeroActivities = await GetAllZeroActivityList(true);
            var zeroActivityModels = zeroActivities as ZeroActivityModel[] ?? zeroActivities.ToArray();
            if (zeroActivityModels.Any())
            {
                var products = await GetAllZeroActivityProductsList(zeroActivityModels, true);

                using (var client2 = CacheHelper.CreateCacheClient(DefaultClientName))
                {
                    var zeroActivityList = await MapZeroActivities(zeroActivityModels, products, true);
                    var setResult = await client2.SetAsync(ZeroActivityCacheListKey, zeroActivityList,
                        TimeSpan.FromMinutes(30));
                    if (!setResult.Success)
                    {
                        Logger.Error($"设置redis缓存({ZeroActivityCacheListKey})失败;Error:{setResult.Message}",
                    ...
                    var data = await DalZeroActivity.SelectZeroActivitySimpleModelAsync();
                    var result2 = await client2.SetAsync(GlobalConstant.HomepageModeuleShowZeroActivityKey, data, TimeSpan.FromHours(1));

                    if (!result2.Success)
                    {
                        Logger.Error($"设置redis缓存失败key：{GlobalConstant.HomepageModeuleShowZeroActivityKey}");
                    }
                    //刷新 首页的 0元众测的 新加 缓存
                    var result3 = client2.Set(GlobalConstant.HomepageZeroActivityListKey,
                     GetHomePageZeroActivityData(data.OrderByDescending(r => r.Period).Take(2).ToList()), TimeSpan.FromHours(1));
                    if (!result3.Success)
                    {
                        Logger.Error($"设置redis缓存失败key：{GlobalConstant.HomepageZeroActivityListKey}");
                    }
                    result = setResult.Success && result2.Success&& result3.Success;
                }
            }
            return result;
        }
        ...
        private static async Task<List<DalZeroActivityModel>> SelectZeroActivityForHomePageCacheAsync()
        {
            using (var cache = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                var result = await cache.GetOrSetAsync(GlobalConstant.HomepageModeuleShowZeroActivityKey,
                    DalZeroActivity.SelectZeroActivitySimpleModelAsync, TimeSpan.FromHours(1));
                if (result.Success)
                {
                ...
                {
                    Logger.Error($"SelectZeroActivityForHomePageCacheAsync从redis获取数据失败");
                    return new List<DalZeroActivityModel>();
                }
            }
        }

        private static List<DalZeroActivityModel> CustomerGroupAndSort(this List<DalZeroActivityModel> soruces)
        {
            var list = new List<DalZeroActivityModel>();
            var acting = soruces.Where(r => r.StartDateTime <= DateTime.Now && r.EndDateTime >= DateTime.Now)?.ToList();
            if (acting.Any())
            {
                list.AddRange(acting.OrderBy(r => r.Period));
            }
            var acttodo = soruces.Where(r => r.StartDateTime > DateTime.Now).ToList();
            if (acttodo.Any())
            {
                list.AddRange(acttodo.OrderBy(r => r.Period));
            }
            var acteding = soruces.Where(r => r.EndDateTime < DateTime.Now && r.EndDateTime.AddDays(30) >= DateTime.Now).ToList();
            if (acteding.Any())
            {
                list.AddRange(acteding.OrderByDescending(r => r.Period));
            }
            var acted = soruces.Where(r => r.EndDateTime.AddDays(30) < DateTime.Now).ToList();
            if (acted.Any())
            {
                list.AddRange(acted.OrderByDescending(r => r.Period));
            }
            return list;
        }
        ...
        /// <returns></returns>
        private static  List<ZeroActivitySimpleRespnseModel> GetHomePageZeroActivityData(List<DalZeroActivityModel> data)
        {
            List<ZeroActivitySimpleRespnseModel> homeData = data.Select(r => new ZeroActivitySimpleRespnseModel
            {
                Pid = r.Pid,
                Quantity = r.Quantity,
                NumOfWinners = r.NumOfWinners,
                NumOfApplications = AsyncHelper.RunSync(() => FetchNumOfAppliesOnPeriod(r.Period))
            }).ToList();
            return homeData;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static async Task<List<ZeroActivitySimpleRespnseModel>> SelectHomePageModuleShowZeroActivityAsync()
        {
            var db = await SelectZeroActivityForHomePageCacheAsync();
            var sortedCacheResult = db.OrderByDescending(r=>r.Period).Take(2).ToList();

            //return sortedCacheResult.Select(r => new ZeroActivitySimpleRespnseModel
            //{
            //    Pid = r.Pid,
            //    Quantity = r.Quantity,
            //    NumOfWinners = r.NumOfWinners,
            ...
            using (var cache = CacheHelper.CreateCacheClient(DefaultClientName))
            {
                var result = cache.GetOrSet(GlobalConstant.HomepageZeroActivityListKey,
                    ()=>GetHomePageZeroActivityData(sortedCacheResult), TimeSpan.FromHours(1));
                if (result.Success)
                {
                    return result.Value.ToList();
                }
                ...
                {
                    Logger.Error($"SelectZeroActivityForHomePageCacheAsync从redis获取数据失败");
                    return new List<ZeroActivitySimpleRespnseModel>();
                }
            }
        }
        #endregion
