...
namespace Tuhu.Provisioning.Business
{
    public class SE_HomePageConfigManager
    {
        ...
        public bool DeleteModule(int id)
        {
            using (var connection = ProcessConnection.OpenGungnir)
            {
                return new DALSE_HomePageConfig().DeleteHomePageModule(connection, id);
            }
        }
        ...
        /// <returns></returns>
        public bool AddHomePageModuleHelper(SE_HomePageModuleHelperConfig model)
        {
            using (var c = ProcessConnection.OpenGungnir)
            {
                ...
                {
                    List<SE_ModuleHelperCityConfig> list = new List<SE_ModuleHelperCityConfig>();
                    string[] citys = model.City.TrimEnd(',').Split(',');
                    foreach (var s in citys)
                    {
                        if (string.IsNullOrWhiteSpace(s))
                            continue;

                        SE_ModuleHelperCityConfig city = new SE_ModuleHelperCityConfig();
                        city.CreateDateTime = DateTime.Now;
                        city.UpdateDateTime = DateTime.Now;
                        city.FKRegionPKID = Convert.ToInt32(s);
                        city.UpdateDateTime = DateTime.Now;
                        list.Add(city);
                    }
                    return dal.AddHomePageModuleHelper(c, model, list);
                }
                else
                {
                    return dal.AddHomePageModuleHelper(c, model, null);
                }
            }
        }
        ...
        public bool UpdateHomePageModuleHelper(SE_HomePageModuleHelperConfig model)
        {
            using (var c = ProcessConnection.OpenGungnir)
            {
                DALSE_HomePageConfig dal = new DALSE_HomePageConfig();
                if (!string.IsNullOrWhiteSpace(model.City))
                {
                    ...
                    return dal.UpdateHomePageModuleHelper(c, model, list);
                }
                else
                {
                    return dal.UpdateHomePageModuleHelper(c, model, null);
                }
            }
        }
        ...
        public int AddHomePageContent(SE_HomePageModuleContentConfig model)
        {
            using (var c = ProcessConnection.OpenGungnir)
            {
                return new DALSE_HomePageConfig().AddHomePageContent(c, model);
            }
        }


        public bool UpdateHomePageContent(SE_HomePageModuleContentConfig model)
        {
            using (var c = ProcessConnection.OpenGungnir)
            {
                return new DALSE_HomePageConfig().UpdateHomePageContent(c, model);
            }
        }

        public bool DeleteHomePageContent(int id)
        {
            using (var c = ProcessConnection.OpenGungnir)
            {
                return new DALSE_HomePageConfig().DeleteHomePageContent(c, id);
            }
        }
                ...
                foreach (var module in moduleList)
                {
                        ...
                        foreach (var moduleHelper in moduleHelperList)
                        {
                            ...
                            if (moduleHelperContentList != null)//添加内容
                                foreach (var content in moduleHelperContentList)
                                {
                                   ...
                                   var contentHelperPKID=  dal.AddHomePageContent(connection, transaction, content);

                                    if (moduleHelper.ModuleType == 30)
                                    {
                                        System.Linq.Expressions.Expression<Func<Tuhu.Provisioning.DataAccess.Mapping.SE_HomePageContentPuzzle, bool>> exp = _ => _.FKID == content.ID;
                                        var puzzleEntity = managerReadOnely.GetEntity<Tuhu.Provisioning.DataAccess.Mapping.SE_HomePageContentPuzzle>(exp);
                                        if (puzzleEntity != null)
                                        {
                                            puzzleEntity.FKID = contentHelperPKID;
                                            puzzleEntity.FKModuleHelperID = moduleHelperPKID;
                                            manager.Add<Tuhu.Provisioning.DataAccess.Mapping.SE_HomePageContentPuzzle>(puzzleEntity);
                                        }
                                    }
                                }
                        }
                    ...
                    if (moduleContentList != null)//添加内容
                        foreach (var content in moduleContentList)
                        {
                            ...
                            int contentPKID = dal.AddHomePageContent(connection, transaction, content);
