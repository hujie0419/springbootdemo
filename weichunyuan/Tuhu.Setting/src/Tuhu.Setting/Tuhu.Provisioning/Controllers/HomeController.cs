...
namespace Tuhu.Provisioning.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        public JsonResult RefreshUserRoles()
        {
            bool result = false;
            string msg = null;
            try
            {
                ...
                {
                    string key = Component.Framework.EnOrDeHelper.GetMd5(User.Identity.Name + "power!@#$" + "BSys", Encoding.UTF8);
                    new PowerManage().GetBusPower(key);
                    result = true;
    }
}
            catch (Exception ex)
            {
                msg = $"RefreshUserRoles=>{ex.Message}";
            }
