...
function GetHuoDongImage() {
    ...
    $.ajax({
        ...
        url: "http://huodong.tuhu.cn/Activity/GetBatteryBanner",
...
function Login() {
    if (userAgentInfo.indexOf("Android") >= 0) {
        window.WebViewJavascriptBridge.callHandler('actityBridge', {
            'param': ""
            ...
            if (data.Code == "1") {
                ...
                if (userAgentInfo.indexOf("Android") >= 0) {
                    ...
                    if (!isAndroidVersion) {
                        ...
                        window.WebViewJavascriptBridge.callHandler('setUserCarInfo', function () {});
                    }
                } else {
                    if (isIosVersion) {
                        NoneFooter();
...
function SelectProvinceItem(Pid, CurrentElement) {
    ...
    var AreaCityCode = '';
    $.each(CurrentCityList, function (k, v) {
        //迭代城市节点
...
function SelectCityItem(Cid, CurrentElement) {
    ...
    CurrentCityItem = CurrentCityList.filter(function (v) {
        if (Cid == v.Cid) return !0;return !1;
    })[0]; //筛选当前城市节点
    $("#city-ul").hide();
    AppendProudts();
}
...
function AppendProudts() {
    ...
    for (var item in currentProductItem) {
        if (currentProductItem[item].ProductID) {
            ...
            htmlProdiuct += ' <div class="product" style="display:none;">' + '<div class="t clear">' + '<div class="checkbox">选择</div>' + '<img style="display:none" src="img/icon.png" alt="" class="right" />' + '<span>&yen; <span>' + currentProductItem[item].Price + '</span></span>' + '<p style="display:none;" class="datajson">' + JSON.stringify(currentProductItem[item]) + '</p>' + '</div>' + '<div class="m clear">' + '<img src="' + imgUrl + '" alt="" />' + '<div>' + '<div>' + currentProductItem[item].DisplayName + '</div>' + tipHtml + '<div>配送范围：' + currentProductItem[item].ServiceRemark + '</div>' + '</div>' + '</div>' + '</div>';
        } else {}
    }
...
function PayClick() {
    ...
    if (!$(this).hasClass("disabled") && !($(".areaSelect>li").eq(1).find(".selectItem").children(".default").text() == "城市/地区、自治州")) {
        if ($(".p-price").text() != "" && $(".p-price").text() != null) {
            ...
            window.location.href = "http://huodong.tuhu.cn/ActivityHtml/BatteryV1/Order.html?RegionSelect=" + selectRegions + "&v=7";
        } else {
            alert("请选择产品！");
        }
    } else {
...
function AutoFillRegion() {
    var RegionPid = GetCookie("RegionPid"),
        RegionCid = GetCookie("RegionCid");
