...
using framework= Tuhu.Component.Framework;
...
namespace Tuhu.Provisioning.Controllers
{
    public class BeautyHomePageConfigController : Controller
    {
        ...
        public ActionResult List(BeautyHomePageConfig model, int pageIndex = 1, int pageSize = 15)
        {
            ...
            var list = new framework.OutData<List<BeautyHomePageConfig>, int>(lists, count);
        ...
        [HttpPost]
        public async Task<JsonResult> SaveBeautyBannerConfig(BeautyHomePageConfig model)
        {
            var result = false;
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    await UpdateBeautyBannerCache(model.Channel);
                }
        ...
        public JsonResult GetPopUpWindowsConfigList()
        {
            ...
            list.Select(s => s.Channel).Distinct()?.ToList().ForEach(f =>
            {
