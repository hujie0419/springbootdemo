...
WHERE   EUTP.AuditStatus = 0; ").ConvertTo<PriceUpdateAuditModel>();
        ...
        /// <returns></returns>
        public static IEnumerable<PriceUpdateAuditModel> SelectAuditLogByPID(string pID, PagerModel pager, string type)
        {
            using (var dbHelper = new SqlDbHelper(ConnectionHelper.GetDecryptConn("Gungnir_AlwaysOnRead")))
            {
                                        ...
                                        WHERE       (
                                            ...
                                            OR  @PID IS NULL
                                                    ...
                                                    AND (
                                                        EUTP.Type = @type
                                                        OR  EUTP.Type = N''
                                                        OR  EUTP.Type IS NULL
                                                    )
                                        ORDER BY    PKID DESC OFFSET (@PageIndex - 1) * @PageSize ROWS FETCH NEXT @PageSize ROWS ONLY;", CommandType.Text,
                                                   new SqlParameter[] {
