    ...
    public class CommentController : Controller
    {
            ...
            var od = dc.Orders.Where(o => o.PKID.Equals(model.CommentOrderId)).FirstOrDefault();
                ...
                if (commentStatus == 2)
                {
                    ...
                    try
                    {
                        ...
                        {
                            var result = await client.ProductCommentApprovedAsync(CommentId);
                            if (result.Success)
                            {
                                isSuccess = " + ES";
                            }
                        ...
                        try
                        {
                            using (var client = new PushClient())
                            {
                                if (data != null && od != null && !string.IsNullOrEmpty(od.UserTel))
                                {
                                    ...
                                    var result = await client.PushMessagesAsync(message);
                                    if (result.Success)
                                    {
                                        isSuccess += " + Push";
                                    }
                    ...
                    if (model.CommentType == 2)//1、晒单
                    {
                        if (od != null)
                        {
                            ...
                            if (od.InstallShopID > 0 && od.OrderType == "1普通")
                            {
                                ...
                                {
                                    var result = await client.InsertOrderToShareAsync((Guid)od.UserID, od.PKID, true);
                                    if (result.Success)
                                    {
                                        isSuccess = " + ShaiDan";
                                    }
                ...
                if (commentStatus == 0 || commentStatus == 3)
                {
                    ...
                    try
                    {
                        using (var client = new Tuhu.Service.Comment.ProductCommentClient())
                        {
                            var result = await client.ProductCommentUnapprovedAsync(CommentId);
                            if (result.Success)
                            {
                                isSuccess = " - ES";
                            }
        ...
        public ActionResult UpdateOfficialReply(int CommentID, string OfficialReply)
        {
            ...
            try
            {
                using (var client = new Tuhu.Service.Comment.ProductCommentClient())
                {
                    var result = client.ProductCommentApproved(CommentID).Result;
                    if (result)
                    {
                        isSuccess = " + ES";
                    }
        ...
        public ActionResult ExcelExport(DateTime startDate,DateTime endDate)
        {
            ...
            if (dtComment != null && dtComment.Rows.Count>0)
            {
                ...
                var tousuInfo = new List<Tuple<int, string>>();
                if (ids.Any())
                {
                    tousuInfo = CommentReportManager.GetTousuType(ids);
                }
                foreach (DataRow item in dtComment.Rows)
                {
                    NPOI.SS.UserModel.IRow rowtemp = sheet1.CreateRow(rowNum++);
                    ...
                    int.TryParse(item["CommentId"]?.ToString(), out var id);
                    var tousu = tousuInfo.FirstOrDefault(g => g.Item1 == id)?.Item2;
                    if (!string.IsNullOrWhiteSpace(tousu))
                    {
                        rowtemp.CreateCell(16).SetCellValue(tousu);
                    }
                }
            }
            System.IO.MemoryStream ms = new System.IO.MemoryStream();
