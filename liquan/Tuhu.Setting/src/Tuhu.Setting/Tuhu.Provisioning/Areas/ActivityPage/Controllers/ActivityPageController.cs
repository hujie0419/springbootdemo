...
namespace Tuhu.Provisioning.Areas.ActivityPage.Controllers
{
    public class ActivityPageController : Controller
    {
        ...
        /// <returns></returns>
        private bool GetRuntimeSwitch(string SwitchName)
        {
            RuntimeSwitchResponse resultRuntimeSwitch = null;
            ...
            try
            {
                ...
                {
                    var result = client.GetOrSetRuntimeSwitch(SwitchName);

                    if (!result.Success)
                        logger.Log(Level.Info, "GetRuntimeSwitch服务调用失败");
                    else
                    {
                        if (result.Result != null)
                        {
                            resultRuntimeSwitch = result.Result;
                        }
                    }
                }

                if (resultRuntimeSwitch != null)
                {
                    if (resultRuntimeSwitch.Value)
                    {
                        var userName = User.Identity.Name;

                        if (userName != "guxiaowen@tuhu.cn" && userName != "huayangyong@tuhu.cn" &&
                            userName != "liuchao1@tuhu.cn")
                        {
                            resultValue = false;
                        }
                    }
            ...
            {
                logger.Log(Level.Error, ex, "GetRuntimeSwitch开关查下接口异常：" + ex.Message + ";堆栈信息：" + ex.StackTrace);
            }
        ...
        public async Task<JsonResult> SaveActivityInfo(ActivityInfoRequest activityInfoRequest)
        {
            ...
            if (!GetRuntimeSwitch("DisableActivityPageSetting"))
            {
                activityInfoResponse.ResponseCode = "0009";
                activityInfoResponse.ResponseMessage = "您无权限操作此页面,请联系管理员!";
            }
            else
            {
                try
        ...
        public async Task<JsonResult> GetActivityList(GetActivityListRequest getActivityListRequest)
        {
            ...
            if (!GetRuntimeSwitch("DisableActivityPageSetting"))
            {
                ...
                getActivityListResponse.ResponseMessage = "您无权限访问此页面,请联系管理员!";
            }
            else
            {
        ...
        /// <returns></returns>
        public async Task<JsonResult> SaveMinuteFigures(SaveMinuteFiguresRequest request)
        {
            request.CurrentUser = User.Identity.Name;
            var response = new SaveMinuteFiguresResponse();

            try
            {
                using (var activityPageClient = new ActivityPageSettingClient())
                {
                    var result = await activityPageClient.SaveMinuteFiguresAsync(request);
            ...
            {
                logger.Log(Level.Error, ex, "SaveMinuteFigures");
            ...
            if (this.HttpContext.Request.Files.Count > 0)
            {
                ...
                using (FileUploadClient fileUploadClient = new FileUploadClient())
                {
                    if (request.FileType == FileType.Image.ToString())
                    {
                        ...
                        {
                            var result = fileContent.UpdateLoadImage();
                            fileUploadResponse.ResponseLinkUrl = result.Item1 ? result.Item2 : "";
                        }
                    }
        ...
        /// 保养分车型保存接口
        /// </summary>
        /// <param name="request">保养分车型保存 请求参数</param>
        /// <returns></returns>
        public async Task<ActionResult> SaveMaintenanceVehicle(SaveMaintenanceVehicleRequest request)
        {
            request.CurrentUser = User.Identity.Name;
            var response = new SaveMaintenanceVehicleResponse();

            try
            {
                using (var activityPageClient = new ActivityPageSettingClient())
                {
                    var result = await activityPageClient.SaveMaintenanceVehicleAsync(request);
            ...
            {
                logger.Log(Level.Error, ex, "SaveMaintenanceVehicle");
                response.ResponseCode = "0009";
                response.ResponseMessage = "程序异常";
            }
        ...
        /// 保养分车型 查询接口
        /// </summary>
        /// <param name="request">保养分车型 查询接口请求参数</param>
        /// <returns></returns>
        public async Task<ActionResult> GetMaintenanceVehicle(GetMaintenanceVehicleRequest request)
        {
            var response = new GetMaintenanceVehicleResponse();

            try
            {
                using (var activityPageClient = new ActivityPageSettingClient())
                {
                    var result = await activityPageClient.GetMaintenanceVehicleConfigAsync(request);
            ...
            {
                logger.Log(Level.Error, ex, "GetMaintenanceVehicle");
                response.ResponseCode = "0009";
                response.ResponseMessage = "程序异常";
            }
            return Json(new { data = response }, JsonRequestBehavior.AllowGet);
        ...
        /// 获取活动页看板列表
        /// </summary>
        /// <param name="request">活动页看板列表查询请求实体</param>
        /// <returns></returns>
        [HttpGet]
        public async Task<JsonResult> GetActivityBoardInfoList(GetActivityBoardInfoListRequest request)
        {
            var getActivityBoardInfoListResponse = new GetActivityBoardInfoListResponse();
            try
            {
                using (var activityPageClient = new ActivityPageSettingClient())
                {
                    var result = await activityPageClient.GetActivityBoardInfoListAsync(request);

                    if (!result.Success)
                    {
                        getActivityBoardInfoListResponse.ResponseCode = "0005";
                        getActivityBoardInfoListResponse.ResponseMessage = result.ErrorMessage;
                    }
                    else
                    {
                        getActivityBoardInfoListResponse = result.Result;
                    }
                }
            }
            catch(Exception ex)
            {
                logger.Log(Level.Error, ex, "GetActivityBoardInfoList");
                getActivityBoardInfoListResponse.ResponseCode = "0009";
                getActivityBoardInfoListResponse.ResponseMessage = "程序异常";
            }
            return Json(new { data = getActivityBoardInfoListResponse }, JsonRequestBehavior.AllowGet);
        }
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<JsonResult> GetActivityBoardDetailInfo(string activityID)
        {
            var getActivityBoardDetailInfoResponse = new GetActivityBoardDetailInfoResponse();
            try
            {
                using (var activityPageClient = new ActivityPageSettingClient())
                {
                    var result = await activityPageClient.GetActivityBoardDetailInfoAsync(activityID);

                    if (!result.Success)
                    {
                        getActivityBoardDetailInfoResponse.ResponseCode = "0005";
                        getActivityBoardDetailInfoResponse.ResponseMessage = result.ErrorMessage;
                    }
                    else
                    {
                        getActivityBoardDetailInfoResponse = result.Result;
                    }
                }
            }
            catch (Exception ex)
            {
                logger.Log(Level.Error, ex, "GetActivityBoardDetailInfo");
                getActivityBoardDetailInfoResponse.ResponseCode = "0009";
                getActivityBoardDetailInfoResponse.ResponseMessage = "程序异常";
            }
            return Json(new { data = getActivityBoardDetailInfoResponse }, JsonRequestBehavior.AllowGet);
        }
        #endregion
    }
}