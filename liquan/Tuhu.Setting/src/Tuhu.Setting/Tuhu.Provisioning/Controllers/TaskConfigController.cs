...
namespace Tuhu.Provisioning.Controllers
{
    public class TaskConfigController : Controller
    {
        ...
        public JsonResult EditTaskConfig(TaskConfigModel request)
        {
            ...
            var code = 1;
            var info = "操作完成";
            if (request.ConditionList?.ToList().Distinct(g => g.ActionName).Count() > 1 && actionList.Intersect(request.ConditionList.Select(g => g.ActionName)).Any())
            {
                code = 0;
                info = "任务完成条件不符合要求";
            }
        ...
        public JsonResult UploadImage()
        {
            ...
            if (Request.Files.Count > 0)
            {
                ...
                {
                    framework.WebLog.LogException(exp);
                }
            }
            return Json(result);
        }
