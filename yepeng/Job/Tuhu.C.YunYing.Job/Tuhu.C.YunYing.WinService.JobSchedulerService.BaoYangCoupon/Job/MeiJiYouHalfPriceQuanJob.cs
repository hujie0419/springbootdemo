...
{
    /// <summary>到店保养券 huhengxing 2016-6-30 美系机油半价购活动 已过期</summary>
    [DisallowConcurrentExecution]
    public class MeiJiYouHalfPriceQuanJob : IJob
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(MeiJiYouHalfPriceQuanJob));
        ...
        public void Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                {
                    Logger.Info("2016-7-1开始发放美系机油半价购活动返券");
                    return;
                }
                if (DateTime.Now > new DateTime(2016, 9, 1))
                {
                    Logger.Info("2016-9-1结束发放美系机油半价购活动返券");
                    return;
                }
                Logger.Info("MeiJiYouHalfPriceQuanJob 启动任务");
                using (var cmd = new SqlCommand(@"[Gungnir].[dbo].[ProMotionCode_MeiJiYouHalfPriceQuan]"))
                {
                    ...
                    foreach (var i in dic)
                    {
                        if (!string.IsNullOrWhiteSpace(i.SumMoney))
                        {
                            ...
                            if (result > 0)
                            {
                                BLL.Business.SendMarketingSms(i.UserTel, "美系机油半价购", m, "途虎现金券");
                            }
                        }
                    }
                }

                Logger.Info("MeiJiYouHalfPriceQuanJob 结束任务");
            }
            catch (Exception ex)
            {
        ...
        public static int CreateOrYzPromotion(string userId, int money, string orderID)
        {
            try
            {
                ...
                promotionCodeModel.Description = "美系机油半价购活动返券，途虎全网到店保养项目通用";
                ...
                promotionCodeModel.CodeChannel = "美系机油半价购活动返券";
