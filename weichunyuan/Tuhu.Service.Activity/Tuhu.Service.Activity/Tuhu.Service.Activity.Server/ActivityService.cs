...
namespace Tuhu.Service.Activity.Server
{
    public class ActivityService : IActivityService
    {
        ...
        /// 分页获取报名参加活动的用户列表
        /// </summary>
        public async Task<OperationResult<PagedModel<UserRegistrationResponse>>> GetUserRegistrationsByAreaAsync(string area, int pageSize = 20, int pageIndex = 1)
        {
            return OperationResult.FromResult(await ActivityManager.GetUserRegistrationsByAreaAsync(area, pageSize, pageIndex));
        }

        /// <summary>
        /// 获取用户活动列表
        /// </summary>
        public async Task<OperationResult<List<UserActivityResponse>>> GetUserActivitiesAsync()
        {
            return OperationResult.FromResult(await ActivityManager.GetUserActivitiesAsync());
        }

        /// <summary>
        /// 获取用户报名信息
        /// </summary>
        public async Task<OperationResult<UserRegistrationResponse>> GetUserRegistrationByIdAsync(long pkid)
        {
            if (pkid < 1) return OperationResult.FromError<UserRegistrationResponse>(ErrorCode.ParameterError, "请提供正确的用户报名ID");
            return OperationResult.FromResult(await ActivityManager.GetUserRegistrationByIdAsync(pkid));
        }

        /// <summary>
        /// 新增用户报名信息
        /// </summary>
        public async Task<OperationResult<bool>> AddUserRegistrationAsync(UserRegistrationRequest request)
        {
            if (string.IsNullOrEmpty(request.Name) ||
                string.IsNullOrEmpty(request.Phone) ||
                string.IsNullOrEmpty(request.Area) || request.ActivityId < 1)
                return OperationResult.FromError<bool>(ErrorCode.ParameterError, "请提供完整的参数信息");

            return OperationResult.FromResult(await ActivityManager.AddUserRegistrationAsync(request));
        }

        /// <summary>
        /// 修改用户报名信息
        /// </summary>
        public async Task<OperationResult<bool>> ModifyUserRegistrationAsync(UserRegistrationRequest request)
        {
            if (string.IsNullOrEmpty(request.Name) ||
                string.IsNullOrEmpty(request.Phone) ||
                string.IsNullOrEmpty(request.Area) ||
                request.ActivityId < 1 || request.PKID < 1)
                return OperationResult.FromError<bool>(ErrorCode.ParameterError, "请提供完整的参数信息");

            return OperationResult.FromResult(await ActivityManager.ModifyUserRegistrationAsync(request));
        }
        ...
        public async Task<OperationResult<bool>> UpdateUserRegistrationStatusAsync(string area, RegistrationStatus status)
        {
            return OperationResult.FromResult(await ActivityManager.UpdateUserRegistrationStatusAsync(area, status));
        }
    }
}
